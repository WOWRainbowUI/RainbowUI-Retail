...
----------------------------------------------------------
	...
	local defaults={
		global={
			...
			alpha=1, shortk=10000, nK="萬", nM="百萬",
			classification={
				worldboss=L[" Boss"],
				rareelite=L["+ Rare"],
				elite=L["+"],
...
options={
	...
	args={ 
		configmode={
			name=L["Config"], desc=L["Preview everything."], type="toggle", order=1, width="half", 
			set=function(info, v)
		...
		highlight={ 
			name=L["Highlight"], desc=L["Highlights currently selected element."], type="toggle", order=2, width="half", 
			set=function(info, v)
		...
		movable={
			name=L["Drag"], desc=L["draghelp"], type="toggle", order=4, width="half",
			set=function(info, v)
				if InCombatLockdown() then
					return ChatFrame1:AddMessage("|cff00ff00Stuf|r: "..L["Unable to process while in combat."])
				end
		...
		args = {
			...
			importbtn = {
				...
				func = function()
					...
					if not result then
						importStatus = L["|cffff0000Import failed:|r "]..( err or "unknown error")
