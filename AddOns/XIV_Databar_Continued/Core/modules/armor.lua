...
function ArmorModule:CreateFrames()
	...
	self.coordText:SetFont(xb:GetFont(xb.db.profile.text.fontSize)) -- 自行修正
end
...
function ArmorModule:UpdatePlayerCoordinates()
    ...
    x = floor(((x - rects[1]) / rects[3]) * 1000) / 10
    y = floor(((y - rects[2]) / rects[4]) * 1000) / 10
