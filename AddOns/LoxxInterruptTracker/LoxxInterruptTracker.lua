...
local ALL_INTERRUPTS = {
    [6552]   = { name = "拳擊", cd = 15, icon = 132938 }, -- Pummel
    [1766]   = { name = "腳踢", cd = 15, icon = 132219 }, -- Kick
    [2139]   = { name = "法術反制", cd = 24, icon = 135856 }, -- Counterspell
    [57994]  = { name = "風切", cd = 12, icon = 136018 }, -- Wind Shear
    [106839] = { name = "碎顱猛擊", cd = 15, icon = 236946 }, -- Skull Bash
    [78675]  = { name = "太陽光束", cd = 60, icon = 236748 }, -- Solar Beam
    [47528]  = { name = "心靈冰封", cd = 15, icon = 237527 }, -- Mind Freeze
    [96231]  = { name = "斥責", cd = 15, icon = 523893 }, -- Rebuke
    [183752] = { name = "瓦解", cd = 15, icon = 1305153 }, -- Disrupt
    [116705] = { name = "插入手", cd = 15, icon = 608940 }, -- Spear Hand Strike
    [15487]  = { name = "沉默", cd = 45, icon = 458230 }, -- Silence
    [147362] = { name = "反制射擊", cd = 24, icon = 249170 }, -- Counter Shot
    [187707] = { name = "閉嘴", cd = 15, icon = 1376045 }, -- Muzzle
    [19647]  = { name = "法術封鎖", cd = 24, icon = 136174 }, -- Spell Lock
    [132409] = { name = "法術封鎖", cd = 24, icon = 136174 }, -- Spell Lock (另一版本)
    [119914] = { name = "斧擊", cd = 30, icon = "Interface\\Icons\\ability_warrior_titansgrip" }, -- Axe Toss
    [1276467] = { name = "惡魔劫掠者", cd = 25, icon = "Interface\\Icons\\spell_shadow_summonfelhunter" }, -- Fel Ravager
    [351338] = { name = "壓制", cd = 20, icon = 4622469 }, -- Quell
}
...
------------------------------------------------------------
local DEFAULTS = {
    frameWidth      = 200,  -- fixed — no auto-scaling
...
local function ShowChangelogWindow()
    ...
    hdrTitle:SetText("|cFFFFD100更新日誌|r")
    ...
    txt:SetText(LOXX_CHANGELOG or "無可用的更新日誌")
...
local function CreateConfigPanel()
    ...
    hdrTitle:SetPoint("TOP", 0, -34)
    hdrTitle:SetJustifyH("CENTER")
    hdrTitle:SetText("|cFFFFD100打斷追蹤|r")
    ...
    widthSlider.Text:SetText("寬度: " .. tostring(initW) .. "px")
    widthSlider.Low:SetText("120")
    widthSlider.High:SetText("400")
    widthSlider:SetScript("OnValueChanged", function(self, value)
        value = math.floor(value / 10 + 0.5) * 10
        db.frameWidth = value
        self.Text:SetText("寬度: " .. tostring(value) .. "px")
        RebuildBars()
    end)
    ...
    heightSlider.Text:SetText("高度: " .. tostring(initH) .. "px")
    heightSlider.Low:SetText("14")
    heightSlider.High:SetText("50")
    heightSlider:SetScript("OnValueChanged", function(self, value)
        value = math.floor(value + 0.5)
        db.barHeight = value
        self.Text:SetText("高度: " .. tostring(value) .. "px")
        RebuildBars()
    end)
    ...
    VisCheck(configFrame, "地城", R_CBX1, yR, "showInDungeon")
    VisCheck(configFrame, "競技場", R_CBX2, yR, "showInArena")
    yR = yR - 28
    VisCheck(configFrame, "野外", R_CBX1, yR, "showInOpenWorld")

    -- SOUND
    yR = yR - 40
    SectionLabelR("音效", yR)
    yR = yR - 24
    local soundOptions = { { name = "無", id = nil } }
    ...
    if rotLbl then rotLbl:SetText("啟用打斷順序") end
    ...
    savePosBtn:SetScript("OnClick", function()
        ...
        local ok, err = pcall(function()
            if not mainFrame then
                toChat("|cFF00DDDD[LOXX]|r 沒有可儲存的視窗。")
                return
            end
            if LoxxSaveFramePosition(mainFrame) then
                toChat("|cFF00DDDD[LOXX]|r |cFF44FF44已儲存位置 !|r")
                toCenter("LOXX: 已儲存位置！")
            else
                toChat("|cFF00DDDD[LOXX]|r 視窗隱藏，無法儲存。")
            end
        end)
        if not ok and err then
            toChat("|cFFFF4444[LOXX]|r 錯誤: " .. tostring(err))
        end
    end)
    ...
    SlashCmdList["LOXX"] = function(msg)
        ...
        elseif cmd == "pos" then
            ...
            if mainFrame then
                ...
                print("|cFF00DDDD[LOXX]|r 當前位置: " .. tostring(l) .. ", " .. tostring(t))
                print("  帳號儲存: " .. tostring(LOXXAccountVars and LOXXAccountVars.frameX) .. ", " .. tostring(LOXXAccountVars and LOXXAccountVars.frameY))
                print("  角色儲存: " .. tostring(db and db.frameX) .. ", " .. tostring(db and db.frameY))
            else
                print("|cFF00DDDD[LOXX]|r 主視窗尚未建立。")
            end
        ...
        elseif cmd == "logs" or cmd == "log" then
            if #loxxErrorLog == 0 then
                print("|cFF00DDDD[LOXX]|r 無錯誤紀錄。")
            else
                print("|cFF00DDDD[LOXX]|r === 最近的錯誤 (" .. #loxxErrorLog .. ") ===")
                for i = 1, math.min(20, #loxxErrorLog) do
                    print("|cFFFF4444[" .. i .. "]|r " .. loxxErrorLog[i])
                end
                if #loxxErrorLog > 20 then
                    print("  ... " .. (#loxxErrorLog - 20) .. " 更多。使用 /loxx logs clear 來清除。")
                end
            end
        elseif cmd == "logs clear" or cmd == "log clear" then
            loxxErrorLog = {}
            if LOXXSavedVars then LOXXSavedVars.loxxErrorLog = {} end
            print("|cFF00DDDD[LOXX]|r 錯誤紀錄已清除。")
        elseif cmd == "help" then
            print("|cFF00DDDD[LOXX]|r /loxx (選項) | show | hide | lock | unlock | test | spy | pos | debug | stats | logs | logs clear")
        else
            -- Default: open config
            CreateConfigPanel()
        end
...
ShowStatsWindow = function()
    ...
    hdrTitle:SetPoint("TOP", 0, -34)
    hdrTitle:SetJustifyH("CENTER")
    hdrTitle:SetText("|cFFFFD100打斷狀態統計|r")
    ...
    -- Current run
    if loxxCurrentRun and next(loxxCurrentRun.players) then
        AddLine("|cFF00DDDD當前次數|r", 0, "GameFontNormal") ; y = y - 18
        ...
        for _, row in ipairs(SortedPlayers(loxxCurrentRun.players)) do
            AddLine("  " .. row.name .. " — " .. row.kicks .. " 打斷" .. (row.kicks ~= 1 and "s" or ""), 8)
            y = y - 14
        end
        y = y - 8
    end
    ...
    if #runs > 0 then
        AddSep()
        AddLine("|cFFFFD100歷史紀錄|r", 0, "GameFontNormal") ; y = y - 20
...
ShowRotationPanel = function()
    if not rotationPanel then
        ...
        hdrTitle:SetPoint("TOP", 0, -34)
        hdrTitle:SetJustifyH("CENTER")
        hdrTitle:SetText("|cFFFFD100打斷順序|r")

        -- Permanent bottom buttons
        local resetBtn = CreateFrame("Button", nil, rotationPanel, "UIPanelButtonTemplate")
        resetBtn:SetSize(80, 22) ; resetBtn:SetPoint("BOTTOMLEFT", 8, 8) ; resetBtn:SetText("重置")
        ...
        syncBtn:SetSize(100, 22) ; syncBtn:SetPoint("BOTTOMRIGHT", -8, 8) ; syncBtn:SetText("同步到隊伍")
        syncBtn:SetScript("OnClick", function()
            ...
            BroadcastRotation()
            print("|cFF00DDDD[LOXX]|r 打斷順序已同步到隊伍。")
        end)
    end
    BuildRotationPanel()
...
local function RegisterBlizzardOptions()
    ...
    title:SetText("|cFF00DDDD打斷追蹤|r")

    local hint = panel:CreateFontString(nil, "OVERLAY", "GameFontDisableSmall")
    hint:SetPoint("TOPLEFT", LX, -40)
    hint:SetText("所有設定可透過指令 /loxx")
    ...
    BH("顯示", LX, yL) ; yL = yL - 22
    BC("顯示標題條",   "showTitle",  LX, yL) ; yL = yL - 24
    BC("鎖定位置",    "locked",     LX, yL) ; yL = yL - 24
    BC("顯示就緒文字",  "showReady",  LX, yL) ; yL = yL - 24
    BC("滑鼠指向顯示提示", "showTooltip",LX, yL) ; yL = yL - 24
    BC("戰鬥外隱藏","hideOutOfCombat", LX, yL) ; yL = yL - 36

    BH("字體大小", LX, yL) ; yL = yL - 22
    BS("LOXX_Blizz_NameFont", panel, LSL, yL, 2, 32, 1,
        math.max(2, db.nameFontSize or 12),
        function(v) return "名字: " .. v end,
...
local function Initialize()
    ...
    -- print("|cFF00DDDD[打斷追蹤]|r v" .. LOXX_VERSION .. " | /loxx")
end
