...
function R.StripResponseLanguageDirective(text)
    local norm = R.Normalize(text)
    if norm == "" then return norm end
    local data = MSUF.AssistantDialogLocaleData
    local directives = data and data.explicitLanguage
    if type(directives) ~= "table" then
        -- The isolated Lua audits load Router without the later DialogLocale
        -- data file. Keep only the structural fallback here; the game runtime
        -- consumes the complete phrase set from AssistantDialogLocaleData.
        directives = {
            de = { "answer in german", "in german", "antworte auf deutsch", "auf deutsch" },
            en = { "answer in english", "in english", "antworte auf englisch", "auf englisch" },
        }
    end

    local candidate = norm:gsub("^please%s+", "")
    local bestPrefix, bestSuffix = 0, 0
    for _, language in ipairs({ "de", "en" }) do
        for _, phrase in ipairs(directives[language] or {}) do
            phrase = R.Normalize(phrase)
            if phrase ~= "" then
                if candidate == phrase then return "" end
                if candidate:sub(1, #phrase + 1) == phrase .. " " and #phrase > bestPrefix then bestPrefix = #phrase end
                local suffixDirective = phrase:match("^answer%s+") or phrase:match("^reply%s+")
                    or phrase:match("^please%s+") or phrase:match("^speak%s+")
                    or phrase:match("^antworte%s+") or phrase:match("^antwort%s+")
                    or phrase:match("^bitte%s+") or phrase:match("^sprich%s+")
                if suffixDirective and candidate:sub(-#phrase - 1) == " " .. phrase and #phrase > bestSuffix then
                    bestSuffix = #phrase
                end
            end
        end
    end
    if bestPrefix > 0 then candidate = R.Trim(candidate:sub(bestPrefix + 1)) end
    if bestSuffix > 0 then candidate = R.Trim(candidate:sub(1, #candidate - bestSuffix - 1)) end
    return candidate
end
...
R.UNMISTAKABLE_BOOLEAN_COMMANDS = {
    enable = true, disable = true, show = true, hide = true,
    aktivieren = true, deaktivieren = true, anzeigen = true, verstecken = true,
    einblenden = true, ausblenden = true,
}
R.UNMISTAKABLE_RELATIVE_COMMANDS = {
    increase = true, decrease = true, raise = true, lower = true, move = true, nudge = true, shift = true,
    erhoehe = true, senke = true, verschiebe = true,
}
R.UNMISTAKABLE_VALUE_COMMANDS = {
    set = true, change = true, make = true, setze = true, stelle = true, aendere = true,
}

function R.HasUnmistakableMutationRequest(text)
    local norm = R.Normalize(text)
    norm = norm:gsub("^please%s+", ""):gsub("^bitte%s+", "")
    norm = norm:gsub("^can%s+you%s+", ""):gsub("^could%s+you%s+", ""):gsub("^would%s+you%s+", "")
    norm = norm:gsub("^kannst%s+du%s+", ""):gsub("^koenntest%s+du%s+", "")
    if norm == "" then return false end
    if norm:match("^show%s+me%s+") or norm:match("^zeige%s+mir%s+")
        or R.LooksLikeKnowledgeQuestionPrefix(norm)
        or R.ContainsAny(norm, { "where", "which setting", "which option", "current value", "explain", "wo", "welche einstellung", "erklaere" })
    then
        return false
    end

    if norm:match("^turn%s+on%s+") or norm:match("^turn%s+off%s+") then return true end
    local firstWord = norm:match("^(%S+)")
    if R.UNMISTAKABLE_BOOLEAN_COMMANDS[firstWord] and norm:find(" ", 1, true) then return true end
    if R.UNMISTAKABLE_RELATIVE_COMMANDS[firstWord] then
        return norm:match("%d") ~= nil or R.ContainsAny(norm, {
            "left", "right", "up", "down", "more", "less", "links", "rechts", "hoch", "runter", "mehr", "weniger",
        })
    end
    if not R.UNMISTAKABLE_VALUE_COMMANDS[firstWord] then return false end
    return norm:match("%d") ~= nil
        or norm:find(" to ", 1, true) ~= nil
        or norm:find(" auf ", 1, true) ~= nil
        or norm:find(" = ", 1, true) ~= nil
        or R.ContainsAny(norm, R.COLOR_TERMS)
...
function R.ReviewedSemanticMutationKind(text)
    local norm = R.Normalize(text)
    ...
    local reviewedAnchorValues = { left = true, center = true, middle = true, right = true }
    if reviewedUnitScopes[unitScope] and reviewedAnchorValues[anchorValue] then
        return "unit-name-anchor"
    end

    local groupScope, spacingValue = norm:match(
        "^set%s+(.+)%s+aura%s+spacing%s+to%s+([%+%-]?%d+%.?%d*)$")
    local reviewedGroupScopes = {
        party = true, raid = true, ["mythic raid"] = true, mythicraid = true,
    }
    if reviewedGroupScopes[groupScope] and tonumber(spacingValue) ~= nil then
        return "group-aura-spacing"
    end
    return nil
end

function R.IsSubjectiveSafePlanningRequest(text)
    local norm = R.StripResponseLanguageDirective(text)
    if norm == "" then return false end
    -- Submit's low-latency mutation lane runs before RouteInput. Keep exact
    ...
    return broadImportantAura
        or R.ContainsAny(norm, planningTerms.vagueTarget or {})
        or (R.ContainsAny(norm, planningTerms.subjectiveAura or {})
...
function A.RouterIsFailClosedReadOnlyRequest(text)
    ...
    if type(R.IncompleteMovementSubject) == "function" and R.IncompleteMovementSubject(norm) then return true end
    ...
    -- The parser owns these complete, reviewed semantic shapes even though
    -- the generic open-ended search sees multiple related controls. Their
    -- closed scope/value grammar prevents this exemption from authorizing a
    -- broader or value-less write.
    if R.ReviewedSemanticMutationKind(norm) then return false end
    ...
    if R.LooksLikeKnowledgeQuestionPrefix(norm)
        ...
        or (type(R.IsExplicitReadOnlyDiagnosticCommand) == "function" and R.IsExplicitReadOnlyDiagnosticCommand(norm))
    then
        return true
    end
    return R.ContainsAny(norm, {
        "missing", "hidden", "not showing", "not visible", "broken", "disabled", "does not work", "doesn't work",
...
function R.LooksLikeKnowledgeFirstRequest(text)    local norm = R.Normalize(text)
    if norm == "" then return false end
    if A.Knowledge and type(A.Knowledge.LooksLikeComplementaryAddonQuestion) == "function"
        and A.Knowledge.LooksLikeComplementaryAddonQuestion(norm)
    then
        return true
    end
...
-- Exact Assistant-meta questions must be handled before registry setting
-- discovery. Words such as "limit" and "cap" are also valid setting terms;
-- without this narrow priority lane, "what are your limits" can be answered
-- with unrelated aura-cap sliders instead of the Assistant's real boundaries.
R.ASSISTANT_LIMIT_QUESTIONS = R.ASSISTANT_LIMIT_QUESTIONS or {
    ["what are your limits"] = true,
    ["what are your limitations"] = true,
    ["assistant limits"] = true,
    ["assistant limitations"] = true,
    ["what cant you do"] = true,
    ["what can't you do"] = true,
    ["what can you not do"] = true,
    ["what do you not do"] = true,
    ["can you do everything"] = true,
    ["are you online"] = true,
    ["do you use internet"] = true,
    ["do you browse"] = true,
    ["was kannst du nicht"] = true,
    ["was kannst du nicht machen"] = true,
    ["wo sind deine grenzen"] = true,
    ["was sind deine grenzen"] = true,
    ["bist du online"] = true,
    ["hast du internet"] = true,
}

function R.TryEarlyAssistantMetaConversationReply(text)
    local norm = R.Normalize(text)
    if R.ASSISTANT_LIMIT_QUESTIONS[norm] then return A.RouterLimitsReply() end
    -- This common German imperative is a capability/help request. Keep it in
    -- the early meta lane so an open Party/Raid page cannot reinterpret
    -- "zeige" as permission to toggle the page's first visible control.
    if norm == "zeige mir befehle" then return R.AssistantCapabilityReply() end
    return nil
end
...
function A.TryImmediateConversationReply(text)
    ...
    then
        -- German setting-location forms have a dedicated compact route. Do
        -- not spend time testing every small-talk phrase first.
        return nil
    end
    local mayBePageHelp = norm:find("page", 1, true) or R.HasNormalizedPhrase(norm, "here")
    local pageHelp = mayBePageHelp and R.TryPageHelpShortcut and R.TryPageHelpShortcut(text, A.HandleCommandInput)
    ...
    then
        ...
        if visualHelp then
            ...
            if R.LooksLikeRegistrySettingLocationQuestion(norm)
                and R.CompactRegistrySettingSearchEntries
                and A.RouterTryRegistrySettingLocationShortcut
            then
...
R.NATURAL_PROBLEM_TERMS = {    "gone", "missing", "failed", "failing", "fails", "failure", "error", "errors", "stuck", "broken",
    "filtered out", "filtered", "blacklisted", "blocked", "disappeared", "vanished", "lost", "not shown", "not showing",
    "not displayed", "not appearing", "does not show", "doesn't show", "cannot see", "can't see", "cant see", "hidden",
    "fix", "repair",
    "weg", "fehlt", "fehlen", "fehlende", "fehlender", "fehlendes", "fehlenden", "fehlgeschlagen", "verschwunden", "nicht angezeigt", "wird nicht angezeigt",
...
function R.TryNaturalProblemShortcut(text, coreHandler)    local norm = R.Normalize(text)
    ...
    if type(coreHandler) == "function" and R.ContainsAny(norm, R.NATURAL_PROFILE_PROBLEM_TERMS) then
        local result = coreHandler("diagnose profile status")
        if result and not (type(result) == "table" and result.kind == "unknown") then return R.AsReadOnlyProblemDiagnostic(result) end
    end
...
A.RouterTryEditModeProblemShortcut = function(text, coreHandler)
    ...
    if R.ContainsAny(norm, terms.anchorPicker) and type(coreHandler) == "function" then
        ...
        if result and not (type(result) == "table" and result.kind == "unknown") then return R.AsNavigationResult(result) end
    end
...
A.RouterProfileProblemTerms = A.RouterProfileProblemTerms or {
    profile = {
        "profile", "profiles", "profile string", "import string", "export string",
        "profil", "profilstring", "profil string", "importzeichenfolge", "exportzeichenfolge",
    },
    import = {
    ...
    copy = {
        "copy profile", "copy a profile", "profile copy", "copy my profile",
        "copy current profile", "copy my current profile", "duplicate profile",
        "backup current profile", "profil kopieren", "profil duplizieren",
        "kopiere profil", "kopiere mein profil", "kopiere ich mein profil",
        "kopiere aktuelles profil", "dupliziere ich mein profil",
    },
    reset = {
...
A.RouterTryProfileProblemShortcut = function(text, coreHandler)
    ...
    -- A procedural "how to copy profile" contains the token " to ", but it
    -- does not name a destination. Keep capability/procedural questions on the
    -- explanatory route even when they mention a hypothetical destination;
    -- only imperative copy commands may fall through to the mutating flow.
    local asksCopyHelp = asksProfileInfo
        or norm:match("^how%s+to%s+") ~= nil
        or norm:match("^how%s+do%s+i%s+") ~= nil
        or norm:match("^how%s+can%s+i%s+") ~= nil
        or norm:match("^could%s+i%s+") ~= nil
        or norm:match("^is%s+there%s+a%s+way%s+to%s+") ~= nil
        or norm:match("^wie%s+") ~= nil
        or norm:match("^kann%s+ich%s+") ~= nil
        or norm:match("^koennte%s+ich%s+") ~= nil
        or norm:match("^wo%s+") ~= nil
    local namesCopyDestination = R.ContainsAny(norm, {
        " to ", " as ", " called ", " named ",
        " nach ", " als ", " namens ",
    })
    if R.ContainsAny(norm, terms.copy) and (asksCopyHelp or not namesCopyDestination) then
        return A.RouterProfileReply(
            "Profile copy help",
    ...
    if R.HasNaturalProblemTerm(norm) and type(coreHandler) == "function" then
        -- The broad problem phrase is deliberately non-mutating. Use the
        -- explicit diagnostic command so the reply can expose a safe repair
        -- choice instead of feeding the vague phrase back into the parser.
        local result = coreHandler("diagnose profile status")
        if result and not (type(result) == "table" and result.kind == "unknown") then return R.AsReadOnlyProblemDiagnostic(result) end
    end
...
function R.AsksSettingLocation(norm)
    ...
    return norm:match("^where%s+") ~= nil
        ...
        or norm:match("^wo%s+") ~= nil
        or norm:match("^wie%s+kann%s+ich") ~= nil
        or norm:match("^welche%s+einstellung") ~= nil
        or norm:match("^welche%s+option") ~= nil
        or R.ContainsAny(norm, {
            "help me find", "help me locate", "help me to find", "help me to locate", "tell me where", "looking for", "show me", "options", "list options", "setting controls", "option controls",
            "how do i use", "how can i use", "can you help me find", "can you help me locate",
            "zeige mir", "hilf mir finden", "einstellung steuert", "option steuert",
        })
end
...
R.AURA_GROWTH_MUTATION_PHRASES = {
    ...
    "would like to change", "like to change", "need to change",
    "help me change", "help me to change", "help me adjust", "help me to adjust",
    "help me configure", "help me to configure",
    "grow differently", "grows differently", "switch the growth", "switch growth",
}

function R.HasAuraGrowthIntent(norm)
    norm = R.Normalize(norm)
    return R.ContainsAny(norm, { "growth direction", "grow direction", "growth", "grow differently", "grows differently" })
...
function R.HasAuraGrowthMutationIntent(norm)
    norm = R.Normalize(norm)
    return R.StartsWithMutationCommand(norm)
        or R.HasUnmistakableMutationRequest(norm)
        or R.ContainsAny(norm, R.AURA_GROWTH_MUTATION_PHRASES)
end
...
function R.AuraGrowthNamedScopes(norm)
    ...
    local function Add(key, label)
        if not seen[key] then
            seen[key] = true
            scopes[#scopes + 1] = { key = key, label = label }
        end
    end

    local direct = norm:gsub("target of target", " "):gsub("targettarget", " ")
        :gsub("focus target", " "):gsub("focustarget", " ")
    if R.ContainsAny(direct, { "player", "player frame", "my frame" }) then Add("player", "Player") end
    ...
    if R.ContainsAny(groups, { "mythic raid", "mythic raid frame", "mythic raid frames", "mythicraid" }) then
        ...
        groups = groups:gsub("mythic%s+raid%s+frames", " ")
            :gsub("mythic%s+raid%s+frame", " ")
            :gsub("mythic%s+raid", " ")
            :gsub("mythicraid", " ")
    end
    if R.ContainsAny(groups, { "raid", "raid frame", "raid frames" }) then Add("raid", "Raid") end
    return scopes
end

function R.ScopedAuraGrowthSetting(norm)
    norm = R.Normalize(norm)
    local lane, laneLabel = R.AuraLaneFromText(norm)
    local scope, scopeLabel, groupScope = R.AuraScopeForDetailText(norm)
    if not lane or not scope then return nil end
    local settingKey = groupScope
        and ("gf_" .. tostring(scope) .. ".auras." .. tostring(lane) .. ".growth")
        or ("auras3." .. tostring(scope) .. "." .. tostring(lane) .. ".growth")
    local registry = A.Registry
    local setting = registry and type(registry.GetSetting) == "function"
        and registry:GetSetting(settingKey) or nil
    return setting, settingKey, lane, laneLabel, scope, scopeLabel, groupScope
end

function R.AuraGrowthMutationAmbiguity(text)
    local norm = R.Normalize(text)
...
function R.TryScopedAuraGrowthClarification(text)
    local kind, scopes = R.AuraGrowthMutationAmbiguity(text)
    if not kind then return nil end
    if type(A.CancelPendingMutationState) == "function" then A.CancelPendingMutationState() end
    local scopeLabel = scopes and scopes[1] and scopes[1].label or "that frame"
    local body
    if kind == "scopes" then
        local labels = {}
        for i = 1, #(scopes or {}) do labels[#labels + 1] = tostring(scopes[i].label) end
        body = "You named more than one frame scope (" .. table.concat(labels, " and ")
            .. "), so I kept every Aura growth setting unchanged. Send one complete frame-and-lane command at a time."
    elseif kind == "lanes" then
        body = "You named both " .. scopeLabel .. " Buff Growth and " .. scopeLabel
            .. " Debuff Growth, so I kept both unchanged. Choose one lane, or send the two complete changes as separate messages."
    else
        body = "Do you mean " .. scopeLabel .. " Buff Growth or " .. scopeLabel
            .. " Debuff Growth? I kept both unchanged because the Aura lane was not specified."
    end
    return {
        text = body .. "\nExamples: change " .. scopeLabel:lower()
            .. " buff growth direction; set " .. scopeLabel:lower() .. " debuff growth to left.",
        status = "ambiguous",
        result = "ambiguous",
        summary = "Clarifies a compound Aura growth request before changing any setting.",
    }
end

function R.TryScopedAuraGrowthReadOnly(text, coreHandler)
    local norm = R.Normalize(text)
    if not R.HasAuraGrowthIntent(norm) or R.HasAuraGrowthMutationIntent(norm) then return nil end
    local scopes = R.AuraGrowthNamedScopes(norm)
    if #scopes ~= 1 or R.AuraHasBothLanes(norm) then return nil end
    local setting, settingKey = R.ScopedAuraGrowthSetting(norm)
    if not setting then return nil end
    local locationQuestion = R.LooksLikeRegistrySettingLocationQuestion(norm)
    local currentQuestion = R.LooksLikeRegistrySettingCurrentValueQuestion(norm)
    local explainQuestion = R.LooksLikeRegistrySettingExplainQuestion(norm)
        or norm:match("^why%s+") or norm:match("^what%s+is%s+")
        or norm:match("^what%s+does%s+") or norm:match("^explain%s+")
    if not locationQuestion and not currentQuestion and not explainQuestion then return nil end

    if type(A.CancelPendingMutationState) == "function" then A.CancelPendingMutationState() end
    local item = R.RegistrySettingItemForKey and R.RegistrySettingItemForKey(settingKey) or nil
    if not item then return nil end
    local entries = { { item = item, score = 40000, rawScore = 40000 } }
    if locationQuestion and A.RouterTryRegistrySettingLocationShortcut then
        return A.RouterTryRegistrySettingLocationShortcut(text, coreHandler, entries)
    end
    if currentQuestion and A.RouterTryRegistrySettingCurrentValueShortcut then
        return A.RouterTryRegistrySettingCurrentValueShortcut(text, coreHandler, entries)
    end
    if A.RouterTryRegistrySettingExplainShortcut then
        local explainText = R.LooksLikeRegistrySettingExplainQuestion(norm)
            and text or ("explain " .. tostring(setting.label or settingKey))
        return A.RouterTryRegistrySettingExplainShortcut(explainText, coreHandler, entries)
    end
    return nil
end
...
function R.TryScopedAuraGrowthChoice(text, coreHandler)
    ...
    if not R.HasAuraGrowthIntent(norm) or not R.HasAuraGrowthMutationIntent(norm) then return nil end
    if R.AuraGrowthMutationAmbiguity(norm) then return nil end
    ...
    -- Use the exact registered control label instead of adding the natural
    -- word "direction" to it.  The latter required a warm fuzzy-search index
    -- because the real labels are "Player Buff Growth", "Raid Debuff Growth",
    -- and so on; on a cold first request it could fall back to the Aura Style
    -- page even though scope and lane were already known exactly.
    local settingKey = groupScope
        and ("gf_" .. tostring(scope) .. ".auras." .. tostring(lane) .. ".growth")
        or ("auras3." .. tostring(scope) .. "." .. tostring(lane) .. ".growth")
    local registry = A.Registry
    local setting = registry and type(registry.GetSetting) == "function"
        and registry:GetSetting(settingKey) or nil
    local canonical = "change " .. tostring(setting and setting.label
        or (tostring(scope) .. " " .. tostring(lane) .. " growth"))
...
A.RouterTryAuraSettingShortcut = function(norm, coreHandler)
    ...
    if asksLocation then
        ...
        if displayOnly and type(coreHandler) == "function" then
            local opened = coreHandler("open " .. page:lower())
            if opened and not A.RouterIsUnknownResult(opened) then
                navigationResult = R.AsNavigationResult(opened)
                if (navigationResult.status or navigationResult.result) == "navigated" then
                    body = "Done. Opened " .. page .. ".\n" .. body
                end
            end
        end
        local didNavigate = navigationResult and (navigationResult.status or navigationResult.result) == "navigated"
...
function R.AuraFilterOverviewReply(norm)
    local lines = {}
    local examples = "hide spell 12345; what active target debuff filters do I have; explain party buff filters; what is a good filter for raid."
    ...
    lines[#lines + 1] = "Examples: " .. examples
...
function R.TextMovementIntent(text, subject)
    ...
    local firstWord = actionable:match("^(%S+)")
    ...
    if fixedAnchorRequested then
        if R.ContainsAny(actionable, { "top left", "upper left" }) then fixedAnchorValue = "TOPLEFT"
    ...
    local direction = not conflictingDirections and (horizontalDirection or verticalDirection) or nil

    local hasMovementVerb = R.ContainsAny(actionable, {
        "move", "nudge", "shift", "position", "reposition", "offset",
    })
    if not hasMovementVerb and not direction then return nil end

    local readOnly = norm:match("^how%s+do%s+i%s+") ~= nil
        or norm:match("^how%s+can%s+i%s+") ~= nil
        or norm:match("^where%s+") ~= nil
        or norm:match("^can%s+i%s+") ~= nil
        or norm:match("^show%s+me%s+where%s+") ~= nil
        or norm:match("^help%s+me%s+find%s+") ~= nil
        or norm:match("^help%s+me%s+locate%s+") ~= nil
        or norm:find("help me find", 1, true) ~= nil
        or norm:find("help me locate", 1, true) ~= nil

    local axis
    if R.ContainsAny(actionable, { "x offset", "horizontal" }) or direction == "left" or direction == "right" then
        axis = "x"
    elseif R.ContainsAny(actionable, { "y offset", "vertical" }) or direction == "up" or direction == "down" then
        axis = "y"
    end
    ...
    return {
        ...
        unitLabel = unitLabel,
        textKind = textKind,
        label = tostring(unitLabel) .. " " .. tostring(settingKindLabel),
        noun = noun,
        xKey = tostring(settingScope) .. "." .. tostring(attrs.x),
        yKey = tostring(settingScope) .. "." .. tostring(attrs.y),
        anchorKey = (isGroup and attrs.anchorGroup or attrs.anchorUnit)
            and (tostring(settingScope) .. "." .. tostring(isGroup and attrs.anchorGroup or attrs.anchorUnit)) or nil,
        fixedAnchorValue = fixedAnchorValue,
        fixedAnchorRequested = fixedAnchorRequested and true or false,
        direction = direction,
...
function R.IsAbsoluteTextOffsetAssignment(text, intent)
    if type(intent) ~= "table" or intent.readOnly or intent.direction
        or intent.conflictingDirections or intent.numberCount ~= 1
        or (intent.axis ~= "x" and intent.axis ~= "y")
    then
        return false
    end
    local parser = A.Parser or {}
    local actionable = type(parser.ActionableText) == "function" and parser.ActionableText(text) or text
    actionable = R.Normalize(actionable)
    if actionable == "" or actionable:find(" by ", 1, true) then return false end
    if not (actionable:match("^set%s+") or actionable:match("^change%s+")
        or actionable:match("^adjust%s+") or actionable:match("^configure%s+")
        or actionable:match("^make%s+"))
    then
        return false
    end
    return actionable:match("%f[%a]" .. tostring(intent.axis) .. "%s+offset%f[%A]") ~= nil
end

function R.TextMovementFixedAnchorResult(intent)
    if type(intent) ~= "table" or not intent.fixedAnchorValue then return nil end
    local item = intent.anchorKey and R.RegistrySettingItemForKey and R.RegistrySettingItemForKey(intent.anchorKey) or nil
    local setting = item and item.setting
    local validValue
    if setting and setting.type == "enum" then
        for i = 1, #(setting.values or {}) do
            if tostring(setting.values[i]):upper() == tostring(intent.fixedAnchorValue):upper() then
                validValue = setting.values[i]
                break
            end
        end
    end
    if validValue and intent.readOnly then
        return {
            text = tostring(item.label or setting.label) .. " is the exact fixed-position control for " .. tostring(intent.label)
                .. ". I kept it unchanged. Ask me to open it or say 'set " .. tostring(intent.noun)
                .. " anchor to " .. tostring(validValue):lower() .. "'.",
            status = "info",
            summary = "Explains the exact fixed text-anchor control.",
            searchResults = R.RegistryLocationResultFollowups({ { item = item, score = 10000, rawScore = 10000 } }, 1),
        }
    end
    if validValue and type(A.ExecutePlan) == "function" then
        return A.ExecutePlan({
            kind = "changes",
            changes = { { setting = setting, value = validValue } },
            label = "Position " .. tostring(intent.label) .. " " .. tostring(validValue):lower(),
            summary = "Changes the exact fixed text-anchor option.",
...
function R.TextMovementFollowupResults(intent)
    if type(intent) ~= "table" then return nil end
    local entries = {}
    local keys = intent.axis == "x" and { intent.xKey }
        or intent.axis == "y" and { intent.yKey }
        or { intent.xKey, intent.yKey }
    ...
    return R.RegistryLocationResultFollowups and R.RegistryLocationResultFollowups(entries, #entries) or nil
end

function R.TextMovementGuidance(intent, readOnly)
    if type(intent) ~= "table" then return nil end
    local xItem = R.RegistrySettingItemForKey and R.RegistrySettingItemForKey(intent.xKey) or nil
    local yItem = R.RegistrySettingItemForKey and R.RegistrySettingItemForKey(intent.yKey) or nil
    if not xItem and not yItem then return nil end
    if not readOnly and type(A.StartPendingFlow) == "function" then
        A.StartPendingFlow("settingMovement", {
            label = intent.label,
            noun = intent.noun,
            xKey = intent.xKey,
            yKey = intent.yKey,
            page = (xItem and xItem.page) or (yItem and yItem.page),
            step = 10,
        })
    end
    local lines = {
        (readOnly and "Absolutely — here is where to move " or "Absolutely — I found ") .. tostring(intent.label) .. ".",
    }
    ...
    return {
        text = table.concat(lines, "\n"),
        status = readOnly and "info" or "ambiguous",
        result = readOnly and "info" or "ambiguous",
        summary = readOnly and "Explains the exact text position controls." or "Guides an exact text position adjustment.",
        searchResults = R.TextMovementFollowupResults(intent),
    }
end

function R.TryTextMovementConversation(text, coreHandler)
    local intent = R.TextMovementIntent(text)
    if not intent then return nil end
    -- "set ... X/Y offset to N" is an absolute registry assignment, not a
    -- direction-less relative movement request. Let the deterministic parser
    -- own it even when the Knowledge index is cold and an alias lookup cannot
    -- yet resolve labels such as "name text" to "Name Y Offset".
    if R.IsAbsoluteTextOffsetAssignment(text, intent) and type(coreHandler) == "function" then
    ...
    if intent.horizontalDirection and intent.verticalDirection then
        ...
        if xSetting and ySetting and type(A.ExecutePlan) == "function" then
            ...
            return A.ExecutePlan({
                kind = "changes",
                changes = {
                    { setting = xSetting, relativeDelta = xDelta, direction = intent.horizontalDirection },
                    { setting = ySetting, relativeDelta = yDelta, direction = intent.verticalDirection },
                },
                label = "Move " .. tostring(intent.label) .. " "
    ...
    if intent.amount then canonical = canonical .. " " .. tostring(intent.amount) end
...
function R.IncompleteMovementSubject(text)
    ...
    local actionable = type(parser.ActionableText) == "function" and parser.ActionableText(text) or text
    actionable = R.Normalize(actionable)
    local verb, subject = actionable:match("^(%S+)%s+(.+)$")
    if not verb or not subject or not ({
        move = true, nudge = true, shift = true, position = true, reposition = true,
    })[verb] then return nil end
    if actionable:match("%d") or R.ContainsAny(actionable, {
        "left", "right", "up", "down", "above", "below", "over", "under", "higher", "lower",
        "x offset", "y offset", "horizontal", "vertical",
    }) then
        return nil
    end
    subject = R.Trim(subject:gsub("^the%s+", ""))
    if subject == "" then return nil end
    if R.ContainsAny(subject, { "frame", "frames", "unit frame", "group frame" })
        and not R.ContainsAny(subject, {
            "text", "name", "health", "hp", "power", "mana", "portrait", "buff", "debuff", "aura",
            "icon", "marker", "indicator", "cast bar", "castbar", "resource", "totem",
        })
    then
        return nil
    end
...
function R.IncompleteMovementEntries(subject)
    subject = R.Normalize(subject)
    if subject == "" then return nil end
    ...
    if type(cached) == "table" and cached.subject == subject and cached.count == #settings then
        return cached.entries ~= false and cached.entries or nil
    end

    -- The old implementation performed four full 4.5k-setting searches (and
    -- repeated all four again on a miss later in the same route). Do one
    -- bounded registry pass, rank the four geometry families, and retain only
    -- the last scalar subject result. This is submit-time/LoD work only.
    ...
    for i = 1, #settings do
        ...
        local padded = " " .. candidateText .. " "
        local ownsSubject = candidateText ~= ""
        for j = 1, #subjectWords do
            if not padded:find(" " .. subjectWords[j] .. " ", 1, true) then
                ownsSubject = false
                break
            end
        end
        if ownsSubject then
            local rank
            for family, value in pairs(familyRank) do
                if padded:find(" " .. family .. " ", 1, true) and (not rank or value > rank) then rank = value end
            end
            if rank then
    ...
    while #entries > 8 do entries[#entries] = nil end
    local result = #entries > 0 and entries or false
    R._incompleteMovementEntryCache = { subject = subject, count = #settings, entries = result }
    return result ~= false and result or nil
end

function R.TryIncompleteMovementConversation(text)
    local subject = R.IncompleteMovementSubject(text)
    if not subject then return nil end
    local entries = R.IncompleteMovementEntries(subject)
    ...
    return {
        ...
        status = "ambiguous",
        result = "ambiguous",
        summary = "Clarifies an incomplete movement request with exact MSUF controls.",
        searchResults = R.RegistryLocationResultFollowups(entries, visible),
    }
end

function R.ActionableMutationText(text)
    local parser = A.Parser or {}
    if type(parser.ActionableText) ~= "function" then return nil end
    local norm = R.Normalize(text)
    local actionable = R.Normalize(parser.ActionableText(text))
    if actionable == "" or actionable == norm then return nil end
    if norm:match("^how%s+do%s+i%s+") or norm:match("^how%s+can%s+i%s+")
        or norm:match("^where%s+") or norm:match("^can%s+i%s+")
        or norm:match("^show%s+me%s+where%s+")
        or norm:match("^help%s+me%s+find%s+") or norm:match("^help%s+me%s+locate%s+")
    then
        return nil
    end
    return actionable
...
function R.TryConversationalMutation(text, coreHandler)
    ...
    if norm:find("target s target", 1, true)
        ...
        or (norm:find("target target", 1, true) and norm:find("on target frame", 1, true))
    then
        return nil
    end
    ...
    local actionable = R.ActionableMutationText(text)
    if not actionable then return nil end
    local openEnded = R.TryOpenEndedSettingIdea and R.TryOpenEndedSettingIdea(actionable, coreHandler)
    if openEnded then return openEnded end
    if not R.HasUnmistakableMutationRequest(actionable) then return nil end
    local result = type(coreHandler) == "function" and coreHandler(actionable) or nil
    if result and not A.RouterIsUnknownResult(result) then return result end
    return nil
end
...
A.RouterTryTextPowerSettingShortcut = function(norm, coreHandler)
    ...
    local textMovement = R.TextMovementIntent and R.TextMovementIntent(norm) or nil
    if textMovement then
        textMovement.readOnly = true
        return R.TextMovementGuidance(textMovement, true)
    end
...
A.RouterTryClassResourceSettingShortcut = function(norm, coreHandler)
    ...
    if isCapabilityQuestion then
        ...
        if type(coreHandler) == "function" then
            local opened = coreHandler("open class resources")
            if opened and not A.RouterIsUnknownResult(opened) then
                opened = R.AsNavigationResult(opened)
                didNavigate = (opened.status or opened.result) == "navigated"
            end
        end
        body = (didNavigate and "Done. Opened Class Resources.\n" or "")
...
function R.TryVisibilityDiagnosticShortcut(text, coreHandler)    if type(coreHandler) ~= "function" then return nil end
    ...
    elseif R.ContainsAny(norm, R.VISIBILITY_AURA_TERMS) then
        ...
        if scope then
            -- Use the explicit diagnostic action lane. The natural phrase
            -- "why are ... hidden" can also look like a boolean-off request
            -- to low-level alias matching; wrapping that result as `info`
            -- would hide a real write behind a read-only status.
            query = "diagnose " .. scope .. " " .. R.VisibilityAuraLaneForText(norm)
        end
    ...
    if not query then
        if group then
            -- Keep problem reports on the explicit read-only diagnostic action;
            -- the natural "... frames hidden" phrase overlaps the frame's
            -- boolean visibility alias and can otherwise perform an off-write.
            query = "diagnose " .. group .. " frames"
        elseif unit then
    ...
    if result and not (type(result) == "table" and result.kind == "unknown") then
        ...
        return R.AsReadOnlyResult(result)
    end
    return nil
end
...
function A.RouterClearPendingChoicesForRoute()
    ...
    -- The runtime keeps a rehydratable mirror for natural candidate replies.
    -- Clearing only pendingChoices lets HandlePending resurrect the old fixes
    -- after the Router already recognized a new problem/topic.
    if A.RuntimePrivate and type(A.RuntimePrivate.ClearPendingCandidates) == "function" then
        A.RuntimePrivate.ClearPendingCandidates()
    end
    local ctx = A.GetContext and A.GetContext()
    if type(ctx) == "table" then
    ...
    if R.ContainsAny(norm, terms.assistant) and R.ContainsAny(norm, terms.matching) then
        if R.ContainsAny(norm, { "german", "deutsch" }) then
            return A.RouterAssistantUsabilityReply(
                ...
                "The Assistant supports deterministic English and German response presentation. Ask 'answer in German ...' or 'answer in English ...' to switch the conversation language; canonical MSUF setting, page, and button names stay unchanged. Menu Language separately controls MSUF menu labels.",
                "answer in German what are your limits; answer in English what are your limits; set menu language to German.",
...
A.RouterTrySafePlanningShortcut = function(text, coreHandler)
    ...
    local broadImportantAura = norm == "show important debuffs"
        or norm == "show important buffs"
    if (broadImportantAura or R.ContainsAny(norm, terms.subjectiveAura))
...
function R.TryPageHelpShortcut(text, coreHandler)    if type(coreHandler) ~= "function" then return nil end
    if not R.IsCurrentPageHelpRequest(text) then return nil end
    local result = coreHandler(text)
    if result and not A.RouterIsUnknownResult(result) then return R.AsReadOnlyResult(result) end
    return nil
end
...
function R.RegistryLocationSubject(norm)
    ...
    local prefixes = {
        ...
        "would you help me to find ", "would you help me to locate ",
        "can you help me find ", "can you help me locate ",
        "could you help me find ", "could you help me locate ",
        "would you help me find ", "would you help me locate ",
        "help me to find ", "help me to locate ",
        ...
        "how do i change ", "how can i change ", "how do i set ", "how can i set ",
        "how do i configure ", "how can i configure ", "how do i adjust ", "how can i adjust ",
        "how do i show ", "how can i show ", "how do i hide ", "how can i hide ",
        "how do i display ", "how can i display ",
        "how do i turn on ", "how can i turn on ", "how do i turn off ", "how can i turn off ",
        "how do i enable ", "how can i enable ", "how do i disable ", "how can i disable ",
        ...
        "wie kann ich ", "wie aendere ich ", "welche einstellung steuert ", "welche option steuert ",
    }
    for i = 1, #prefixes do
        local prefix = prefixes[i]
...
function R.RegistrySettingTypeText(controlType, item)
    local setting = item and (item.setting or item) or { type = controlType }
    if type(A.DisplaySettingControl) == "function" then
...
function R.RegistryLocationLine(index, item)
    ...
    return tostring(index) .. ". " .. label .. " - " .. pageLabel
end
...
R.COMPACT_SETTING_STOP_WORDS = {
    ["the"] = true, ["a"] = true, ["an"] = true,
    ["frame"] = true, ["frames"] = true, ["setting"] = true, ["settings"] = true,
    ["option"] = true, ["options"] = true, ["control"] = true, ["controls"] = true,
}
...
function R.CompactRegistrySettingTokens(text)
    local norm = tostring(text or ""):lower():gsub("[^%w]+", " "):gsub("%s+", " ")
    norm = R.Trim(norm):gsub("cast bar", "castbar")
    local words, set = {}, {}
    for word in norm:gmatch("%S+") do
        word = R.COMPACT_SETTING_WORDS[word] or word
        if not R.COMPACT_SETTING_STOP_WORDS[word] and not set[word] then
            set[word] = true
            words[#words + 1] = word
        end
    end
...
function R.CompactRegistrySettingLabel(text)
...
function R.CompactRegistrySettingSearchEntries(text, limit)
    ...
    for i = 1, #settings do
        ...
        local all = candidateText ~= ""
        for j = 1, #queryWords do
            if not paddedCandidate:find(" " .. queryWords[j] .. " ", 1, true) then all = false; break end
        end
        if all then
            local candidateCount = 0
            for _ in candidateText:gmatch("%S+") do candidateCount = candidateCount + 1 end
            local extra = candidateCount - #queryWords
...
-- Reuse the parser's already-bounded exact-alias index for conversational
-- location questions.  This avoids a second large alias table and lets an
-- exact registered phrase win before broad page/help wording.  The index is
-- built lazily only after the LoadOnDemand Assistant receives a request.
function R.ExactRegistrySettingAliasEntries(subject, limit)
    subject = R.Normalize(subject)
    if subject == "" then return nil end
    local tokenCount = 0
    ...
    local entries, seen = {}, {}
    for i = 1, #bucket do
        local setting = bucket[i]
        local key = tostring(setting and setting.key or "")
        if key ~= "" and not seen[key] then
            seen[key] = true
            local item = R.RegistrySettingItemForKey and R.RegistrySettingItemForKey(key) or nil
            if item then entries[#entries + 1] = { score = 20000, rawScore = 20000, item = item } end
        end
        if #entries >= (tonumber(limit) or 16) then break end
    end
    if #entries == 0 then return nil end
    table.sort(entries, function(a, b)
        return tostring(a.item and a.item.label or "") < tostring(b.item and b.item.label or "")
...
function R.ExactRegistrySettingLabelEntries(subject, limit)
    subject = R.Normalize(subject)
    if subject == "" then return nil end
    local settings = R.EnsureCompleteSettingRegistry()
    if type(settings) ~= "table" then return nil end
...
R.EXPLICIT_SEARCH_CANONICAL_KEYS = {
    ...
    ["class resources player power texture"] = "player.powerBarTexture",
    ["class resources player power bar texture"] = "player.powerBarTexture",
    ["class resource player power texture"] = "player.powerBarTexture",
    ["class resource player power bar texture"] = "player.powerBarTexture",
...
function R.RegistrySettingPurpose(item)
    ...
    -- The declared control type owns the meaning. Label words only refine an
    -- explanation inside a compatible type, so a boolean Border is never
    -- described as a style and an enum Outline is never called thickness.
    if kind == "boolean" then
        if R.ContainsAny(normLabel, { "range fade" }) then return label .. " turns out-of-range frame fading on or off." end
        return label .. " turns that MSUF option on or off."
    end
    if kind == "color" then
        ...
        return label .. " sets the color used by that MSUF option."
    end
    if kind == "number" then
        ...
        if R.ContainsAny(normLabel, { "opacity", "alpha" }) then return label .. " controls the opacity amount for that MSUF option." end
        ...
        if R.ContainsAny(normLabel, { "border thickness", "outline thickness", "outline size" }) then return label .. " controls how thick the border or outline is." end
        return label .. " changes the numeric amount for that MSUF option."
    end
    if kind == "enum" then
        if R.ContainsAny(normLabel, { "hidden aura preset", "blacklist preset" }) then return label .. " chooses a preset for hidden/blacklisted aura handling." end
        if R.ContainsAny(normLabel, { "health color scheme", "health color mode" }) then return label .. " chooses how MSUF colors the health bar." end
        if R.ContainsAny(normLabel, { "outline", "border" }) then return label .. " chooses which outline or border style MSUF uses." end
        return label .. " chooses which mode or style MSUF uses for that option."
    end
...
function R.OpenEndedMutationSubject(text)
    ...
    subject = R.Trim(subject:gsub("^the%s+", ""))
    subject = subject:gsub("grow%s+direction", "growth direction")
    for _ = 1, 3 do
        subject = R.Trim(subject
            :gsub("%s+different$", "")
            :gsub("%s+differently$", "")
...
local function OpenEndedEnumContainsValue(setting, value)
    ...
    for i = 1, #setting.values do
        ...
        if type(candidate) == "table" then candidate = candidate.value or candidate.key end
        if candidate == value or tostring(candidate or ""):lower() == tostring(value or ""):lower() then return true end
    end
    return false
end
...
-- Portrait Position or Portrait Border. Free-form strings never qualify.
function R.OpenEndedSpecialistProvesUniqueEnumValue(plan, entries)
    if type(plan) ~= "table" or plan.kind ~= "changes" or type(plan.changes) ~= "table" or #plan.changes ~= 1 then return false end
    if type(entries) ~= "table" or #entries < 2 then return false end
    local change = plan.changes[1]
    local selected = change and change.setting
    local selectedKey = tostring(selected and selected.key or "")
    if selectedKey == "" or not OpenEndedEnumContainsValue(selected, change.value) then return false end

    local selectedIsCandidate, compatible = false, 0
    for i = 1, #entries do
        local setting = entries[i] and entries[i].item and entries[i].item.setting
        if setting and tostring(setting.key or "") == selectedKey then selectedIsCandidate = true end
        if OpenEndedEnumContainsValue(setting, change.value) then compatible = compatible + 1 end
    end
...
function R.OpenColorSettingPickerResult(item, summary)
    local setting = item and item.setting
    if not (setting and setting.type == "color"
        and type(A.OpenColorSettingPickerForSetting) == "function")
    then
        return nil
    end
    local opened, message = A.OpenColorSettingPickerForSetting(
        setting, item.page, item.label or setting.label)
    if opened ~= true then return nil end
    return {
        text = tostring(message or ("Opened Color Painter for " .. tostring(item.label or setting.label or "that color") .. "."))
            .. "\nColor changes preview live. Done keeps the selected color; Cancel restores the original color.",
        status = "navigated",
        result = "navigated",
        summary = summary or "Opens Color Painter for the exact Assistant color setting.",
...
local function ExactSettingValuePrompt(item, invalidValue)
    ...
    local pickerResult = R.OpenColorSettingPickerResult(item,
        invalidValue and "Opens Color Painter after an invalid color value."
            or "Opens Color Painter for a value-less color request.")
...
function R.TryOpenEndedSettingIdea(text, coreHandler)
    ...
    if analysis.explicitValue == true then
        ...
        -- needs a Buff/Debuff lane). Mutations may bypass the generic response
        -- only when a fixed-list value proves one unique candidate.
        local parseOk, specialized = false, nil
        if type(A.Parse) == "function" then parseOk, specialized = pcall(A.Parse, text) end
        local specialistCanRun = specialized and (
            specialized.kind == "answer" or specialized.kind == "ambiguous"
            or R.OpenEndedSpecialistProvesUniqueEnumValue(specialized, entries)
        )
        if parseOk and specialistCanRun
...
function R.SettingControlNoun(item)
    local setting = item and (item.setting or item)
    if type(A.DisplaySettingControl) == "function" then return A.DisplaySettingControl(setting, "noun") end
    return "control"
end
...
function R.TryLastChangeSettingFollowup(text)
    ...
    if asksLocation and not asksValue and not asksExplain then
        ...
        lines[#lines + 1] = "This is the last setting I changed. It lives on " .. pageLabel .. " and is " .. R.RegistrySettingTypeText(controlType, item) .. ". I did not change it from this location question."
    ...
    lines[#lines + 1] = label .. " lives on " .. pageLabel .. ". It is " .. R.RegistrySettingTypeText(controlType, item) .. ". I did not change it from this explanation question."
    if valueLine then lines[#lines + 1] = valueLine end
...
function R.RegistrySettingItemForKey(settingKey)
    ...
    -- AutoCoverage retains inverse legacy name-anchor spellings as raw
    -- compatibility fallbacks: unit frames own `nameTextAnchor`, while group
    -- frames own `nameAnchor`. An exact visible-label lookup can otherwise let
    -- the generated string shadow win before the semantic parser reaches the
    -- reviewed enum, turning a harmless explicit no-op into an unsafe refusal.
    if setting.generated == true and setting.type == "string" then
        local owner = settingKey:match("^(.-)%.nameAnchor$")
        local reviewedKey = owner and (owner .. ".nameTextAnchor") or nil
        if not reviewedKey then
            owner = settingKey:match("^(.-)%.nameTextAnchor$")
            reviewedKey = owner and (owner .. ".nameAnchor") or nil
        end
        local reviewed = reviewedKey and registry:GetSetting(reviewedKey) or nil
        if reviewed and reviewed.generated ~= true and reviewed.type == "enum" then
            settingKey, setting = reviewedKey, reviewed
        end
    end
...
function A.RouterTryRegistrySettingExplainShortcut(text, coreHandler, precomputedEntries)
    local norm = R.Normalize(text)
    ...
    local lines = {
        ...
        label .. " lives on " .. pageLabel .. ". It is " .. R.RegistrySettingTypeText(controlType, item) .. ". I did not change it.",
    }
...
function A.RouterTryRegistrySettingCurrentValueShortcut(text, coreHandler, precomputedEntries)
    local norm = R.Normalize(text)
    ...
    local entries = precomputedEntries
        or (directItem and { { item = directItem, score = 9999, rawScore = 9999 } })
...
function A.RouterTryRegistrySettingLocationShortcut(text, coreHandler, precomputedEntries)
    local norm = R.Normalize(text)
    if not R.LooksLikeRegistrySettingLocationQuestion(norm) then return nil end
    local subject = R.RegistryLocationSubject(norm)
    local directKey = R.NameShorteningDotsSettingKey(text)
...
function R.ContextualVariants(text)    if R.ShouldSkipContext(text) then return nil end
    ...
    elseif M.activeKey == "opt_castbar" then
        ...
        if norm == "show timer" or norm == "enable timer" then R.AddUnique(variants, "show target cast time text") end
        if norm == "hide timer" or norm == "disable timer" then R.AddUnique(variants, "hide target cast time text") end
...
function A.RouterShouldPreferPageContext(text)
    if not R.CurrentPageContext() or R.ShouldSkipContext(text) then return false end
    return R.ContextualVariants(text) ~= nil
end
    ...
    for i = 1, #variants do
        ...
        if result and not A.RouterIsUnknownResult(result) then
            if not A.RouterIsAmbiguousResult(result) then
                local contextSummary = "Current-page context: " .. R.CurrentPageSummaryLabel()
                local priorSummary = tostring(result.summary or "")
                if not priorSummary:find("Current-page context:", 1, true) then
                    if priorSummary ~= "" and priorSummary ~= "MSUF options change." and priorSummary ~= "Changes the matched option." then
                        contextSummary = contextSummary .. " - " .. priorSummary
                    end
                    result.summary = contextSummary
                end
                return result
            end
...
function A.RouteInput(text, coreHandler)
    ...
    if A.Knowledge and type(A.Knowledge.LooksLikeComplementaryAddonQuestion) == "function"
        and A.Knowledge.LooksLikeComplementaryAddonQuestion(text)
        and type(A.Knowledge.Answer) == "function"
    then
        local answer = A.Knowledge.Answer(text, { currentPage = M and M.activeKey })
        if answer then
            local hadPending = type(A.RouterHasBlockingPendingAssistantState) == "function"
                and A.RouterHasBlockingPendingAssistantState() == true
            local hadPendingResults = (type(A.RouterHasPendingSearchResults) == "function" and A.RouterHasPendingSearchResults())
                or (type(A.RouterHasPendingSelectedResult) == "function" and A.RouterHasPendingSelectedResult())
            if hadPending and type(A.CancelPendingMutationState) == "function" then
                A.CancelPendingMutationState()
            end
            if type(A.RouterClearPendingChoicesForRoute) == "function" then
                A.RouterClearPendingChoicesForRoute()
            end
            if type(A.RouterClearPendingResultsForRoute) == "function" then
    ...
    -- Pending workflows must see the user's original casing and punctuation.
    -- The routing-language normalizer below intentionally lowercases ordinary
    -- intent text, which is correct for matching but would corrupt free-form
    -- values and profile names (for example "Shadow Word: Pain").
    ...
    local function Core(value)
        ...
        if coreCache[value] == nil then
            ...
            if result and result._readOnlyGuard == true then result = nil end
            if result and R.IsExplicitNavigationCommand(value) then
                result = R.AsNavigationResult(result)
            elseif result and R.IsExplicitReadOnlyDiagnosticCommand(value) then
    ...
    local scopedAuraGrowthClarification = R.TryScopedAuraGrowthClarification
        and R.TryScopedAuraGrowthClarification(text)
    if scopedAuraGrowthClarification then return scopedAuraGrowthClarification end

    local scopedAuraGrowthReadOnly = R.TryScopedAuraGrowthReadOnly
        and R.TryScopedAuraGrowthReadOnly(text, Core)
    if scopedAuraGrowthReadOnly then return scopedAuraGrowthReadOnly end
    ...
    do
        ...
        local broadImportantAura = normalized == "show important debuffs"
            or normalized == "show important buffs"
        local earlyPlanningIntent = R.ContainsAny(normalized, terms.compare or {})
            ...
            or R.ContainsAny(normalized, terms.vagueTarget or {})
            or broadImportantAura
    ...
    -- Retired Private Aura layout labels still exist in generated read-only
    -- inventory data, but they must never alias ordinary Buff/Debuff lanes.
    -- Resolve this safety boundary before the setting browser or visible-page
    -- context can reinterpret the requested size/anchor/count as a live Aura
    -- mutation. The parser owns the canonical explanation when available;
    -- the local fallback keeps the public route fail-closed during partial
    -- runtime loads as well.
    do
        local normalized = R.Normalize(text)
        if R.ContainsAny(normalized, {
            "private aura", "private auras", "private-aura", "private-auras",
        }) then
            local parsed = type(A.Parse) == "function" and A.Parse(text) or nil
            if type(parsed) == "table" and parsed.kind == "answer"
                and tostring(parsed.status or "info") == "info"
            then
                return {
                    text = parsed.text,
                    status = "info",
    ...
    -- Complete reviewed semantic commands outrank page-local terse context.
    -- Scope and value are deliberately closed so this cannot broaden writes:
    ...
    then
        ...
        if reviewedSemanticMutation and not A.RouterIsUnknownResult(reviewedSemanticMutation) then
            return reviewedSemanticMutation
        end
    end
    ...
    local parserIntent = A.Parser and type(A.Parser.NonMutatingIntent) == "function"
        and A.Parser.NonMutatingIntent(text) or nil
    if hasPendingChoices
        and not pendingResultReply
        and not A.RouterHasPendingConfirmationOrFlow()
        and ((parserIntent == "problem" and not A.RouterLooksLikePendingChoiceFollowup(text))
            or A.RouterLooksLikePendingChoiceTopicSwitch(text))
    then
        A.RouterClearPendingChoicesForRoute()
        hasPendingState = A.RouterHasPendingAssistantState()
        hasBlockingPendingState = A.RouterHasBlockingPendingAssistantState()
    ...
    if explicitSearchRequest and not hasBlockingPendingState and not pendingResultReply then
        local compactSearch = R.TryCompactExplicitSettingSearch(text)
        if compactSearch then return compactSearch end
    end
    if not hasPendingState and not pendingResultReply then
        local contextlessReference = R.TryContextlessReferenceClarification(text)
        if contextlessReference then return contextlessReference end
    end
    if R.IsExactGenericDiagnosticRequest(text) and not pendingResultReply then
        local diagnosticResult = Core(text)
        if diagnosticResult then return R.AsReadOnlyResult(diagnosticResult) end
    end
    ...
    if not hasBlockingPendingState and not pendingResultReply then
        ...
        -- "move player buffs" as "show player buffs".
        local incompleteMovement = not R.TextMovementIntent(text)
            and R.TryIncompleteMovementConversation(text) or nil
        if incompleteMovement then return incompleteMovement end
        ...
        local germanLocation = normForGermanLocation:match("^wo%s+")
            or normForGermanLocation:match("^wie%s+kann%s+ich%s+")
            or normForGermanLocation:match("^welche%s+einstellung%s+")
            or normForGermanLocation:match("^welche%s+option%s+")
        if germanLocation and R.LooksLikeRegistrySettingLocationQuestion(text) then
            local subject = R.RegistryLocationSubject(text)
            local entries = subject ~= "" and R.CompactRegistrySettingSearchEntries(subject, 3) or nil
            if entries then
                local location = A.RouterTryRegistrySettingLocationShortcut and A.RouterTryRegistrySettingLocationShortcut(text, Core, entries)
                if location then return location end
            end
        end
        ...
        local earlySignalProblemResult = R.TrySignalProblemShortcut(text)
        if earlySignalProblemResult then return earlySignalProblemResult end
        ...
        if compactExplainEntries then
            local compactExplainResult = A.RouterTryRegistrySettingExplainShortcut and A.RouterTryRegistrySettingExplainShortcut(text, Core, compactExplainEntries)
            if compactExplainResult then return compactExplainResult end
        end
    ...
    if not hasBlockingPendingState and not pendingResultReply then
        ...
        -- cross-frame, dependent-frame, and registry specialists ahead of it;
        -- then preserve how/where questions and guide incomplete text movement.
        local conversationalMutation = R.TryConversationalMutation(text, Core)
        if conversationalMutation then return conversationalMutation end
