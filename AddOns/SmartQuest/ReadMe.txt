===========
Smart Quest
===========

Author: Zensunim of Dragonblight

Usage: /SQ or go to Interface->Add-ons->Smart Quest

Special thanks: 
	The creators of SCT, PartyQuest, QuestAnnouncer, and CT_RaidAssist

Change Log:
1.0 
* Initial Release.

1.1
* Added menu options

1.1.1
* Bug fix: Sound options not saving correctly

1.2
* Fixed TOC
* Color options for quest monitoring text

1.2.1
* Updated TOC for 3.0
* Fixed /sq monitor option

1.3
* Updated for Cataclysm

1.4
* Released to the public
* Added text color configuration

1.5
* Updated sounds to play on the Master channel instead of the SFX channel

1.5.1
* Updated TOC to 4.1

1.5.2
* Updated TOC to 4.2

1.5.3
* Updated TOC to 4.3

1.5.4
* Updated TOC to 4.3 (for real this time)

1.5.5
* Updated TOC to 5.0.4

1.5.6
* Updated TOC to 5.1

1.5.7
* Updated TOC to 5.2

1.5.8
* Updated TOC to 5.3

1.6
* Updated communication system for 5.4.7 changes
* Updated TOC to 5.4

1.7
* Updated communication system for 6.0 changes

1.7.1
* Fixed LUA bug on fresh startup

1.7.2
* Updated TOC to 6.0

1.7.3
* Converted all sounds to .ogg format

1.7.4
* Fixed quest complete status detection

1.7.5
* Updated TOC to 6.2

1.8
* Added quest turn-in support

1.9
* Added Legion support

1.9.1
* Fixed Legion support for latest beta
* Updated TOC to 7.0

1.10
* Fixed a bug with quest names having the same name

1.11
* Added support for 7.1

1.11.1
* Updated TOC to 7.2

1.11.2
* Updated TOC to 7.3

1.12
* Additional sound options

1.13
* Added support for 8.0

1.14
* Reduced sound spam

1.14.1
* Updated TOC for 8.0

1.15
* Added support for outputting messages into another chat window
* Bug fixes

1.15.1
* Bug fixes

1.16
* Additional sound options
* Bug fixes

1.16.1
* Bug fixes

1.16.2
* Fixed "Your Not In a Party" bug while in LFR/LFG

1.16.3
* Updated TOC for 8.1

1.17
* WoW Classic support

1.17.1
* Updated TOC for 8.2

1.18
* WoW Classic 1.13.2 support

1.18.1
* Updated TOC for 8.2.5

1.18.2
* Updated TOC for 8.3.0, Classic 1.13.3

1.19
* WoW Shadowlands support

1.20
* Updated TOC for 9.0

1.21
* LUA error bug fix

1.21.1
* Updated TOC for 9.0.5

1.22
* WoW Classic 2.x support
* Updated TOC for Classic 2.5.1

1.22.1
* Updated TOC for 9.2

1.23
* Added support for percentage progress quests

1.23.1
* Added support for Classic 3.0.4

1.24
* Added support for Dragonflight

1.25
* Added support for self quest monitoring

1.25.1
* Updated TOC for 10.0

1.25.2
* Updated TOC for 10.0.2

1.25.3
* Updated TOC for 10.1.7

1.25.4
* Updated TOC for 10.2
* Updated TOC for Classic 1.4
* Updated TOC for Wrath 3.4.3

1.26
* Fixed issue with color picker not working
* Updated TOC for 10.2.7
* Updated TOC for Cataclysm 4.4.0
