...
local children = {}
local function internalRegisterSettings(savedvariable, settings, title)
	-- create a vertical layout category, handing off all elements to Blizzard
	local category = Settings.RegisterVerticalLayoutCategory(title or C_AddOns.GetAddOnMetadata(addonName, 'Title'))
...
```
--]]
function addon:RegisterSettings(savedvariable, settings, title)
	...
	if isReady then
		internalRegisterSettings(savedvariable, settings, title)
	else
