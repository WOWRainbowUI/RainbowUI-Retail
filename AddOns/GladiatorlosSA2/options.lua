...
function GSA:ShowConfig()
	Settings.OpenToCategory(L["GladiatorlosSA"]) -- 暫時修正
...
local function setOption(info, value)
	...
	gsadb[name] = value
	if value then
		if not gsadb.path_menu then print("請先選擇測試語音包。") return end -- 暫時修正
					...
					spellAuraApplied = {
						...
						args = {
							...
							dk	= {	-- AuraApplied
								...
								name = L["DeathKnight"],
								order = 50,
								args = listOption({48792,55233,51271,48707,152279,219809,194679,194844,207319,212332,212337,91800,91797,116888,49039,288977,315443,287254},"auraApplied"),
							},
							...
							druid = { -- AuraApplied
								...
								name = L["Druid"],
								order = 70,
								args = listOption({102560,102543,102558,33891,61336,22812,252216,1850,69369,102342,102351,155835,29166,383410,305497,163505,106951,108291,108292.108293,108294,22842,132158,124974,382912,200851},"auraApplied"),
							},
							evoker = { -- AuraApplied
								type = 'group',
								inline = true,
							...
							hunter = { -- AuraApplied
								...
								name = L["Hunter"],
								order = 80,
								args = listOption({19263,53271,62305,53480,186265,186257,212640,288613,193530,266779,3355,202748,212704,356727,357021,264735,360966,360952,117526},"auraApplied"),
							},
							mage = { -- AuraApplied
								type = 'group',
								inline = true,
								name = L["Mage"],
								order = 90,
								args = listOption({45438,12472,198111,198144,86949,87024,190319,110909,342246,353128,82691,353084,389831},"auraApplied"),
							},
							monk = { -- AuraApplied
								type = 'group',
								inline = true,
							...
							priest	= { -- AuraApplied
								...
								name = L["Priest"],
								order = 120,
								args = listOption({33206,47585,47788,197862,197871,200183,197268,109964,47536,228260,15286,213610,10060,328530,372616,372760,211319},"auraApplied"),
							},
							rogue = { -- AuraApplied
								type = 'group',
								inline = true,
					...
					spellCastStart = {
						...
						args = {
							...
							mage = { -- CastStart
								...
								name = L["Mage"],
								order = 80,
								args = listOption({365350,28271,28272,61305,61721,61025,61780,161372,161355,161353,161354,126819,118,31687,203286,199786,113724,257537,376103,353082,352278,389794},"castStart"),
							},
							monk = { -- CastStart
								type = 'group',
								inline = true,
							...
							paladin = { -- CastStart
								...
								name = L["Paladin"],
								order = 100,
								args = listOption({20066,10326,200652,410126,2812},"castStart"),
							},
							priest	= { -- CastStart
								type = 'group',
								inline = true,
								name = L["Priest"],
								order = 110,
								args = listOption({9484,605,32375,265202,289666,375901,214621,373178},"castStart"),
							},
							--rogue = { -- CastStart
							--	type = 'group',
							--	inline = true,
							...
							shaman	= { -- CastStart
								...
								name = L["Shaman"],
								order = 130,
								args = listOption({210873,211004,211015,211010,51514,191634,320137,210714,117014},"castStart"),
							},
							warlock	= { -- CastStart
								type = 'group',
								inline = true,
								name = L["Warlock"],
								order = 140,
								args = listOption({5782,710,152108,691,712,697,112866,112867,112870,112868,112869,30146,157757,157898,688,316099,116858,30283,115268,6358,265187,29893,183601,6201,386997,6353,353753,267217},"castStart"),
							},
							warrior = { -- CastStart
								type = 'group',
								inline = true,
					...
					spellCastSuccess = {
						...
						args = {
							...
							general = { -- CastSuccess
								...
								name = L["General Abilities"],
								order = 30,
								args = listOption({178207,2825,272678,80353,90355,160452,32182,204361,204362,28730,232633,25046,50613,69179,155145,129597,202719,80483,107079,20549,58984,20594,7744,59752,287712,295707,42292,214027,195710,208683,23034,23035,213664,6262,265221,256948,257040,255654,368970,357214},"castSuccess"),
							},
							enemyInterrupts = { -- CastSuccess
								...
								order = 35,
								args = listOption({47528,183752,78675,106839,147362,187707,2139,116705,96231,1766,57994,19647,132409,119910,251523,251922,288047,171140,171138,115781,212619,119910,6552,351338},"castSuccess"),
							},
							dk	= { -- CastSuccess
								type = 'group',
								inline = true,
								name = L["DeathKnight"],
								order = 40,
								args = listOption({47476,207127,47568,207289,207349,49206,77606,108194,108199,152280,207167,204160,305392,190778,275699,49576,212468,212552,49028,48265,203173,48743,46584,51052,46585,327574,343294,279302,63560,210128},"castSuccess"),
							},
							demonhunter = { -- CastSuccess
								...
								name = L["DemonHunter"],
								order = 50,
								args = listOption({202138,202140,207684,207682,202137,179057,206649,205604,205629,205630,211881,221527,217832,196718,198013,390163,258920},"castSuccess"),
							},
							druid = { -- CastSuccess
								type = 'group',
								inline = true,
								name = L["Druid"],
								order = 70,
								args = listOption({740,108238,99,5211,102359,102417,102383,49376,16979,102416,102401,203651,201664,61391,132469,5215,236026,22570,209749,2908,202246,102793,197721,391888,393414,18562,354654,274837,392160},"castSuccess"),
							},
							evoker = { -- CastSuccess
								type = 'group',
								inline = true,
								name = L["Evoker"],
								order = 75,
								args = listOption({358385,357210,374251,368432,370665,372048,374227,374968,370452,358267,360995,363534,370537,403631},"castSuccess"),
							},
							hunter = { -- CastSuccess
								type = 'group',
								inline = true,
								name = L["Hunter"],
								order = 80,
								args = listOption({109248,109304,131894,208652,205691,201430,213691,187650,186387,1543,199483,236776,248518,375891,257284,19577,212431,19574,356719,356707,136,388045,162488,203415,359844,407028},"castSuccess"),
							},
							mage = { -- CastSuccess
								type = 'group',
								inline = true,
								name = L["Mage"],
								order = 90,
								args = listOption({66,110959,198158,12051,30449,153561,205021,235219,235450,235313,11426,205025,108839,31661,55342,122,382440,389713,321507,198100},"castSuccess"),
							},
							monk = { -- CastSuccess
								type = 'group',
								inline = true,
								name = L["Monk"],
								order = 100,
								args = listOption({116841,119381,123904,115078,101643,119996,137639,115310,388615,132578,115080,233759,122470,116844,233759,202370,325197,322118,386276,388193,113656,392983,325153,324312},"castSuccess"),
							},
							paladin = { -- CastSuccess
								type = 'group',
								inline = true,
								name = L["Paladin"],
								order = 110,
								args = listOption({6940,199448,199452,31821,853,190784,115750,210256,633,343527,343721,388013,388007,388010,388011,375576,383185},"castSuccess"),
							},
							priest	= { -- CastSuccess
								type = 'group',
								inline = true,
								name = L["Priest"],
								order = 120,
								args = listOption({8122,34433,64044,15487,64843,19236,123040,204263,2050,88625,205369,211522,108968,271466,62618,263165,73325,215769,209780,289657,32379,316262,313347,109964,64901,373481,108920,314867,341374},"castSuccess"),
							},
							rogue = { -- CastSuccess
								type = 'group',
								inline = true,
							...
							shaman	= { -- CastSuccess
								...
								name = L["Shaman"],
								order = 140,
								args = listOption({198067,198103,192249,108281,118345,204437,51490,375982,356824,356738,108285,51533,409293},"castSuccess"),
							},
							...
							warlock = { -- CastSuccess
								...
								name = L["Warlock"],
								order = 150,
								args = listOption({6789,48018,48020,111859,111895,111896,111897,111898,1122,205180,201996,199954,199892,199890,80240,5484,205179,344566,234153,328774,353294,353601,89766,386951,221703,410598},"castSuccess"),
							},
							warrior	= { -- CastSuccess
								type = 'group',
								inline = true,
								name = L["Warrior"],
								order = 160,
								args = listOption({97462,5246,46968,107570,152277,228920,176289,1160,213915,236077,236236,236320,6544,206572,376079,64382,262161,167105,384318,384100,383762,385059,385952},"castSuccess"),
							},
						},
					},
				},
