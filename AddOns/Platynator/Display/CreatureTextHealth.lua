...
addonTable.Display.CreatureTextHealthMixin = {}

function addonTable.Display.CreatureTextHealthMixin:SetUnit(unit)
  self.unit = unit
  if self.unit then
...
function addonTable.Display.CreatureTextHealthMixin:Strip()
...
function addonTable.Display.CreatureTextHealthMixin:SetColor(r, g, b)
  if not r then
    local c =  self.details.color
    r, g, b = c.r, c.g, c.b
  end
...
function addonTable.Display.CreatureTextHealthMixin:OnEvent(eventName, ...)
...
function addonTable.Display.CreatureTextHealthMixin:ApplyTarget()
  if self.details.showWhenWowDoes then
    self:SetShown(UnitIsUnit(self.unit, "target") or UnitShouldDisplayName(self.unit))
  end
end

function addonTable.Display.CreatureTextHealthMixin:ApplyTextOverride()
