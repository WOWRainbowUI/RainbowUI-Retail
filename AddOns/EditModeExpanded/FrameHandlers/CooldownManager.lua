...
function addon:initCooldownManager()
    ...
    if db.EMEOptions.cooldownManager then
        ...
        settingFrame.activeFontString:SetText("顯示圖示")
        ...
        settingFrame.addRow.addFontString:SetText("新增法術ID:")
        ...
        settingFrame.addItemRow.addFontString:SetText("新增物品ID:")
        ...
        local function initFrame(frame, db)
            lib:RegisterCustomButton(frame, "新增/移除自訂圖示", function()
            ...
            lib:RegisterCustomButton(frame, "新增飾品", function()
                local db = db[getCurrentLoadoutID(frame, db)]
        ...
        local dropdownOptions = {"無", "依持續時間在最上方", "依持續時間在最下方"}
        
        libDD:UIDropDownMenu_Initialize(dropdown, function(self, level, menuList)
            local db = getSettingDB()
            local info = libDD:UIDropDownMenu_CreateInfo()        
            
            if db.checked == nil then db.checked = "無" end
        ...
        libDD:UIDropDownMenu_SetText(dropdown, "排序圖示:")
        
        BuffBarCooldownViewer:HookScript("OnEvent", function(self)
            local settingDB = getSettingDB()
            if settingDB.checked == "無" then return end
            
            if settingDB.checked == "依持續時間在最上方" then
