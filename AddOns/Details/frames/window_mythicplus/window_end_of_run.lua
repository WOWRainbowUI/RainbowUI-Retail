...
local createPlayerBanner = function(parent, name)
    ...
    DetailsFramework:SetFontSize(playerNameFontString, 16)
	...
	DetailsFramework:SetFontSize(lootItemLevel, 16)
...
function mythicDungeonFrames.ShowEndOfMythicPlusPanel(bIsDebug)
	...
	if (Details222.MythicPlus.Level and Details222.MythicPlus.Level < 28 and not Details.user_is_patreon_supporter) then
		...
		if (not mythicDungeonFrames.ReadyFrame) then
			...
			--show button
			---@type df_button
			readyFrame.ShowChartButton = DetailsFramework:CreateButton(readyFrame, function() mythicDungeonCharts.ShowChart(); readyFrame:Hide() end, 80, 20, Loc["Show Damage Graphic"])
			...
			local timeNotInCombatLabel = DetailsFramework:CreateLabel(readyFrame, Loc["Time not in combat:"], textSize, "orangered")
			...
			local elapsedTimeLabel = DetailsFramework:CreateLabel(readyFrame, L["Run Time:"], textSize, textColor)
	...
	if (not mythicDungeonCharts.ReadyFrame) then
		...
		--warning footer
		local warningFooter = DetailsFramework:CreateLabel(readyFrame, Loc["You are seeing this because it's a 28 or above. Under development."], 12, "yellow")
		...
		---@type df_button
		readyFrame.ShowBreakdownButton = DetailsFramework:CreateButton(readyFrame, showBreakdownFunc, 145, 30, Loc["Show Breakdown"])
		...
		---@type df_button
		readyFrame.ShowChartButton = DetailsFramework:CreateButton(readyFrame, showChartFunc, 145, 30, Loc["Show Damage Graphic"])
		...
		local elapsedTimeLabel = DetailsFramework:CreateLabel(readyFrame, Loc["Run Time:"], textSize, textColor)
	...
	C_Timer.After(1.5, function()
		...
		if (overallMythicDungeonCombat:GetCombatType() == DETAILS_SEGMENTTYPE_MYTHICDUNGEON_OVERALL) then
			...
			readyFrame.TimeNotInCombatAmountLabel.text = Loc["Unknown for this run"]
		end
	end)
