...
function mythicDungeonCharts.ShowChart()
	if (not mythicDungeonCharts.Frame) then
		...
		local enabledSwitch, enabledLabel = detailsFramework:CreateSwitch(dungeonChartFrame, on_switch_enable, Details.mythic_plus.show_damage_graphic, _, _, _, _, _, _, _, _, _, Loc["Enabled"], detailsFramework:GetTemplate("switch", "OPTIONS_CHECKBOX_BRIGHT_TEMPLATE"), "GameFontHighlightLeft")
	...
	detailsFramework:SetFontSize(mythicDungeonCharts.Frame.ChartFrame.chart_title, 16)
