...
function mythicDungeonCharts.ShowReadyPanel()
	...
	if (not mythicDungeonCharts.ReadyFrame) then
		...
		DetailsFramework:CreateTitleBar (readyFrame, Loc["Details! Damage Chart for M+"])
...
function mythicDungeonCharts.ShowChart()
	if (not mythicDungeonCharts.Frame) then
		...
		--titlebar when minimized
			...
			--title
			local titleLabelMinimized = Details.gump:NewLabel(titlebarMinimized, titlebarMinimized, nil, "titulo", Loc["Dungeon Run Chart"], "GameFontHighlightLeft", 14, {227/255, 186/255, 4/255})
		...
		local enabledSwitch, enabledLabel = Details.gump:CreateSwitch(dungeonChartFrame, on_switch_enable, Details.mythic_plus.show_damage_graphic, _, _, _, _, _, _, _, _, _, Loc["Enabled"], Details.gump:GetTemplate("switch", "OPTIONS_CHECKBOX_BRIGHT_TEMPLATE"), "GameFontHighlightLeft")
