...
function itemProto:GetCategory()
  ...
  if self.data.itemInfo.equipmentSet then
    self.data.itemInfo.category = L:G("Gear: ") .. self.data.itemInfo.equipmentSet
    return self.data.itemInfo.category
  end
...
function itemFrame:_DoCreate()
  ...
  local ilvlText = button:CreateFontString(nil, "OVERLAY", "GameFontNormalOutline") -- 自行修改
  ilvlText:SetPoint("TOPLEFT", 2, 0) -- 自行修改
