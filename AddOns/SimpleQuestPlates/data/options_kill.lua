...
function SQP:CreateKillOptions(content)
    ...
    killAnimIntensityLabel:SetText(string.format(self.L["Intensity: %d%%"], SQPSettings.killAnimationIntensity or 100))
    ...
    local killAnimIntensityReset = self:CreateInlineResetButton(leftColumn, function()
        ...
        killAnimIntensityLabel:SetText(self.L["Intensity: 100%"])
        ActivateKill()
        SQP:RefreshAllNameplates()
    end)
    ...
    killAnimIntensitySlider:SetScript("OnValueChanged", function(self, newVal)
        ...
        killAnimIntensityLabel:SetText(string.format(self.L["Intensity: %d%%"], newVal))
        ActivateKill()
        SQP:RefreshAllNameplates()
    end)
    yOffset = yOffset - 38
    ...
    resetBtn:SetScript("OnClick", function()
        ...
        if oc.killAnimationIntensityLabel then
            oc.killAnimationIntensityLabel:SetText(string.format(self.L["Intensity: %d%%"], D.killAnimationIntensity))
        end
