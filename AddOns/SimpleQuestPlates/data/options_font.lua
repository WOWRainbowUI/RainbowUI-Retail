...
function SQP:CreateFontOptions(content)
    ...
    local fontOptions = {
        ...
        {text = "bLEI00D (中文)", value = "bLEI00D", font = "Fonts\\bLEI00D.TTF"},
    ...
    local thinButton = self:CreateStyledButton(leftColumn, self.L["None"] or "None", 80, 25)
    thinButton:SetPoint("TOPLEFT", 20, yOffset)
    
    local normalButton = self:CreateStyledButton(leftColumn, self.L["Normal"] or "Normal", 80, 25)
    normalButton:SetPoint("LEFT", thinButton, "RIGHT", 5, 0)
    
    local thickButton = self:CreateStyledButton(leftColumn, self.L["Thick"] or "Thick", 80, 25)
    ...
    local function CreateColorPicker(parent, label, colorKey, x, y)
        ...
        -- Reset button
        local resetBtn = self:CreateStyledButton(container, self.L["Reset"] or "Reset", 50, 20)
