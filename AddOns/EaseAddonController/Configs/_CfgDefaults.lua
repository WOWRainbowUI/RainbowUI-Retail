...
D["DiGuaTimelineAudioHelper"] = {
	defaultEnable = 0,
	tags = { "BOSSRAID" },
	title = "神秘地瓜副本語音助手",
	desc = "戰鬥通知插件，透過戰鬥時間軸系統在首領戰中提供精準的語音提示。``相較於傳統僅依靠視覺計時器的模組，它能根據戰鬥節奏自動播放自訂音效，並支援私有光環機制、角色/專精過濾，以及事件驅動的智慧觸發。``這讓玩家能在激烈戰鬥中專注於戰場，不必分心盯著進度條，同時也非常適合團隊領隊用來建立節奏感或提醒重要技能。`",
};
D["Dominos"] = {
	defaultEnable = 1,
	title = "達美樂快捷列",
...
D["MyusKnowledgePointsTracker"] = {
    defaultEnable = 0,
	tags = { "PROFESSION" },
	title = "專業點數追蹤",
	desc = "幫助玩家更有效率地管理專業技能知識點。``插件整合了社群在 Beta 測試中找到的大部分寶藏，並提供多種追蹤模式：包括顯示獨特寶藏位置與需求、追蹤每週任務與寶藏、檢視隱藏的貨幣來源（如製作訂單、採集、分解），以及計算完成整個技能樹所需的剩餘知識點。``玩家可透過右鍵快速切換資料片，並使用 /mkpt 指令來顯示或隱藏介面、小地圖圖示、插件選單入口，或調整介面大小。``這款插件不僅提供清晰的功能結構，也結合社群貢獻與測試回饋，成為專業技能玩家在新版本中不可或缺的工具。`",
	modifier = "彩虹ui",
	{
		type = "text",
        text = "使用方法: 點小地圖按鈕。",       
...
D["Myslot"] = {
	...
	tags = { "ACTIONBAR" },
	title = "匯出/匯入快捷列",
	desc = "把你的快捷列配置、巨集和按鍵綁定匯出成一段文字，之後可貼到其他角色、帳號或電腦重新匯入，省去重新排技能和設定快捷鍵的時間。``最常見的用途就是：先把主角色排好的技能欄與按鍵設定「存檔」，之後練分身、換電腦或重裝介面時，再用這個插件快速還原。``要注意的是，匯入會變更目前角色的快捷列與按鍵綁定，建議在操作前先匯出一份現有設定作備份。不同職業也可以匯入，但角色沒有的技能或不能使用的法術不會正常出現在可用配置中。`",
	{
        text = "設定選項",
        callback = function() SlashCmdList["MYSLOT"]("") end,
    },
};
D["NoAutoClose"] = {
    defaultEnable = 0,
