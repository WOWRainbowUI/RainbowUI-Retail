...
-- Create a module.
local module = addon:CreateModule("AuraSkin_BigDefensives")

-- Setup the module.
function module:OnEnable()
  local db_obj = CopyTable(addon.db.profile.module_data.AuraSkin_BigDefensives)
  local border_color = db_obj.border_color
  local border_size = db_obj.border_size
  local indicator_scale = db_obj.indicator_scale
  ...
  local function on_frame_setup(cuf_frame)
    style_buff_frame(cuf_frame.CenterDefensiveBuff)
    cuf_frame.CenterDefensiveBuff:SetScale(indicator_scale)
  end
