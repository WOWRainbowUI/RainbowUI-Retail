...
local function BuildInterrupts()
    ...
    do
        ...
        title:SetText(LS("INFO_INT_127_TITLE", "Interrupt Tracker reworked"))
...
function BIT.SettingsUI:CreateMinimapButton()
    ...
    local dataObj = ldb:NewDataObject("BliZziInterrupts", {
        ...
        OnTooltipShow = function(tt)
            tt:AddLine("|cff0091edBliZzi|r |cffffa300Party Tools|r", 1, 1, 1)
            tt:AddLine(" ")
            ...
            local function statusLine(label, state)
                ...
                if state == "active" then
                    txt, r, g, b = "Active", 0.30, 1.00, 0.30
                elseif state == "idle" then
                    txt, r, g, b = "Idle", 1.00, 0.82, 0.00
                elseif state == "hidden" then
                    txt, r, g, b = "Hidden", 0.45, 0.65, 0.95
                else
                    txt, r, g, b = "Deactivated", 0.60, 0.60, 0.60
                end
            ...
            statusLine("Interrupt Tracker", intState)
            ...
            statusLine("PI Caller", "deactivated")
            ...
            statusLine("Smart Misdirect", smdState)
            ...
            statusLine("Keystone List", keyState)
            ...
            if IsInGroup and IsInGroup() and not (IsInRaid and IsInRaid()) then
                tt:AddLine(" ")
                tt:AddLine("Group (LibSpec)", 0.9, 0.9, 0.9)
                for _, unit in ipairs({ "player", "party1", "party2", "party3", "party4" }) do
                    if UnitExists(unit) then
                        ...
                        if type(name) == "string" and name ~= "" then
                            ...
                            if has then
                                tt:AddDoubleLine(colored, "Yes", 1, 1, 1, 0.30, 1.00, 0.30)
                            else
                                tt:AddDoubleLine(colored, "No", 1, 1, 1, 1.00, 0.35, 0.35)
                            end
                        end
                    end
                end
            ...
            tt:AddLine("|cFFFFD700Right-Click|r  Move all frames on/off", 0.8, 0.8, 0.8)
...
function BIT.SettingsUI:ShowReworkNotice()
    ...
    titleFs:SetText(LS("REWORK_TITLE", "Interrupt Tracker reworked"))
    ...
    bodyFs:SetText(LS("REWORK_BODY",
        "Ever since the Midnight release I've been trying to make the interrupt tracker perfect. But other players' interrupts simply can't be tracked 100% reliably — the game doesn't expose that to addons.\n\n"
    ...
    ackLbl:SetText(LS("REWORK_ACK", "Click here and this notice won't appear again"))
