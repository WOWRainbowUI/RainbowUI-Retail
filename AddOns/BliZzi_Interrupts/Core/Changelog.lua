-- BliZzi Interrupts — In-game changelog data
--
-- Mirrors CHANGELOG.md so users can read release notes without
-- alt-tabbing out of the game. Newest version FIRST — the settings
-- page renders one tab per entry, tab order matches the table order.
--
-- Format per entry:
--   {
--     version = "4.0.1",
--     date    = "2026-05-18",      -- optional, shown next to the tab label
--     bullets = {                  -- short-form, one line per UI row
--       "Title — body text…",
--       ...
--     },
--   }
--
-- Keep bullets concise (the in-game panel doesn't word-wrap nicely past
-- ~120 chars). For longer detail, point users at CHANGELOG.md in the
-- addon folder via a footer label.
...
BIT.CHANGELOG = {
    ...
    },
    {
        ...
        bullets = {
            ...
            "Removed: Interrupt Tracker Display Mode setting — standalone-only now; attached modes couldn't work with the interrupt history. Existing attached-mode users are moved back to the window automatically.",
            ...
            "Fixed: failed interrupt shows the red countdown number again (red feedback had been turned off during the rework, leaving the number blank on a wasted kick).",
            "Changed: hovering your interrupt icon during the success overlay shows the interrupted spell's tooltip (matching the icon); in instances where the enemy spell is hidden, no tooltip is shown there instead of the mismatched own-kick one.",
            "Fixed: bar contents no longer shift ~1px (e.g. the player name) on the first setting change after login — bars are now pixel-snapped to the final frame scale at load.",
        },
    },
    {
        version = "4.1.0",
    ...
    },
    {
        ...
        bullets = {
            ...
            "Fixed: phantom Rebuke CD ticking on a nearby Pala when the local player kicked (event-order race).",
