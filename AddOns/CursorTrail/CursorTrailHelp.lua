...
啟用「閃閃發光」時，所選的圖形顏色將被忽略，而圖形將改為「閃爍」。(閃閃發光不會影響尾巴顏色。)

* 尾巴: 可供選擇的游標動畫效果清單。可以利用下方的「調整位置」來移動位置。

* 陰影%: 控制黑色背景圓圈的強度。99% 最暗，而 0% 是隱形 (關閉)。

* 縮放大小%: 控制效果的大小。可以是 1 到 998。

* 不透明度%: 控制圖形和尾巴的透明度。100% 完全可見，而 0% 則是隱形 (關閉)。

* 框架層級: 控制圖形和尾巴繪製在其他使用者介面的後面或前面，不會影響陰影選項。(背景會繪製在最底層，而浮動提示則是最高層。)

* 調整位置: 移動尾巴效果的中心位置。第一個數字框是水平移動 (負數向左移動，正數向右移動)。第二個數字框是垂直移動 (負數向下移動，正數向上移動)。
...
- 如果縮放設定得太低，部分尾巴會消失。如果選擇尾巴後卻沒有顯示，請嘗試較大的縮放百分比。(所有尾巴皆能在 100% 縮放下運作。)
...
如果輸入 /ct 時 CTMod 一直開啟，請使用 /cursortrail 開啟滑鼠的尾巴。
如果輸入 /ct 時滑鼠的尾巴一直開啟，可以手動將滑鼠的尾巴的指令變更為其他指令，例如 /ctr。
  重要提醒 - 您需要在每次下載滑鼠的尾巴後重複做變更。
  1.使用任何文字編輯器，開啟位於 Warcraft 插件資料夾內滑鼠的尾巴資料夾中的 CursorTrail.lua。
