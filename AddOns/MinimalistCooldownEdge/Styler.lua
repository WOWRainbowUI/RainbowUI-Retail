...
local GetFrameUnitToken
local GetFrameAuraInstanceID
local GetCooldownSpellID
...
local MAX_COOLDOWN_OWNER_SCAN_DEPTH = STYLER_CONSTANTS.MaxCooldownOwnerScanDepth
...
GetFrameUnitToken = function(frame)
...
GetFrameAuraInstanceID = function(frame)
    if not frame then return nil end
...
local function IsCompactPartyCenterDefensiveBuffFrame(frame)
    if not frame or not frame.GetParent then
        return false
    end

    local parent = frame:GetParent()
    return parent and parent.CenterDefensiveBuff == frame or false
end
...
local function GetCompactGroupFrameTypeFromUnit(unitToken)
    ...
    if strfind(unitToken, GROUP_FRAME_TYPE.Raid, 1, true) == 1 then
        return GROUP_FRAME_TYPE.Raid
    end

    if strfind(unitToken, GROUP_FRAME_TYPE.Party, 1, true) == 1 then
...
local function GetCompactGroupFrameTypeFromName(frameName)
    if type(frameName) ~= "string" then
        return nil
    end

    if strfind(frameName, "CompactRaidFrame", 1, true) then
        return GROUP_FRAME_TYPE.Raid
    end

    if strfind(frameName, "CompactPartyFrame", 1, true) then
        return GROUP_FRAME_TYPE.Party
    end
...
local function ShouldUseCompactPartyAuraText(config, frameType)
    ...
    if frameType == GROUP_FRAME_TYPE.Raid then
        return config.raidEnabled
    end

    if frameType == GROUP_FRAME_TYPE.Party then
        return true
    end
...
local function GetCompactPartyAuraFontSize(cdFrame, config)
    if not config then
        return nil
    end
...
local function rawNearlyEqual(left, right)
    return math_abs(left - right) < STYLER_CONSTANTS.NumericComparisonEpsilon
end
...
local function IsNearlyEqual(a, b)
    ...
    return math_abs(a - b) < STYLER_CONSTANTS.NumericComparisonEpsilon
end
...
local function ApplyCompactPartyAuraTextStyle(cdFrame, config)
    ...
    ApplyFontStringStyle(
        ...
        GetCompactPartyAuraFontSize(cdFrame, config),
...
local function SyncCompactPartyAuraCooldown(cdFrame)
    ...
    if RefreshTrackedDurationColor then
        RefreshTrackedDurationColor(cdFrame, CATEGORY.CompactPartyAura, config)
    end

    return true
end

local COOLDOWN_MEMBER_KEYS = STYLER_CONSTANTS.CooldownMemberKeys
...
local function ContainsNameplatePattern(value)
    for i = 1, #CLASSIFIER_CONSTANTS.NameplatePatterns do
        if strfind(value, CLASSIFIER_CONSTANTS.NameplatePatterns[i], 1, true) then
            return true
        end
    end
...
local function IsLiveNameplateAuraContext(cdFrame)
    ...
    while current and current ~= UIParent and depth < CLASSIFIER_CONSTANTS.ScanDepth do
        ...
        if not sawAuraContext then
            ...
            if type(name) == "string"
               and (strfind(name, "Buff", 1, true)
                    or strfind(name, "Debuff", 1, true)
                    or strfind(name, "Aura", 1, true)) then
                sawAuraContext = true
            elseif GetFrameAuraInstanceID(current) ~= nil then
                sawAuraContext = true
            end
        end
        ...
        if sawAuraContext then
            ...
            if type(name) == "string" and name ~= "" then
                ...
                if ContainsNameplatePattern(lowerName) then
                    return true
                end
            end
...
local function ShouldSuppressDurationColorsForNameplate(cdFrame, sourceKey)
    return sourceKey ~= CATEGORY.Nameplate and IsLiveNameplateAuraContext(cdFrame)
end

local function GetDurationResetConfig(cdFrame, sourceKey, config)
    if not ShouldSuppressDurationColorsForNameplate(cdFrame, sourceKey) then
        return config
    end
...
local function IsDurationColorEnabledForSource(cdFrame, sourceKey, config)
    ...
    if ShouldSuppressDurationColorsForNameplate(cdFrame, sourceKey) then
        return false
    end
...
GetCooldownSpellID = function(owner)
...
ResolveCooldownContext = function(cdFrame, forceRefresh)
    ...
    for _ = 1, MAX_COOLDOWN_OWNER_SCAN_DEPTH do
        ...
        -- Midnight adds a dedicated unnamed CenterDefensiveBuff button on compact
        -- party/raid frames, so name-based aura detection does not see it.
        if not compactPartyCenterDefensiveBuff and IsCompactPartyCenterDefensiveBuffFrame(current) then
           ...
           or strfind(name, "Aura", 1, true) then
            ...
            if compactPartyAuraType == false then
                if strfind(name, "CompactPartyFrame", 1, true) then
                    compactPartyAuraType = GROUP_FRAME_TYPE.Party
                elseif strfind(name, "CompactRaidFrame", 1, true) then
                    compactPartyAuraType = GROUP_FRAME_TYPE.Raid
                end
            end
        end

        if compactPartyAuraType == false and sawAuraContext and strfind(name, "Compact", 1, true) then
            -- Midnight's CenterDefensiveBuff button is unnamed, and party member 1
            -- commonly exposes unit="player", so unit-token detection alone is not
            -- enough to resolve Party vs Raid for the defensive center buff.
            local frameType = GetCompactGroupFrameTypeFromName(name)
                or GetCompactGroupFrameTypeFromUnit(GetFrameUnitToken(current))
            if frameType then
                compactPartyAuraType = frameType
            end
        end
...
local function ShouldUseAuraDurationFallback(cdFrame)
    ...
    if category == CATEGORY.Actionbar or category == CATEGORY.MiniCC then
        return false
    end

    if category == CATEGORY.CooldownManager then
        local viewerType = Classifier:GetCooldownManagerViewerType(cdFrame)
        return viewerType == VIEWER_TYPE.BuffIcon
    end

    if category == CATEGORY.Nameplate or category == CATEGORY.Unitframe then
        return true
    end
...
local function GetFallbackDurationObject(cdFrame, forceContextRefresh)
    local parent = cdFrame and cdFrame.GetParent and cdFrame:GetParent()
    if not parent then return nil end

    local fs = ResolveCooldownContext(cdFrame, forceContextRefresh == true)
...
local function GetCooldownDurationObject(cdFrame, sourceKey)
    if sourceKey == CATEGORY.Actionbar then
        -- Main action-bar buttons are frequently rebound/paged in combat.
        -- Re-read the live action duration so duration threshold colors do not
        -- evaluate a stale cached object from the previous slot occupant.
        local currentDurationObject = GetFallbackDurationObject(cdFrame, true)
...
ClearTrackedDurationColor = function(cdFrame)
    ...
    local fs = frameState[cdFrame]
    local hadDurationObject = fs and fs.durationObject ~= nil or false
    local hadAppliedTextColor = fs and fs.appliedTextColor ~= nil or false

    if not trackedSource and not wasActive and not hadDurationObject and not hadAppliedTextColor then
        return false, trackedSource, wasActive, hadDurationObject, hadAppliedTextColor
    end
    ...
    return true, trackedSource, wasActive, hadDurationObject, hadAppliedTextColor
end
...
local function UpdateDurationColors()
    ...
    if not curve then
        for cdFrame, sourceKey in pairs(durationColoredFrames) do
            ...
            ResetCountdownTextColor(cdFrame, GetDurationResetConfig(cdFrame, sourceKey, config))
            ClearTrackedDurationColor(cdFrame)
        end
        StopDurationColorTicker()
        return
...
local function HandleCooldownDurationUpdate(cooldown, durationObject)
    ...
    if sourceKey then
        ...
        if curve and IsDurationColorEnabledForSource(cooldown, sourceKey, config) then
            ...
            if config then
                ResetCountdownTextColor(cooldown, GetDurationResetConfig(cooldown, sourceKey, config))
            end
            ClearTrackedDurationColor(cooldown)
        end
    end
    ...
    local hasUsableDurationObject = durationObject and IsSupportedDurationObject(durationObject)
    local retryInterval = STYLER_CONSTANTS.AuraRetryMinInterval or 0.25
    local now = GetTime()

    if IsAuraDrivenCooldown(cooldown)
       and not hasUsableDurationObject
       and not fs.pendingAuraRefresh
       and (type(fs.nextAuraRefreshAt) ~= "number" or fs.nextAuraRefreshAt <= now) then
        fs.nextAuraRefreshAt = now + retryInterval
...
local function GetDesiredHideCountdownNumbers(cdFrame, category, config)
    ...
    if category == CATEGORY.Actionbar and IsAssistedCombatActionCooldown(cdFrame) then
        return true
    end
...
-- =========================================================================
-- STACK COUNT STYLING  (action bar + nameplate auras + CooldownManager viewers)
-- =========================================================================
...
local function GetStackCountRegion(cdFrame, category)
    ...
    elseif category == CATEGORY.Nameplate then
        -- Nameplate aura buttons commonly expose their applications via CountFrame.Count.
        -- Avoid generic Count fallbacks here because some nameplate addons reuse those
        -- fields for countdown text, which can be recolored by other style paths.
...
local function GetCooldownFontSize(cdFrame, category, config)
    ...
    local viewerType = Classifier:GetCooldownManagerViewerType(cdFrame)
    if viewerType == VIEWER_TYPE.Essential then
        return config.essentialFontSize or config.fontSize
    end
    if viewerType == VIEWER_TYPE.Utility then
...
local function ProcessPendingAuras()
    ...
    for cdFrame in pairs(pendingAuras) do
        if cdFrame and not MCE:IsForbidden(cdFrame) then
            ...
            if retryCategory == nil or retryCategory == CATEGORY.Global or retryCategory == CATEGORY.AuraPending then
                retryCategory = Classifier:ClassifyFrame(cdFrame)
                if retryCategory == CATEGORY.AuraPending then
                    retryCategory = cachedCategory ~= CATEGORY.AuraPending and cachedCategory or CATEGORY.Global
                end
            end
...
function Styler:ApplyStyle(cdFrame, forcedCategory)
    ...
    if forcedCategory == CATEGORY.Nameplate and Classifier:GetMiniCCFrameType(cdFrame) then
        forcedCategory = CATEGORY.MiniCC
    end

    trackedCooldowns[cdFrame] = true

    if SyncCompactPartyAuraCooldown(cdFrame) then
        return
    end

    if Classifier:IsBlacklisted(cdFrame) then return end

    if forcedCategory and forcedCategory ~= CATEGORY.Global then
    ...
    local isAssistedCombat = (category == CATEGORY.Actionbar and IsAssistedCombatActionCooldown(cdFrame))
    ...
    elseif category == CATEGORY.MiniCC then
    ...
    if needsFullRestyle or category == CATEGORY.MiniCC then
    ...
    if needsFullRestyle or textRegionsChanged then
        ...
        do
            ...
            local enforceFont = (category == CATEGORY.MiniCC)
