...
local function InitVariables_Once() -- {{{
    ...
    D.Revision = "97f733b"; -- not used here but some other add-on may request it from outside
    D.date = "2026-03-08T18:57:26Z";
    D.version = "2.8.0-RC2";

    if D.date ~= "@project".."-date-iso@" then
        -- 1772996246 doesn't work
...
function D:VersionWarnings(forceDisplay) -- {{{
    ...
    if DcrMaxTOC < T._tocversion then
        ...
        elseif time() - self.db.global.TocExpiredDetection > 3600 * 24 * 40 or debug then -- if more than 40 days elapsed since the detection
            ...
            if time() - self.db.global.LastExpirationAlert > 48 * 3600 or forceDisplay or debug then
                ...
                T._ShowNotice ("|cff00ff00Decursive version: 2.8.0-RC2|r\n\n" .. "|cFFFFAA66" .. L["TOC_VERSION_EXPIRED"] .. "|r");
    ...
    if (("2.8.0-RC2"):lower()):find("beta") or ("2.8.0-RC2"):find("RC") or ("2.8.0-RC2"):find("Candidate") or alpha then
        ...
        if self.db.global.NonRelease ~= "2.8.0-RC2" then
            self.db.global.NonRelease = "2.8.0-RC2";
            T._ShowNotice ("|cff00ff00Decursive version: 2.8.0-RC2|r\n\n" .. "|cFFFFAA66" .. (("2.8.0-RC2"):find("RC") and L["ER_VERSION_NOTICE"] or L["DEV_VERSION_ALERT"]) .. "|r");
        end
    end
...
T._LoadedFiles["DCR_init.lua"] = "2.8.0-RC2";
...
97f733b14644f9af085cefc16a102e48588069ae
