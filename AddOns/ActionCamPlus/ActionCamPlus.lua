...
BINDING_HEADER_ACTIONCAMPLUS = "動感鏡頭 Plus"
local CAMERA_ZOOM_PRECISION = .1
local CAMERA_ZOOM_MIN_CORRECTION = .04
local _

ACP.transitionFunctions = {
	{name = "線性", func = "linear", coefficients = {0, 0, 1, 1}},
	{name = "緩出", func = "cbSpecialCase", coefficients = {0, 1, 1, 1}},
	{name = "緩入", func = "ease", coefficients = {.6, 0, 1, 1}},
	{name = "緩入緩出", func = "ease", coefficients = {.5, 0, .5, 1}}
}
...
function ACP.SetPitch(enable)
	if enable then
	    ...
	    SetCVar("test_cameraDynamicPitchBaseFovPad", 0.4, "ACP") -- 上下偏移距離
	else
