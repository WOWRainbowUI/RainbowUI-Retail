...
function StreamOverlay.OpenOptionsPanel (fromOptionsPanel)
	if (not StreamOverlayOptionsPanel) then
		...
		if (not Details_StreamerDB.profiles [pname]) then
			...
			optionsFrame.NewProfileButton = Details.gump:CreateButton (optionsFrame, add_profile, 60, 18, Loc["New Profiile"], _, _, _, _, _, _, Details.gump:GetTemplate ("dropdown", "OPTIONS_DROPDOWN_TEMPLATE"), Details.gump:GetTemplate ("font", "OPTIONS_FONT_TEMPLATE"))
