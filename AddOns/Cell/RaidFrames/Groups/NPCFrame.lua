...
dumb:SetScript("OnDragStart", function()
    F.StartAnchorMoving(separateAnchor, function()
...
-- ! BLIZZARD, FIX IT!
-- NOTE: boss6-8 have no unit events of their own, and the CLEU top-up that used to cover
-- them is impossible on 12.x (addons cannot register COMBAT_LOG_EVENT_UNFILTERED), so their
-- health/auras come from the periodic poll (elapsed3 >= 5) plus the UNIT_HEALTH / UNIT_AURA
-- registrations below. The dead CLEU frame and its two arm/disarm timers are gone.
-------------------------------------------------
...
for i = 6, 8 do
    ...
    -- ⚠ one event frame PER SLOT. RegisterUnitEvent keeps at most two units per event, so a
    -- single shared frame would silently stop watching boss6 the moment boss8 registered --
    -- which is what the old shared `cleu` frame did once the CLEU path was gone.
    local events = CreateFrame("Frame")
    events:SetScript("OnEvent", function(_, event)
        if event == "UNIT_AURA" then
            B.UpdateAuras(button)
        else -- UNIT_HEALTH / UNIT_MAXHEALTH
    ...
    button.helper:HookScript("OnShow", function()
        ...
        if unit then
            ...
            events:RegisterUnitEvent("UNIT_AURA", unit)
        end
    end)
