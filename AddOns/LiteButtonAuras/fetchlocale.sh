...
fetch () {
    curl \
        -s -H "X-Api-Token: $APIKEY" \
        "https://wow.curseforge.com/api/projects/526431/localization/export?export-type=TableAdditions&lang=$1&unlocalized=ShowBlankAsComment" \
        | grep -v 'Translation missing' \
        | sed -e 's/^/    /; s/ *$//;'
}
