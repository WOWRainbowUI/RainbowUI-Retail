...
f:SetScript("OnEvent", function(self, event)
    ...
    local dataobj = LDB:NewDataObject("Stuf Unit Frames", {
        ...
        OnTooltipShow = function(tooltip)
            tooltip:AddLine(L["|cfffed100Stuf|cffffffff Unit Frames"])
            tooltip:AddLine(" ")
            tooltip:AddLine(L["|cffffffffLeft-click|r       Toggle config/drag mode"])
            tooltip:AddLine(L["|cffffffffRight-click|r      Open Stuf options"])
            if C_AddOns.IsAddOnLoaded("StufRaid") then
                tooltip:AddLine(L["|cffffffffAlt+Right-click|r Open StufRaid config"])
            end
        end,
    })
