...
local function StyleFactionSectionTab(button, active, hovered)
    if not (button and lv.GetTheme) then return end
    local t = lv.GetTheme()
    ...
    button.activeBridge:SetColorTexture(unpack(t.background))
...
local function CreateFactionSectionTab(mode, labelKey, anchor)
    local button = CreateFrame("Button", nil, LVWindow, "BackdropTemplate")
    ...
    button.Text:SetText(L[labelKey])
...
local function UpdateFactionCardEmblem(card)
    ...
    if resolvedAtlas then
        card.emblemIcon:SetAtlas(resolvedAtlas, true)
        card.emblemIcon:Show()
        card.emblemText:Hide()
        return
    end
...
local function ApplyFactionCardVisual(card, theme, hovered)
    ...
    card.emblemAccent:SetBackdropBorderColor(ar, ag, ab, 0.88)
...
local function CreateFactionCards()
    for index, cfg in ipairs(FACTION_CARD_CONFIG) do
        ...
        card.emblemAccent = CreateFrame("Frame", nil, card.emblemBg, "BackdropTemplate")
        card.emblemAccent:SetPoint("TOPLEFT", 3, -3)
        card.emblemAccent:SetPoint("BOTTOMRIGHT", -3, 3)
        card.emblemAccent:SetBackdrop({ edgeFile = "Interface\\Buttons\\WHITE8X8", edgeSize = 1 })
...
local function GetFactionRenownDetails(mode)
    ...
    if mode == "valeera" and C_GossipInfo and C_GossipInfo.GetFriendshipReputation then
        ...
        if friendship and friendship.friendshipFactionID then
            ...
            return {
                kind="friendship", factionID=factionID, name=friendship.name, level=math.min(level, 80),
    ...
    if standardFactionData then
        ...
        return {
            kind="standard", factionID=factionID, name=standardFactionData.name, reaction=standardFactionData.reaction or 0,
    ...
    if details then
        if details.kind == "friendship" then
            if details.isAtMaxRenown then
                renownText = string.format("%s %d - %s", UIText("LABEL_RENOWN_LEVEL"), details.maxLevel or 80, UIText("LABEL_MAXIMUM"))
            else
                renownText = string.format("%s %d (%d/%d)", UIText("LABEL_RENOWN_LEVEL"), details.level or 0, details.earned or 0, details.threshold or 0)
            end
...
local function BuildFactionTooltipText(details)
    ...
    local lines = {}
    if details.kind == "friendship" or details.kind == "standard" then
        lines[#lines + 1] = BuildFactionRenownText(details.kind == "friendship" and "valeera" or "slayersduellum")
    elseif details.isParagon then
...
RefreshFactionCards = function()
    ...
    for _, card in ipairs(factionCards) do
        ...
        elseif info.kind == "friendship" then
            card.levelText:SetText(string.format("|cffffd100%s %d|r", UIText("LABEL_RENOWN_LEVEL"), info.level or 0))
...
local function BuildFactionQuestText(quests)
    ...
    if #rows == 0 then
        return "No weekly completions tracked.", 0
    end
    return table.concat(rows, "\n"), #rows
end
