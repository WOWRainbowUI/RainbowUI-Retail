...
if lv.GetActiveUpgradeSeasonKey() == "midnight_s2" then
    lv.ORDERED_KEYWORDS[#lv.ORDERED_KEYWORDS + 1] = "Corrosive Coin"
end
for _, def in ipairs(lv.GetActiveCrestDefinitions()) do
    lv.ORDERED_KEYWORDS[#lv.ORDERED_KEYWORDS + 1] = def.key
end

-- Currency.lua is the authoritative Midnight currency list. Core's early
-- defaults exist only so startup code has a safe table before this module loads.
lv.CURRENCY_KEYWORDS = {}
for _, currencyName in ipairs(lv.ORDERED_KEYWORDS) do lv.CURRENCY_KEYWORDS[currencyName] = true end
...
if lv.GetActiveUpgradeSeasonKey() == "midnight_s2" then
    table.insert(CURRENCY_DISPLAY_ORDER, 6, "Corrosive Coin")
end
for _, def in ipairs(lv.GetActiveCrestDefinitions()) do
    CURRENCY_DISPLAY_ORDER[#CURRENCY_DISPLAY_ORDER + 1] = def.key
end
...
local function CanonicalizeCrestKey(rawName)
    ...
    local family = n:find("mistcrest", 1, true) and "Mistcrest" or (n:find("dawncrest", 1, true) and "Dawncrest" or nil)
    if not family then return nil end
    if n:find("adventurer", 1, true) then return "Adventurer " .. family end
    if n:find("veteran", 1, true) then return "Veteran " .. family end
    if n:find("champion", 1, true) then return "Champion " .. family end
    if n:find("hero", 1, true) then return "Hero " .. family end
    if n:find("myth", 1, true) or n:find("mythic", 1, true) then return "Myth " .. family end
    return nil
end
...
for _, def in ipairs(lv.GetActiveCrestDefinitions()) do ALWAYS_SHOW_ZERO[def.key] = true end
...
for _, def in ipairs(lv.GetActiveCrestDefinitions()) do
    ...
    if currencyID then MIDNIGHT_CREST_CURRENCY_IDS[def.key] = currencyID end
end

local UPGRADE_CREST_KEYS = {}
for _, def in ipairs(lv.GetActiveCrestDefinitions()) do UPGRADE_CREST_KEYS[def.key] = true end
...
if lv.GetActiveUpgradeSeasonKey() == "midnight_s2" then
    MIDNIGHT_DIRECT_CURRENCY_IDS["Corrosive Coin"] = 3448
end
...
local function RefreshActiveCrestCurrencyIDs()
    for _, def in ipairs(lv.GetActiveCrestDefinitions()) do
        MIDNIGHT_CREST_CURRENCY_IDS[def.key] = nil
        CURRENCY_ID_BY_KEYWORD[def.key] = nil
        local currencyID = lv.ResolveActiveCrestCurrencyID(def)
        if currencyID then
            MIDNIGHT_CREST_CURRENCY_IDS[def.key] = currencyID
            CURRENCY_ID_BY_KEYWORD[def.key] = currencyID
        end
    end
end
...
eventFrame:SetScript("OnEvent", function(self, event, arg1)
    ...
    elseif event == "PLAYER_ENTERING_WORLD" then
        C_Timer.After(1, lv.ScanCurrencies)
    elseif event == "CURRENCY_DISPLAY_UPDATE" or event == "ACCOUNT_CHARACTER_CURRENCY_DATA_RECEIVED" or event == "BAG_UPDATE" or event == "MAIL_CLOSED" or event == "BANKFRAME_CLOSED" then
        QueueCurrencyScan()
...
lv.EnsureBorderStyle(LVCurrencyWindow, "panelCompact")
...
C_Timer.After(0, function()
    if lv.RegisterThemedElement then
        ...
        lv.ApplyBorderStyle(LVCurrencyWindow, "panel", t)
    end
end)
    ...
    for i, keyword in ipairs(CURRENCY_DISPLAY_ORDER) do
        ...
        if keyword == CURRENCY_DIVIDER_DELVE or keyword == CURRENCY_DIVIDER_UPGRADE_CRESTS then
            ...
            if curData or ALWAYS_SHOW_ZERO[keyword] then
                ...
                local accountTotal = GetAccountCurrencyTotal(currencyID, amount)
                if accountTotal and accountTotal ~= amount then
                    row.divider:SetText("/")
                    row.divider:Show()
                    row.accountVal:SetText(BreakUpLargeNumbers(accountTotal))
                    row.accountVal:Show()
                else
                    row.divider:Hide()
