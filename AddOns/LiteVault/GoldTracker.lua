...
local function DetermineSource(nowTime, diff)
    ...
    if not claim and IsWarbandBankActive() then
        claim = BuildSourceClaim("warbandBank", "warband bank context active", 3, "warbandBankContext")
    end
...
function lv.GetProfitDailyStats(year, month, opts)
    ...
    for _, tx in ipairs(storage.transactions or {}) do
        if type(tx) == "table"
            ...
            and (not query.charKey or tx.charKey == query.charKey) then
            local charData = LiteVaultDB and tx.charKey and LiteVaultDB[tx.charKey] or nil
            if charData and charData.class then
...
function lv.GetProfitSources()
    local results = {}
    for index, source in ipairs(SOURCE_DEFS) do
        results[index] = ShallowCopy(source)
    end
    return results
end
