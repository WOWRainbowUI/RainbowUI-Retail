...
L.MSG_INVALID_SOUND_FILE			= "音效必須為 .mp3 或 .ogg 檔案。"
...
obj["language"] = { label = "語言", tooltip = "顯示當前遊戲語系，以及 MSBT 的語言選擇方式。" }
obj["profile"]	= { label = "設定檔", tooltip = "管理和切換設定檔" }
...
obj["disableOutgoingInGroup"] = { label = "在隊伍中停用輸出", tooltip = "當處於隊伍或團隊時，隱藏分配到『輸出』滾動區的事件。" }
obj["disableIncomingInGroup"] = { label = "在隊伍中停用輸入", tooltip = "當處於隊伍或團隊時，隱藏分配到『輸入』滾動區的事件。" }
obj["disableNotificationInGroup"] = { label = "在隊伍中停用通知", tooltip = "當處於隊伍或團隊時，隱藏分配到『通知』滾動區的事件。" }
obj["disableStaticInGroup"] = { label = "在隊伍中停用靜態", tooltip = "當處於隊伍或團隊時，隱藏分配到『靜態』滾動區的事件。" }
obj["enableBlizzardV2CombatText"] = { label = "單人時停用內建戰鬥文字", tooltip = "勾選後，單人時停用暴雪浮動戰鬥文字的傷害/治療顯示。" }
obj["enableBlizzardV2InGroup"] = { label = "隊伍中啟用內建戰鬥文字", tooltip = "僅在隊伍或團隊中啟用暴雪戰鬥文字。此設定會覆蓋『單人時停用暴雪戰鬥文字』。"}
...
obj["stackSimilarHits"] = { label = "合併相似傷害", tooltip = "在短時間內同一技能的重複傷害，會附加命中/爆擊次數（例如：3 次命中，1 次爆擊）。單次傷害則僅顯示數值。" }
