...
local PATTERN_FALLING = CreatePattern(COMBATLOG_ENVIRONMENTAL_DAMAGE_FALLING_SELF)
...
local function FormatEvent(message, amount, damageType, overhealAmount, overkillAmount, powerType, name, class, effectName, partialEffects, mergeTrailer, ignoreDamageColoring, hideSkills, hideNames)
	...
	if name == nil or IsSecret(name) or type(name) ~= "string" then
		name = ""
	end
	-- [[ END GUARD ]]
	...
	if effectName and effectName ~= "" then
		if string_find(message, "%s", 1, true) then
			if (hideSkills) then
				...
				local isChanged
				local success, sub = pcall(function() return currentProfile.abilitySubstitutions[effectName] end)
...
local function HandleComboPoints(amount, powerType)
	...
	if eventSettings.disabled or amount == 0 then
		return
	end
	if amount <= 0 then
		return
...
local function DamageHandler(parserEvent, currentProfile)
	...
	if parserEvent.damageType == DAMAGETYPE_PHYSICAL and parserEvent.recipientUnit == "player" then
...
local function HealHandler(parserEvent, currentProfile)
	...
	if parserEvent.sourceName == parserEvent.recipientName then eventTypeString = "SELF" end
...
local function DispelHandler(parserEvent, currentProfile)
	...
	if parserEvent.sourceUnit == "player" then eventTypeString = "OUTGOING_DISPEL"
	elseif parserEvent.sourceUnit == "pet" then eventTypeString = "PET_OUTGOING_DISPEL"
	else return end
...
local function ParserEventsHandler(parserEvent)
	...
	local skillID = parserEvent.skillID
	if skillID == SPELLID_AUTOSHOT then skillID = nil; effectName = nil end
	...
	if not currentProfile.skillIconsDisabled and IsScrollAreaIconShown(eventSettings.scrollArea) then
		...
		if not effectTexture and effectName then _, _, effectTexture = GetSpellInfo(effectName) end
	end
	...
	elseif currentProfile.mergeExclusions[effectName] or (not effectName and currentProfile.mergeSwingsDisabled) then
		...
		if effectName then
			...
			if not throttleDuration then
				if parserEvent.isDoT and currentProfile.dotThrottleDuration > 0 then throttleDuration = currentProfile.dotThrottleDuration
				elseif parserEvent.isHoT and currentProfile.hotThrottleDuration > 0 then throttleDuration = currentProfile.hotThrottleDuration
				elseif parserEvent.powerType and currentProfile.powerThrottleDuration > 0 then throttleDuration = currentProfile.powerThrottleDuration
				end
			end
	...
	if updateInfo.addedAuras then
		for _, auraData in pairs(updateInfo.addedAuras) do
			local success, name = pcall(function() 
				if auraData.name and auraData.name ~= "" then return auraData.name end
				return nil
			end)
			if success and name then
				activePlayerAuras[auraData.auraInstanceID] = {
					name = name, icon = auraData.icon, isHelpful = auraData.isHelpful, spellId = auraData.spellId
				}
				local auraType = auraData.isHelpful and "BUFF" or "DEBUFF"
				local eventSettings = currentProfile.events["NOTIFICATION_" .. auraType]
				if eventSettings and not eventSettings.disabled then
					local message = eventSettings.message
					local formattedName = string_format("|cFF%02x%02x%02x%s|r", (eventSettings.colorR or 1)*255, (eventSettings.colorG or 1)*255, (eventSettings.colorB or 0)*255, name)
	...
	if updateInfo.removedAuraInstanceIDs then
		for _, auraInstanceID in pairs(updateInfo.removedAuraInstanceIDs) do
			...
			if auraInfo then
				...
				local eventSettings = currentProfile.events["NOTIFICATION_" .. auraType .. "_FADE"]
				if eventSettings and not eventSettings.disabled then
					local message = eventSettings.message
					local formattedName = string_format("|cFF%02x%02x%02x%s|r", (eventSettings.colorR or 1)*255, (eventSettings.colorG or 1)*255, (eventSettings.colorB or 0)*255, auraInfo.name)
					message = string_gsub(message, "%%e", formattedName)
					message = string_gsub(message, "%%sl", formattedName)
...
function eventFrame:CHAT_MSG_COMBAT_XP_GAIN(message)
	local mobName, amount = string_match(message, "(.*) dies, you gain (%d+) experience")
	if mobName then
		local kbSettings = MSBTProfiles.currentProfile.events.NOTIFICATION_NPC_KILLING_BLOW
		if not kbSettings.disabled then
			local output = FormatEvent(kbSettings.message, nil, nil, nil, nil, nil, mobName, nil, nil)
			DisplayEvent(kbSettings, output)
		end
	end
	if not amount then amount = string_match(message, "gain (%d+)") end
	if amount then
		local xpSettings = MSBTProfiles.currentProfile.events.NOTIFICATION_EXPERIENCE_GAIN
...
function eventFrame:CHAT_MSG_COMBAT_FACTION_CHANGE(message)
	...
	if faction and amount then
		local eventSettings = MSBTProfiles.currentProfile.events.NOTIFICATION_REP_GAIN
		if not eventSettings.disabled then
			local output = FormatEvent(eventSettings.message, tonumber(amount), nil, nil, nil, nil, nil, nil, faction)
	...
	if faction and amount then
		local eventSettings = MSBTProfiles.currentProfile.events.NOTIFICATION_REP_LOSS
		if not eventSettings.disabled then
			local output = FormatEvent(eventSettings.message, tonumber(amount), nil, nil, nil, nil, nil, nil, faction)
			DisplayEvent(eventSettings, output)
		end
...
function eventFrame:UNIT_SPELLCAST_INTERRUPTED(unit, castGUID, spellID)
	if unit == "target" or unit == "focus" then
		...
		local eventSettings = MSBTProfiles.currentProfile.events.OUTGOING_SPELL_INTERRUPT
		if not eventSettings.disabled then
			local output = FormatEvent(eventSettings.message, nil, nil, nil, nil, nil, targetName, nil, targetName)
			DisplayEvent(eventSettings, output)
		end
	end
end
	...
	if unit == "player" then
		if action == "WOUND" and (descriptor == nil or descriptor == "") then
			if (GetTime() - lastFallTime) < 0.5 then
				...
				if dmg then
					local envSettings = MSBTProfiles.currentProfile.events.INCOMING_ENVIRONMENTAL
					if not envSettings.disabled then
						local output = FormatEvent(envSettings.message, dmg, nil, nil, nil, nil, nil, nil, "Falling")
...
local function Show12_0_1Notice()
	C_Timer.After(5, function()
		MikSBT.Print("|cFFFFD700MSBT 12.0.1 Notice:|r Buff/debuff notifications are unavailable in combat due to Blizzard API restrictions. All damage, healing, and other combat text features are working normally.", 1, 0.82, 0)
	end)
end
