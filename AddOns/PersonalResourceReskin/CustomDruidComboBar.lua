...
local function ApplyComboBarSettings()
    ...
    dprint("套用設定:", "anchorToPRD=", CustomDruidComboBarDB.anchorToPRD, "anchorFrame=", anchorFrame and "yes" or "no")

    if anchorFrame then
        if CustomDruidComboBarDB.anchorPosition == "ABOVE" then
            dprint("對齊到個人資源條上方，偏移量為", CustomDruidComboBarDB.anchorOffset or 10)
            comboPointBar:SetPoint("BOTTOM", anchorFrame, "TOP", 0, CustomDruidComboBarDB.anchorOffset or 10)
        else
            dprint("對齊到個人資源條下方，偏移量為", CustomDruidComboBarDB.anchorOffset or 10)
            comboPointBar:SetPoint("TOP", anchorFrame, "BOTTOM", 0, -(CustomDruidComboBarDB.anchorOffset or 10))
        end
    else
        dprint("置中位置 x=", CustomDruidComboBarDB.x or 0, "y=", CustomDruidComboBarDB.y or 0)
...
comboPointBar:SetScript("OnDragStop", function(self)
    ...
    dprint("拖曳停止：已更新 x,y:", CustomDruidComboBarDB.x, CustomDruidComboBarDB.y)
end)
