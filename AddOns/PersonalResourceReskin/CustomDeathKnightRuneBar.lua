...
local function ApplyBarSettings()
    ...
    if CustomDeathKnightRuneBarDB.anchorToPRD then
        ...
        if anchorFrame then
            if CustomDeathKnightRuneBarDB.anchorPosition == "ABOVE" then
                ...
                if CustomDeathKnightRuneBarDB.debug then print("[DKRuneBar] 對齊到個人資源條上方，偏移量", CustomDeathKnightRuneBarDB.anchorOffset or 10) end
            else
                runeBar:SetPoint("TOP", anchorFrame, "BOTTOM", 0, -(CustomDeathKnightRuneBarDB.anchorOffset or 10))
                if CustomDeathKnightRuneBarDB.debug then print("[DKRuneBar] 對齊到個人資源條下方，偏移量", CustomDeathKnightRuneBarDB.anchorOffset or 10) end
            end
        else
            runeBar:SetPoint("CENTER", UIParent, "CENTER", CustomDeathKnightRuneBarDB.x, CustomDeathKnightRuneBarDB.y)
            if CustomDeathKnightRuneBarDB.debug then print("[DKRuneBar] 找不到個人資源條對齊點，預設為 UIParent") end
        end
    else
        runeBar:SetPoint("CENTER", UIParent, "CENTER", CustomDeathKnightRuneBarDB.x, CustomDeathKnightRuneBarDB.y)
    end
    ...
    if CustomDeathKnightRuneBarDB.debug then print("[DKRuneBar] ApplyBarSettings", "寬度:", runeWidth, "高度:", runeHeight, "總寬度:", totalWidth, "鎖定:", CustomDeathKnightRuneBarDB.locked) end
end
...
SlashCmdList["CUSTOMDKRUNEBAR"] = function(msg)
    ...
    if cmd == "lock" then
        ...
        ApplyBarSettings()
        print("CustomDeathKnightRuneBar 已鎖定。")
    elseif cmd == "unlock" then
        CustomDeathKnightRuneBarDB.locked = false
        ApplyBarSettings()
        print("CustomDeathKnightRuneBar 已解鎖。拖曳以移動。")
    elseif cmd == "size" and tonumber(arg1) and tonumber(arg2) then
        ...
        ApplyBarSettings()
        print("CustomDeathKnightRuneBar 符文尺寸設定為 "..arg1.."x"..arg2)
    elseif cmd == "debug" then
        CustomDeathKnightRuneBarDB.debug = not CustomDeathKnightRuneBarDB.debug
        print("CustomDeathKnightRuneBar 偵錯模式：", CustomDeathKnightRuneBarDB.debug)
    else
...
if PersonalResourceReskinPlus_Options then
    PersonalResourceReskinPlus_Options.RegisterSubOptions("CustomDeathKnightRuneBar", {
        name = "|cffff2020自訂死亡騎士符文條|r",
        type = "group",
        args = {
            enableCustomRuneBar = {
                name = "啟用自訂符文條",
                desc = "顯示自訂符文條。停用此選項會改用預設的個人資源條符文。",
                type = "toggle",
            ...
            runeWidth = {
                name = "符文寬度 (每個)",
                desc = "設定每個符文的寬度。如果有設定 '符文條總寬度'，此設定將被忽略。",
                type = "range",
            ...
            totalWidth = {
                name = "符文條總寬度",
                desc = "設定所有符文的總寬度。如果有設定，符文將自動調整以符合此寬度。",
                type = "range",
            ...
            locked = {
                ...
                set = function(_, val)
                    ...
                    ApplyBarSettings()
                    if CustomDeathKnightRuneBarDB.debug then print("[DKRuneBar] 鎖定狀態設定為", val) end
                end,
                order = 11,
            },
            debug = {
                ...
                set = function(_, val)
                    ...
                    if val then print("[DKRuneBar] 偵錯模式已啟用") else print("[DKRuneBar] 偵錯模式已停用") end
                end,
                order = 12,
            },
        },
