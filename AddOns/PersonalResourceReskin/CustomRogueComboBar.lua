-- Register options subpage with AceConfig/AceConfigDialog
-- Register as a child tab under main options, matching WarriorTracker
local function RegisterCustomRogueComboBarOptions()
    local _, class = UnitClass("player")
    if class ~= "ROGUE" then return end
    if PersonalResourceReskinPlus_Options and PersonalResourceReskinPlus_Options.RegisterSubOptions then
        PersonalResourceReskinPlus_Options.RegisterSubOptions("CustomRogueComboBar", CustomRogueComboBarOptions)
    end
end
...
regFrame:SetScript("OnEvent", function(self)
    ...
    RegisterCustomRogueComboBarOptions()
end)
-- CustomRogueComboBar.lua
...
local COMBO_POINT_TYPE = Enum and Enum.PowerType and Enum.PowerType.ComboPoints or 4

local chargedComboPoints = {}

local function dprint(...)
    if CustomRogueComboBarDB and CustomRogueComboBarDB.debug then
        print("[CRCB]", ...)
    end
end
...
local function GetPRDAnchorFrame()
    if not CustomRogueComboBarDB.anchorToPRD then return nil end
    local target = CustomRogueComboBarDB.anchorTarget or "HEALTH"
...
local comboPointBar = CreateFrame("Frame", "CustomRogueComboBar", UIParent)
comboPointBar.comboPoints = {}

function UpdateRogueComboBarEnabled()
    if CustomRogueComboBarDB.enabled then
        HideDefaultComboBar()
        comboPointBar:Show()
    else
        ShowDefaultComboBar()
...
local function GetComboPointWidth(numPoints)
    ...
    if CustomRogueComboBarDB.totalWidth and CustomRogueComboBarDB.totalWidth > 0 then
        return CustomRogueComboBarDB.totalWidth / numPoints
    end
    return CustomRogueComboBarDB.comboPointWidth or 24
end
...
local function CreateComboPoints()
    ...
    local height = CustomRogueComboBarDB.comboPointHeight or 24
...
local function UpdateComboPointBarSize()
    ...
    local width
    if CustomRogueComboBarDB.totalWidth and CustomRogueComboBarDB.totalWidth > 0 then
        width = CustomRogueComboBarDB.totalWidth
    else
        width = (CustomRogueComboBarDB.comboPointWidth or 24) * numPoints
    end
    comboPointBar:SetSize(width, CustomRogueComboBarDB.comboPointHeight or 24)
end
...
local function ApplyComboBarSettings()
    ...
    dprint("Apply settings:", "anchorToPRD=", CustomRogueComboBarDB.anchorToPRD, "anchorFrame=", anchorFrame and "yes" or "no")

    if anchorFrame then
        local pos = CustomRogueComboBarDB.anchorPosition or "BELOW"
        local offset = CustomRogueComboBarDB.anchorOffset or 10
        if pos == "ABOVE" then
            comboPointBar:SetPoint("BOTTOM", anchorFrame, "TOP", 0, offset)
        else
            comboPointBar:SetPoint("TOP", anchorFrame, "BOTTOM", 0, -offset)
        end
    else
        comboPointBar:SetPoint("CENTER", UIParent, "CENTER", CustomRogueComboBarDB.x or 0, CustomRogueComboBarDB.y or 0)
    end
end
...
local function UpdateComboPoints()
    ...
    local width = GetComboPointWidth(numPoints)
    local height = CustomRogueComboBarDB.comboPointHeight or 24
    local bgR, bgG, bgB, bgA = SafeUnpackColor(CustomRogueComboBarDB.comboPointBgColor, {0, 0, 0, 0.5})
    ...
    for i = 1, numPoints do
        ...
        if fillAmount > 0 then
            ...
            local color
            if chargedComboPoints[i] then
                color = CustomRogueComboBarDB.chargedComboPointColor or {0.2, 0.6, 1, 1}
            elseif CustomRogueComboBarDB.gradientColoringEnabled then
                local t = (numPoints == 1) and 0 or (i-1)/(numPoints-1)
                color = {LerpColor(CustomRogueComboBarDB.gradientColorStart, CustomRogueComboBarDB.gradientColorEnd, t)}
            else
                color = CustomRogueComboBarDB.comboPointFillColors[i] or {1, 0.7, 0.2, 1}
            end
...
local function HookPRDAnchor()
    ...
    anchorFrame._crcb_hooks = anchorFrame._crcb_hooks or {}

    if not anchorFrame._crcb_hooks.SetPoint then
        anchorFrame._crcb_hooks.SetPoint = true
        hooksecurefunc(anchorFrame, "SetPoint", function()
            if CustomRogueComboBarDB.anchorToPRD then ApplyComboBarSettings() end
        end)
    end
    if not anchorFrame._crcb_hooks.SetSize then
        anchorFrame._crcb_hooks.SetSize = true
        hooksecurefunc(anchorFrame, "SetSize", function()
            if CustomRogueComboBarDB.anchorToPRD then ApplyComboBarSettings() end
        end)
    end
    if not anchorFrame._crcb_hooks.SetScale then
        anchorFrame._crcb_hooks.SetScale = true
...
comboPointBar:SetScript("OnDragStart", function(self)
    if not CustomRogueComboBarDB.locked and not CustomRogueComboBarDB.anchorToPRD then self:StartMoving() end
end)
comboPointBar:SetScript("OnDragStop", function(self)
    self:StopMovingOrSizing()
    local point, _, _, x, y = self:GetPoint()
    CustomRogueComboBarDB.x = x or 0
    CustomRogueComboBarDB.y = y or 0
    dprint("DragStop: updated x,y:", CustomRogueComboBarDB.x, CustomRogueComboBarDB.y)
end)

local function UpdateVisibility()
    if ShouldShowComboPointBar() and CustomRogueComboBarDB.enabled then
        comboPointBar:Show()
    else
        comboPointBar:Hide()
    end
...
local function UpdateRogueComboBarSettings()
    ApplyComboBarSettings()
    UpdateComboPoints()
    if CustomRogueComboBarDB.anchorToPRD then
        TryHookPRDAnchor()
    end
end

_G.UpdateComboPoints = UpdateComboPoints
_G.UpdateRogueComboBarSettings = UpdateRogueComboBarSettings
