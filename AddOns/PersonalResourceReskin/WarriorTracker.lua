...
function Whirlwind:GetStacks()
    ...
    -- Always return max as 4; stacks as integer 0-4
    return self.IW_MAX_STACKS, iwStacks or 0
end
...
local function UpdateTracker()
    ...
    local fill = math.max(0, math.min(numSegments, stacks or 0))
    bar:SetValue(fill)
    bar:Show()
    UpdateTicks(numSegments)
    WarriorTracker:Show()
    WarriorTracker:SetScript("OnUpdate", OnUpdateThrottled)
end
...
WarriorTracker:SetScript("OnEvent", function(self, event, ...)
    ...
    -- Forward spellcast and relevant events to Whirlwind logic
    if event == "UNIT_SPELLCAST_SUCCEEDED" or event == "PLAYER_DEAD" or event == "PLAYER_ALIVE" then
        Whirlwind:OnEvent(self, event, ...)
    end
    UpdateTracker()
end)
...
C_Timer.After(1, UpdateTracker) -- Ensure bar state is correct after options/UI load
