...
local difficultyInfo = {
    [1] = {name = "普通地城", expanded = false},
    [2] = { name = "英雄地城", expanded = false},
    [3] = {name = "普通 (10人) 團隊副本", expanded = false},
    [4] = {name = "普通 (25人) 團隊副本", expanded = false},
    [5] = {name = "英雄 (10人) 團隊副本", expanded = false},
    [6] = {name = "英雄 (25人) 團隊副本", expanded = false},
    [14] = {name = "普通團隊副本", expanded = false},
    [15] = {name = "英雄團隊副本", expanded = false},
    [16] = {name = "傳奇團隊副本", expanded = false},
    [17] = {name = "隨機團隊副本", expanded = false},
    [23] = {name = "傳奇地城", expanded = false},
...
local LDB = LibStub("LibDataBroker-1.1"):NewDataObject("BountyHelper",{
  ...
  OnTooltipShow = function(tooltip)
    tooltip:AddLine("|cffffffff賞金小幫手")
    tooltip:AddLine(COLORS.GREEN .. "<點一下顯示主視窗>")
...
function MyMountTracker:CreateUI()
    ...
    title:SetText(COLORS.GOLD .. "賞金小幫手")
    ...
    toMountViewButton:SetText("依坐騎排序")
    ...
    toDifficultyViewButton:SetText("依難度排序")
    ...
    scaleSlider:SetScript("OnValueChanged", function(self, value)
        ...
        MyMountTracker.Frames.ScaleValueText:SetText(string.format("介面縮放大小: %.2f", roundedValue))
    end)
    ...
    hideButtonCheckbox.text:SetText("隱藏按鈕")
    ...
    hideOwnedCheckbox.text:SetText("隱藏已有的")
    ...
    hideKilledCheckbox.text:SetText("隱藏已擊殺")
    ...
    lockCheckbox.text:SetText("停用按 Esc 關閉視窗")
...
function MyMountTracker:CreateBossRow(parent, bossData, header, instanceID)
    ...
    local data = bossData
    local mapName = instanceToMap[instanceID].mapName or "未知的地圖"
    ...
    chanceText:SetFormattedText( "機率: %s%s|r -> %s%.2f%%|r", COLORS.GREY, (data.chance == 0) and "未知" or string.format("%.2f%%", data.chance), COLORS.GREEN, data.chance + 5.0)
...
function MyMountTracker:UpdateLayout()
    ...
    for _, frame in ipairs(MyMountTracker.ContentFrames) do
        ...
        if frame.isHeader then
            ...
            if isOwned then
                frame.statusText:SetText(COLORS.GOLD .. "已收藏")
            else
                frame.statusText:SetText(data.killed and (COLORS.GREEN .. "已擊殺") or (COLORS.RED .. "未擊殺"))
            end
        end
...
function MyMountTracker:LoadMountData(onCompleteCallback)
    ...
    for _, mountID in ipairs(uniqueMountIDs) do
        ...
        item:ContinueOnItemLoad(function()
            self.cachedMountNames[mountID] = item:GetItemName() or ("未知坐騎: " .. mountID)
    ...
    for _, mountID in ipairs(sortedKeys) do
        ...
        if mountGroup.sources and #mountGroup.sources > 0 then
            ...
            headerRow:SetScript("OnEnter", function(self)
                ...
                GameTooltip:SetOwner(self, "ANCHOR_CURSOR");
                GameTooltip:SetText(COLORS.GREEN .. "<點一下在地圖上放置標記>");
                GameTooltip:Show()
            end)
            ...
            headerRow:SetScript("OnClick", function(self, button)
                ...
                elseif waypoints[instanceID] then
                    ...
                    print(string.format("%s賞金小幫手:|r 已設定導航到 %s", COLORS.GOLD, mountLink))
                else
                    print(string.format("%s賞金小幫手:|r 沒有 %s 的位置資料", COLORS.GOLD, mountLink))
                end
            end)
        end
...
function MyMountTracker:CreateMountHeaderRow(parent, mountID, journalMountID, mountName, chance)
    ...
    chanceText:SetFont(STANDARD_TEXT_FONT, 14)
    chanceText:SetJustifyH("RIGHT")
    chanceText:SetFormattedText( "機率: %s%s|r -> %s%.2f%%|r", COLORS.GREY, (chance == 0) and "未知" or string.format("%.2f%%", chance), COLORS.GREEN, chance + 5.0)
...
function MyMountTracker:CreateMountSourceRow(parent, sourceData)
    ...
    local mapName = instanceToMap[sourceData.instanceID].mapName or "未知的地圖"
    ...
    local shorterText
    if data.journalMountID == 69 then shorterText = "普通地城" end
    difficultyText:SetText(COLORS.GREY .. (shorterText or difficultyInfo[sourceData.difficultyID].name or "未知難度"))
    ...
    if sourceData.difficultyID == 17 then
        ...
        arrow:SetScript("OnClick", function()
            ...
            print(string.format("%s賞金小幫手:|r 已設定導航", COLORS.GOLD))
        end)
    end
    ...
    for _, frame in ipairs(MyMountTracker.MountViewContentFrames) do
        if frame.isMountHeader then
            ...
            for _, sourceRow in ipairs(frame.sourceRows) do
                ...
                if isOwned then
                    sourceRow.statusText:SetText(COLORS.GOLD .. "已收藏")
                else
                    sourceRow.statusText:SetText(data.killed and (COLORS.GREEN .. "已擊殺") or (COLORS.RED .. "未擊殺"))
                end
...
eventHandlerFrame:SetScript("OnEvent", function(self, event, ...)
    ...
    elseif event == "FIRST_FRAME_RENDERED" then
        ...
        MyMountTracker.Frames.ScaleValueText:SetText(string.format("介面大小: %.2f", currentScale))
