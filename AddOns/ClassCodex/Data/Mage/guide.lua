ClassCodexData = ClassCodexData or {}
ClassCodexData["MAGE"] = {
  ["arcane"] = {
    label = "Arcane Mage",
    priorities = {
      {
        heroTalent = "Spellslinger",
        context = "通用",
        stats = {
          { "精通" },
          { "加速" },
          { "致命一擊" },
          { "臨機應變" },
        },
      },
      {
        heroTalent = "Sunfury",
        context = "通用",
        stats = {
          { "精通" },
          { "加速" },
          { "致命一擊" },
          { "臨機應變" },
        },
      },
    },
    talents = {
      {
        heroTalent = "Spellslinger",
        context = "團隊單目標",
        buildLabel = "Orb Master",
        exportString = "C4DAAAAAAAAAAAAAAAAAAAAAAYGGLzMzswMzQzMzAAAwAAgAmZmZZZmZYBAgtxMzMmtFLzMzYmxYMzMGLMzMjZAAGAAAzsAAmBADD",
      },
      {
        heroTalent = "Spellslinger",
        context = "傳奇+",
        buildLabel = "Orb Master",
        exportString = "C4DAAAAAAAAAAAAAAAAAAAAAAMzwYZmZmFmZGamxAAAwAAgAmZmZZZmJWAAYbGzMjZzsMzMjZGjhZmZswMzMmBAYAAAMzCAYGAMM",
      },
      {
        heroTalent = "Spellslinger",
        context = "探究",
        buildLabel = "Orb Master",
        exportString = "C4DAAAAAAAAAAAAAAAAAAAAAAMzwYZmZmFmZGamxAAAwAAgAmZmZZZmJWAAYbGzMjZzsMzMjZGjhZmZswMzMmBAYAAAMzCAYGAMM",
      },
      {
        heroTalent = "Sunfury",
        context = "團隊單目標",
        exportString = "C4DAAAAAAAAAAAAAAAAAAAAAAYGGLzMzswMzQzMzAAAwAAmZmmlltZAgYDAgNmZmxYzyMzMLzMGjZmxYhZmZMDAwAAAMAzMgZAwwA",
      },
      {
        heroTalent = "Sunfury",
        context = "傳奇+",
        exportString = "C4DAAAAAAAAAAAAAAAAAAAAAAMzwMLzMzsgZGamxAAAwAAmZmmlttZAgYDAgNDzMDbzyMzMLzwYGzMDLMzMzDMDAwAAAMAzMgZAwwA",
      },
      {
        heroTalent = "Sunfury",
        context = "探究",
        exportString = "C4DAAAAAAAAAAAAAAAAAAAAAAMzwMLzMzsgZGamxAAAwAAmZmmlttZAgYDAgNDzMDbzyMzMLzwYGzMDLMzMzDMDAwAAAMAzMgZAwwA",
      },
    },
    rotation = {
      {
        heroTalent = "All",
        context = "簡易模式",
        steps = {
          "{365350}",
          "{321507}",
          "?(H\"Spellslinger Missile Build\"|H\"Spellslinger Orb Build\"): {44425} when you have max {384452} stacks; 20 for Spellslinger",
          "?(H\"Sunfury\"): {44425} when you have max {384452} stacks; 25 for Sunfury",
          "?(H\"Sunfury\"): {44425} if you have {263725}或{153626} in AOE",
          "?(H\"Sunfury\"|H\"Spellslinger Missile Build\"): {5143} if you have {263725}",
          "?(H\"Sunfury\"|H\"Spellslinger Missile Build\"): {153626} if you have less than 2 {36032}s",
          "?(H\"Spellslinger Orb Build\"): {153626} if you just cast {44425}",
          "{1243460} when you have 3或more targets.",
          "{30451}",
          "{44425} if you run out of mana",
        },
      },
      {
        heroTalent = "Spellslinger Orb Build",
        context = "技能循環",
        steps = {
          "{153626} whenever you have just cast {44425} and have {263725}.",
          "{44425} with 4x {36032}s if you have 20 stacks of {384452}.",
          "?(H\"Presence of Mind\"): Use {205025} to cast {30451} quicker when you have no {36032}s或better generators.",
          "?(H\"Arcane Pulse (M+)\"): {1243460} if you have 3或more targets或if you have 2或fewer {36032}s.",
          "{30451}.",
          "?(H\"Evocation\"): {12051} if you do not have enough mana for {30451}.",
          "{44425} if you don't have enough mana for {30451}.",
        },
      },
      {
        heroTalent = "Spellslinger Missile Build",
        context = "技能循環",
        steps = {
          "{44425} with 4x {36032}s if you have 20 stacks of {384452}.",
          "{44425} with 4x {36032}s if you have 5或more {384452}, {263725}, and {1277009}.",
          "{5143} whenever less than 10 stacks of {384452}或less than 3x {36032}s.",
          "{153626} whenever you have less than 4x {36032}s.",
          "?(H\"Presence of Mind\"): Use {205025} to cast {30451} quicker when you have no {36032}s.",
          "?(H\"Arcane Pulse\"): {1243460} if you have 2或more targets.",
          "{30451}.",
          "?(H\"Evocation\"): {12051} if you do not have enough mana for {30451}.",
          "{44425} if you don't have enough mana for {30451}.",
        },
      },
      {
        heroTalent = "Sunfury",
        context = "技能循環",
        steps = {
          "{44425} if {451038} is active.",
          "{5143} if you have {263725} and {365350} is about to end.",
          "{44425} with 4x {36032}s if you have just cast {321507}或it is about to end.",
          "{44425} with 4x {36032}s if you have 25 stacks of {384452}.",
          "{44425} with 4x {36032}s, less than 19 stacks of {384452}, and a {263725} proc or, if you have also have 3或more enemies, you can do this with an available {153626} charge.",
          "{5143} whenever you have {263725}.",
          "{153626} whenever you have less than 2x {36032}s.",
          "?(H\"Presence of Mind\"): Use {205025} to cast {30451} quicker when you have no {36032}s.",
          "?(H\"Arcane Pulse 2+ Targets\"): {1243460} if you have 2或more targets.",
          "{30451}.",
          "?(H\"Evocation\"): {12051} if you do not have enough mana for {30451}.",
          "{44425} if you don't have enough mana for {30451}.",
        },
      },
      {
        heroTalent = "Spellslinger Missile Build",
        context = "起手 / 冷卻",
        steps = {
          "?(H\"Resource Pooling\"&H\"Arcane Pulse (3+ targets)\"): {1243460} 2 seconds before pulling.",
          "?(H\"Resource Pooling\"&!H\"Arcane Pulse (3+ targets)\"): {30451} 2 seconds before pulling.",
          "?(H\"Resource Pooling\"): {153626} and {80353}.",
          "?(H\"Resource Pooling\"&H\"Arcane Pulse (3+ targets)\"): {1243460} until you have 20 stacks of {384452}.",
          "?(H\"Resource Pooling\"&!H\"Arcane Pulse (3+ targets)\"): {30451} until you have 20 stacks of {384452}.",
          "{365350}.",
          "?(!H\"Resource Pooling\"): {80353}.",
          "{44425} and then {321507} while it is mid-air.",
          "45-seconds later, when {321507}s cooldown finishes, repeat the last step.",
        },
      },
      {
        heroTalent = "Spellslinger Orb Build",
        context = "起手 / 冷卻",
        steps = {
          "?(H\"Resource Pooling\"&H\"Arcane Pulse (3+ targets)\"): {1243460} 2 seconds before pulling.",
          "?(H\"Resource Pooling\"&!H\"Arcane Pulse (3+ targets)\"): {30451} 2 seconds before pulling.",
          "?(H\"Resource Pooling\"): {153626} and {80353}.",
          "?(H\"Resource Pooling\"&H\"Arcane Pulse (3+ targets)\"): {1243460} until you have 20 stacks of {384452}.",
          "?(H\"Resource Pooling\"&!H\"Arcane Pulse (3+ targets)\"): {30451} until you have 20 stacks of {384452}.",
          "{365350}.",
          "?(!H\"Resource Pooling\"): {80353}.",
          "{44425} and then {321507} while it is mid-air.",
          "45-seconds later, when {321507}s cooldown finishes, repeat the last step.",
        },
      },
      {
        heroTalent = "Sunfury",
        context = "起手 / 冷卻",
        steps = {
          "Use any potions, racials, items,或trinkets that buff your stats so that they will last through {321507}. As examples; if its a 30-second effect, you can use it before {365350}; if its an 8-second effect you should use it when {321507} has around 8 seconds remaining; and a 15-second effect would be best used 3 seconds before using {321507}.",
          "{365350}, this can be your prepull cast.",
          "{80353} and do follow the rotation until it's time to touch, but remember to hold 25 {384452} stacks for Touch.",
          "{321507} when {365350} has 4-6 seconds remaining.",
          "45-seconds later, when {321507}s cooldown finishes, use it!",
        },
      },
    },
  },
  ["fire"] = {
    label = "Fire Mage",
    priorities = {
      {
        heroTalent = "Sunfury",
        context = "通用",
        stats = {
          { "加速" },
          { "精通" },
          { "臨機應變" },
          { "致命一擊" },
        },
      },
      {
        heroTalent = "Frostfire",
        context = "通用",
        stats = {
          { "加速" },
          { "精通" },
          { "臨機應變" },
          { "致命一擊" },
        },
      },
    },
    talents = {
      {
        heroTalent = "Sunfury",
        context = "團隊單目標",
        recommended = true,
        exportString = "C8DAAAAAAAAAAAAAAAAAAAAAAYGGLzMzswMDZmZGAAAGAwMz0sssMDAwmZmx2YmZGAAAAAgFzMzMDAAGzYGzMzMzGAmZIjxYwMMA",
      },
      {
        heroTalent = "Frostfire",
        context = "團隊單目標",
        exportString = "C8DAAAAAAAAAAAAAAAAAAAAAAYGGLzMzswMDZmZGAAAmZZGzMLzCEAAwiZmx2YmZGAAAAAgFzMzMDAAGzYmZmZmZ2AmZADzYMYwA",
      },
      {
        heroTalent = "All",
        context = "團隊單目標",
        buildLabel = "Cinderstorm 單目標",
        exportString = "C8DAAAAAAAAAAAAAAAAAAAAAAYGGLzMzswMDZmZGAAAGAwMz0sssMDAwmZmZ2GGzMAAAAAgFzMzMDAAGmZGzMMz2AwMDZMGGmhB",
      },
      {
        heroTalent = "Frostfire",
        context = "傳奇+",
        exportString = "C8DAAAAAAAAAAAAAAAAAAAAAAMzwMLzMzsgZGZmxAAAwMLzYmZZWgAAAWMzM2GzMzYBAAAAAWMzMzMAAYMjZmZmZmZDYmBMYMGMYA",
      },
      {
        heroTalent = "Sunfury",
        context = "傳奇+",
        exportString = "C8DAAAAAAAAAAAAAAAAAAAAAAMzwMLzMzsgZGZmxAAAwAAmZmmlltZAA2MzM2GzMzYDAAAAAWMzMzMAAYMDjZmZmZbAYmhwYMYGG",
      },
      {
        heroTalent = "Frostfire",
        context = "探究",
        recommended = true,
        exportString = "C8DAAAAAAAAAAAAAAAAAAAAAAMzwMLzMzsgZGZmxAAAwMLzYmZZWgAAAWMzM2GzMzYBAAAAA2MzMzMAAYMjZmZmZmZDYmBMYMGMYA",
      },
      {
        heroTalent = "Sunfury",
        context = "探究",
        exportString = "C8DAAAAAAAAAAAAAAAAAAAAAAMzwMLzMzsgZGZmxAAAwAAmZmmlltZAA2MzM2GzMzYBAAAAA2MzMzMAAYMDjZmZmZbAYmhwYMYGG",
      },
    },
    rotation = {
      {
        heroTalent = "Sunfury",
        context = "單目標",
        steps = {
          "{153561} (if it will land during {190319}) if <2 {269650} stacks.",
          "{153561} (if it will land during {190319})",
          "?((H\"Meteor\")&(H\"Burnout\")): {153561} in the last 8 seconds of {190319}.",
          "{11366} with {48108}",
          "?((H\"Scorch\")): {11366} again if you just spent {48108} at the end of a {2948} cast.",
          "?(!(H\"Scorch\")): {11366} again if you just spent {48108} at the end of a {133} cast.",
          "Hard-cast {11366} with {269650}.",
          "The below casts are used to generate {48108}:",
          "{108853}.",
          "{2948} with {457735} and {48107}.",
          "{2948} as filler.",
          "?(!(H\"Scorch\")): {133} as filler.",
        },
      },
      {
        heroTalent = "Frostfire",
        context = "單目標",
        steps = {
          "?((H\"Burnout\")): {153561} in the last 8 seconds of {190319}.",
          "?(!(H\"Burnout\")): {153561} if it will hit during {190319}.",
          "{11366} with {48108}",
          "{11366} again if you just spent {48108} at the end of a {2948} cast.",
          "{11366} again if you just spent {48108} at the end of a {431044} cast.",
          "Hard-cast {11366} with {269650}.",
          "The below casts are used to generate {48108}:",
          "{108853}.",
          "{2948} with {457735} and {48107}.",
          "{2948} as filler.",
          "{431044} as filler.",
        },
      },
      {
        heroTalent = "Sunfury",
        context = "多目標",
        steps = {
          "{153561} (if it will land during {190319}) if <2 {269650} stacks.",
          "{153561} (if it will land during {190319})",
          "{153561} in the last 8 seconds of {190319}.",
          "{2120} with {48108}",
          "?((H\"Scorch\")): {2120} again if you just spent {48108} at the end of a {2948} cast.",
          "?(!(H\"Scorch\")): {2120} again if you just spent {48108} at the end of a {133} cast.",
          "Hard-cast {2120} with {269650}.",
          "The below casts are used to generate {48108}:",
          "{108853}.",
          "{2948} with {457735} and {48107}.",
          "{2948} as filler.",
          "?(!(H\"Scorch\")): {133} as filler.",
        },
      },
      {
        heroTalent = "Frostfire",
        context = "多目標",
        steps = {
          "{153561} if it will hit during {190319}.",
          "{2120} with {48108}",
          "{2120} again if you just spent {48108} at the end of a {2948} cast.",
          "{2120} again if you just spent {48108} at the end of a {431044} cast.",
          "Hard-cast {2120} with {269650}.",
          "The below casts are used to generate {48108}:",
          "{108853}.",
          "{2948} with {457735} and {48107}.",
          "{2948} as filler.",
          "{431044} as filler.",
        },
      },
      {
        heroTalent = "Sunfury",
        context = "起手",
        steps = {
          "Pre-cast {11366} before the pull (hard-cast).",
          "{133}.",
          "{108853} during the {133} cast.",
          "{11366} with {48108} twice",
          "Alternate between {108853} and {11366} until you run out of charges.",
          "If the target is still above 90% health, repeat the following: {133}",
          "{108853} during the {133} cast.",
          "Without {48108}, {133} again.",
          "{11366} with {48108} twice",
        },
      },
      {
        heroTalent = "Frostfire",
        context = "起手",
        steps = {
          "Pre-cast {11366} before the pull (hard-cast).",
          "{431044}.",
          "{108853} during the {431044} cast.",
          "{11366} with {48108} twice",
          "Alternate between {108853} and {11366} until you run out of charges.",
          "Follow this priority until the target approaches 90% health: {11366} with {48108}.",
          "{108853} with {48107}.",
          "{431044}",
        },
      },
      {
        heroTalent = "All",
        context = "Firestarter Examples",
        steps = {
          "{133}",
          "{108853} (if available)或{133}",
          "Queue {11366} as the {133} ends.",
          "{11366} again.",
        },
      },
    },
  },
  ["frost"] = {
    label = "Frost Mage",
    priorities = {
      {
        heroTalent = "Frostfire",
        context = "通用",
        stats = {
          { "精通" },
          { "致命一擊" },
          { "加速" },
          { "臨機應變" },
        },
      },
      {
        heroTalent = "Spellslinger",
        context = "通用",
        stats = {
          { "精通" },
          { "致命一擊" },
          { "加速" },
          { "臨機應變" },
        },
      },
    },
    talents = {
      {
        heroTalent = "Spellslinger",
        context = "團隊單目標",
        recommended = true,
        exportString = "CAEAAAAAAAAAAAAAAAAAAAAAAYGGLzMzswMzEzMzMzMzMziZmZMjZAAAgZmZWWmZaDAAAAAAsBw22YmZGMbDjZYDAAgZ2AmJjBMDGA",
      },
      {
        heroTalent = "Spellslinger",
        context = "傳奇+",
        recommended = true,
        exportString = "CAEAAAAAAAAAAAAAAAAAAAAAAMzwYZmZmFmZmYGmZmZmZWMzMMjZAAAgZmZWWmZaDAAWAAAAWAYbbMzMDmthxMjNAAAmZDYmMGwMYA",
      },
      {
        heroTalent = "Spellslinger",
        context = "探究",
        recommended = true,
        exportString = "CAEAAAAAAAAAAAAAAAAAAAAAAYGGLzMzswMzEzMzMzMzMziZmhZMDAAAMzMzyyMTbAAAAAAgFA22GzMzgZbYMzYDAAgZ2AmJjBMDGA",
      },
      {
        heroTalent = "Frostfire",
        context = "團隊單目標",
        exportString = "CAEAAAAAAAAAAAAAAAAAAAAAAYGGLzMzsMmZmYmxMjZMziZmZmxMzsMTzMLzCAAAaBAAAAAAsBw22YmZGYZGjZstAAAAmZwMZMgBGA",
      },
      {
        heroTalent = "Frostfire",
        context = "傳奇+",
        exportString = "CAEAAAAAAAAAAAAAAAAAAAAAAMzwYZmZmFmZmYGmZmZmZWMzMzMGzsMTzMLzCAAAaBAAWAAAAWAYbbMzMDGGzMzYDAAAMzgZyYAgB",
      },
      {
        heroTalent = "Frostfire",
        context = "探究",
        exportString = "CAEAAAAAAAAAAAAAAAAAAAAAAYGGLzMzswMzEzMzMzMzMziZmZmxYmlZamZZWAAAQLAAAAAAgFA22GzMzghxMzM2AAAAzMYmMGAYA",
      },
    },
    rotation = {
      {
        heroTalent = "Spellslinger",
        context = "單目標",
        steps = {
          "{44614} if {190447} is active and {1247729} is not active",
          "{30455} if {112965} at 2 stacks",
          "{84714}",
          "{199786}",
          "{30455} if {112965} is active",
          "{30455} if {1221389} at 6或more stacks",
          "{205021} if you have 3或fewer {1246832}",
          "{44614}",
          "{116}",
        },
      },
      {
        heroTalent = "Frostfire",
        context = "單目標",
        steps = {
          "{44614} if {190447} is active and {1247729} is not active",
          "{84714}",
          "{199786}",
          "{1247777}",
          "{30455} if {112965} is active",
          "{30455} if {1221389} at 10或more stacks",
          "{44614}",
          "{205021}",
          "{431044}",
        },
      },
      {
        heroTalent = "Spellslinger",
        context = "多目標",
        steps = {
          "{190356} if {270233} is active",
          "{44614} if {190447} is active and {1247729} is not active",
          "{30455} if {112965} at 2 stacks",
          "{84714}",
          "{199786}",
          "{30455} if {112965} is active",
          "{30455} if {1221389} at 6或more stacks",
          "{205021} if you have 3或fewer {1246832}",
          "{44614}",
          "{116}",
        },
      },
      {
        heroTalent = "Frostfire",
        context = "多目標",
        steps = {
          "{190356} if {270233} is active",
          "{44614} if {190447} is active and {1247729} is not active",
          "{84714}",
          "{199786}",
          "{1247777}",
          "{190356}",
          "{30455} if {112965} is active",
          "{30455} if {1221389} at 10或more stacks",
          "{44614}",
          "{205021} if {431176} is not active",
          "{431044}",
        },
      },
      {
        heroTalent = "Spellslinger",
        context = "起手",
        steps = {
          "{190356} (for AoE)",
          "{116} (for ST)",
          "{205021}",
        },
      },
      {
        heroTalent = "Frostfire",
        context = "起手",
        steps = {
          "{190356} (for AoE)",
          "{431044} (for ST)",
          "{205021}",
        },
      },
    },
  },
}
