...
L["The default behavior is to perform the target or mouseover interaction once until you interact with another NPC with a saved team. Check this to always interact with NPCs that have a saved team not already loaded."] = "預設行為是需要選為目標或滑鼠指向過一次，直到你與另一個擁有已儲存隊伍的 NPC 互動。勾選此選項可讓你總是與擁有尚未載入已儲存隊伍的 NPC 進行互動。"
