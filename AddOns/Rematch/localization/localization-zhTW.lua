...
L["Leveling preferences choose which pets are picked first in the leveling queue. All criteria are optional."] = "練等偏好可讓您選擇要在練等佇列中優先選取哪隻寵物。所有的條件皆為選填。"
...
L["No targets :("] = "沒有對手 :("
L["Load a set of random high level pets. If the recent target's pets are known, random pets are preferred if they are strong vs and tough vs the opponent pets."] = "載入一組隨機高等級寵物。如果最近對手的寵物已知，則優先選擇強大且對手寵物具有抗性的隨機寵物。"
...
L["The default behavior is to perform the target or mouseover interaction once until you interact with another NPC with a saved team. Check this to always interact with NPCs that have a saved team not already loaded."] = "預設行為是在您與另一個擁有已儲存隊伍的 NPC 互動之前，選為目標或滑鼠指向過一次。勾選此選項可讓您總是與擁有尚未載入已儲存隊伍的 NPC 進行互動。"
L["Prefer Uninjured Teams"] = "偏好未受傷的隊伍"
L["When you interact with an NPC that has more than one team saved to it, choose the team with no injured pets instead of the topmost team for the target. On Prompt To Load and Show Minimized Window options where you can choose which team to load, start with the healthiest team."] = "當您與一個已儲存多個隊伍的 NPC 互動時，為對手選擇沒有受傷寵物的隊伍，而非最上方的隊伍。在提示載入及顯示最小化視窗的選項中，您可以選擇要載入哪個隊伍，從最健康的隊伍開始。"
