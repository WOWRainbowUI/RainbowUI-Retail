...
function WorldQuestTrackerAddon.OpenOptionsPanel()
    ...
    optionsFrame.TitleBar.Text:SetText(L["World Quest Tracker Options"])
	...
	local tabContainer = DF:CreateTabContainer(optionsFrame, L["WQT Options"], "WQTOptionsPanelContainer",
	{
    ...
    --buttons moved from the statusbar
			...
			WorldQuestTracker.SetupStatusbarButton(statisticsButton, L["Statistics"])
			if (GameCooltip.InjectQuickTooltip) then
				GameCooltip:InjectQuickTooltip(statisticsButton, L["Click to show reward statistics from world quests, timeline and quests available on your other characters."])
			end
			...
			local BuildFilterMenu = function()
				...
				GameCooltip:AddLine(L["Select All"])
				GameCooltip:AddMenu(1, toggle_filters_all_on)

				GameCooltip:AddLine(L["Select None"])
				...
				if (WorldQuestTracker.db.profile.filter_force_show_brokenshore) then
					GameCooltip:AddLine(L["Ignore New Zones"], "", 1, "orange")
					GameCooltip:AddLine(L["World quets on new zones will always be shown.\n\nCurrent new zones:\n-Najatar\n-Machagon."], "", 2)
					GameCooltip:AddIcon([[Interface\ICONS\70_inscription_vantus_rune_tomb]], 1, 1, 23*.54, 37*.40, 0, 1, 0, 1)
					GameCooltip:AddIcon([[Interface\BUTTONS\UI-CheckBox-Check]], 1, 2, 16, 16, 0, 1, 0, 1, overlayColor, nil, true)
				else
					GameCooltip:AddLine(L["Ignore New Zones"], "", 1, "silver")
					GameCooltip:AddLine(L["World quets on new zones will always be shown.\n\nCurrent new zones:\n-Najatar\n-Machagon"], "", 2)
