...
D["!!!gmFonts"] = {
	...
	tags = { "MISC" }, 
	title = "遊戲字體",
	desc = "載入時會自動將遊戲預設的系統、聊天、提示說明和傷害數字，更改為字體材質包中的字體。``可以在設定選項中調整視窗介面文字的字體和大小，其他地方的文字則是在每個插件中分別設定。例如聊天文字在 '聊天視窗美化' 插件設定、玩家和怪頭上的名字在 '威力血條' 設定...等等。``要使用自己的字體，請看問與答`https://addons.miliui.com/wow/rainbowui#q157 ``關閉此插件便可以換回遊戲原本的字體。``|cffFF2D2D特別注意：請不要選擇英文字體，會無法顯示中文字。`",
	modifier = "彩虹ui",
    {
        text = "設定選項",
...
D["Favorites"] = {
	...
	tags = { "SOCIAL" },
	title = "最愛好友名單",
	desc = "在遊戲內建的好友名單中新增 '加入最愛' 和 '搜尋好友' 的功能。還可以自訂多個不同的群組，幫好友名單分類，輕鬆管理你的好友名單。``現在更包含了彩色好友名單的功能，會使用職業顏色來顯示好友的角色名稱。`",
	modifier = "彩虹ui",
...
D["TinyExtraBars_MacroTextHelpers"] = {
	defaultEnable = 0,
	tags = { "ACTIONBAR" },
	title = "快捷列彈出式按鈕",
...
D["Talentless"] = {
	...
	{
		type = "text",
		text = "用快速鍵切換專精：從 Esc > 選項 > 按鍵綁定 > 插件 > 啟用專精 1~4，設定按鍵。",
	}
};
D["TankMD"] = {
	defaultEnable = 0,
