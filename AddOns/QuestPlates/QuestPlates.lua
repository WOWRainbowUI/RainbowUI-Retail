        ...
        if uiMapID then
            for _, task in pairs(C_TaskQuest.GetQuestsForPlayerByMapID(uiMapID) or {}) do
                if task.inProgress then
                    local questID = task.questID  -- Fix: Use task.questID (correct case)
                    if questID then  -- Add nil check for safety
                        local questName = C_TaskQuest.GetQuestInfoByQuestID(questID)
                        if questName then
                            ActiveWorldQuests[questName] = questID
                        else
                            print("[QuestPlates] Warning: No quest name for questID:", questID)
                        end
                    else
                        print("[QuestPlates] Error: Nil questID in task:", task.mapID, task.x, task.y)
                    end
                end
            end
        end
