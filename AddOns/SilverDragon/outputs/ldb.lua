...
function module:SetupWorldMap()
	...
	function button:Refresh()
		...
		if overlay then
			...
			button.options.help = {
				RED_FONT_COLOR:WrapTextInColorCode("沒有稀有怪獸與牠們的產地的地圖圖示: 已停用地圖圖示")
			}
			tAppendAll(button.options.help, default_help)
		end
...
do
	...
	local function show_mob_tooltip(cell, mobid)
		...
		GameTooltip:AddLine("左鍵: 顯示地圖位置", 0, 1, 1)
		GameTooltip:AddLine("Ctrl+左鍵: 開始導航", 0, 1, 1)
		GameTooltip:AddLine("Shift+左鍵: 發送位置到聊天", 0, 1, 1)
		GameTooltip:Show()
	...
	function module:ShowTooltip(parent, options)
		...
		if #sorted_mobs > 0 then
			local headerLine, headerIndex = tooltip:AddHeader("名字", "次數", "最近看到", hunter_icon)
