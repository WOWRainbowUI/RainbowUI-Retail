...
function addon:FormatLastSeen(t)
	...
	if minutes > 119 then
		...
		if hours > 23 then
			return math.floor(((currentTime - t) / 86400) + 0.5).." 天"
		else
			return hours.." 小時"
		end
	else
...
function addon:GetClosestLocationForMob(id)
	...
	for zone2, coords in pairs(ns.mobdb[id].locations) do
		for i, coord in ipairs(coords) do
			-- skip a spot the mob only stands in under a condition that isn't met
			if self:CoordGateMet(ns.mobdb[id], zone2, coord) then
