...
-- ------------------------------------------------------------
local MSUF_TIPS = {
    "更大幅度調整：按住 |cff00ff00SHIFT|r 拖動滑桿可加快數值變更。",
    "微調：按住 |cff00ff00CTRL|r 拖動滑桿可進行微小調整。",
    "快速重置：如果感覺不對勁，試試 |cff00ff00/msuf reset|r (重置框架位置)。",
    "原廠重置：使用 |cff00ff00主選單 → 進階 → 原廠重置|r (或 /msuf fullreset confirm + /reload)。",
    "編輯模式：使用 |cff00ff00切換編輯模式|r 來快速移動框架，然後用位置彈出視窗進行微調。",
    "設定檔安全性：在進行大膽實驗前先建立一個新的設定檔 — 需要時可隨時切換回來。",
    "顏色：|cff00ff00顏色|r 標籤頁讓你自訂幾乎所有內容 (字型、計量條、施法條、顯著標示)。",
    "遊戲性：|cff00ff00遊戲性|r 標籤頁包含你可以啟用/停用的額外 UI 工具與警告。",
    "推薦：|cff00ff00Sensei Resource Bar|r 與 MSUF 搭配極佳，能清晰追蹤資源。",
    "UI 縮放提示：MSUF 擁有獨立的 UI 縮放 — 與全域 UI 縮放分開。",
    "故障排除：如果視覺效果沒有更新，快速執行 |cff00ff00/reload|r 通常能修復大多數 UI 狀態問題。",
    "可讀性：稍微大一點的字型通常比更大的框架更有幫助 (尤其是在團隊副本中)。",
    "在 MSUF Unhalted 開發期間，R41z0r 和其他優秀的插件開發者提供了許多協助！",
    "Danders 是一個很棒的隊伍/團隊框架插件，與 MSUF 運作非常良好。",
    "社群：如果你喜歡 MSUF，請分享給朋友 — UI 插件是靠口碑成長的。",
}
...
local function MSUF_ApplyPreset(presetName)
  ...
  if type(preset) ~= "table" then
    print("|cffff3333MSUF:|r 找不到預設檔：" .. tostring(presetName))
    return
  end
  ...
  if type(MSUF_DB) ~= "table" then
    print("|cffff3333MSUF:|r 資料庫尚未準備好 (MSUF_DB 遺失)。")
    return
  end
  ...
  if type(importStr) == "string" then
    ...
    if okPrefix and (prefix == "MSUF2" or prefix == "MSUF3") then
      ...
      if type(imp) == "function" then
        ...
        print("|cff00ff00MSUF:|r 已載入預設檔：" .. tostring(presetName))
        if type(MSUF_ShowReloadRecommendedPopup) == "function" then
          MSUF_ShowReloadRecommendedPopup("預設檔：" .. tostring(presetName))
        end
        return
      else
        print("|cffff3333MSUF:|r 無法載入此預設檔 (MSUF_ImportFromString 遺失)。")
      end
    end
  end
...
MSUF_ShowReloadRecommendedPopup = function(label)
  if InCombatLockdown and InCombatLockdown() then
    if type(MSUF_Print) == "function" then
      MSUF_Print("建議重新載入介面 (戰鬥中無法顯示彈出視窗)。")
    else
      print("|cffffaa00MSUF:|r 建議重新載入介面 (戰鬥中無法顯示彈出視窗)。")
    end
    return
  end
...
local function MSUF_EnsureCopyLinkPopup()
    ...
    f:SetScript("OnShow", function(self)
        if self._msufTitleFS then
            self._msufTitleFS:SetText(self._msufTitle or "連結")
        end
...
local function MSUF_ShowCopyLink(title, url)
    ...
    f._msufTitle = tostring(title or "連結")
...
local function MSUF_SetGlobalUiScale(scale, silent, opts)
    ...
    if InCombatLockdown and InCombatLockdown() then
        ...
        if not silent then
            MSUF_Print("無法在戰鬥中變更全域 UI 縮放。將在戰鬥結束後套用。")
        end
        return
    end
    ...
    if not silent then
        local cvarScale = clamp(scale, MSUF_MIN_UISCALE_CVAR, MSUF_MAX_UISCALE_CVAR)
        MSUF_Print(string.format("全域 UI 縮放設定為 %.3f (CVar %.3f)", scale, cvarScale))
    end
end
...
local function MSUF_SetScalingDisabled(disable, silent)
    ...
    if InCombatLockdown and InCombatLockdown() then
        ...
        if not silent then
            MSUF_Print("MSUF 縮放已停用。將在戰鬥結束後完全重置。")
        end
        return
    end
    ...
    if not silent then
        MSUF_Print("MSUF 縮放已停用 (現在由暴雪處理縮放)。")
    end
end
...
MSUF_BuildTools = function(parent, opts)
    ...
    local titleText = (opts and opts.title) or "工具"
...
local btn1440 = UI_Button(parent, "1440", segW, segH, "LEFT", btn1080, "RIGHT", segGap, 0, function()
    MSUF_ShowReloadConfirm("全域 UI 縮放：1440p", function()
        if _G and _G.MSUF_SetScalingDisabled then _G.MSUF_SetScalingDisabled(false, true) end
...
local btnAuto = UI_Button(parent, "自動", segW, segH, "LEFT", btn1440, "RIGHT", segGap, 0, function()
    MSUF_ShowReloadConfirm("全域 UI 縮放：自動", function()
        if _G and _G.MSUF_SetScalingDisabled then _G.MSUF_SetScalingDisabled(false, true) end
    ...
    if MSUF_AddTooltip then
        MSUF_AddTooltip(btn1080, "全域 UI 縮放：1080", "套用 MSUF 的 1080p 設定並重載介面。重載後自動恢復暴雪縮放。")
        MSUF_AddTooltip(btn1440, "全域 UI 縮放：1440", "套用 MSUF 的 1440p 設定並重載介面。重載後自動恢復暴雪縮放。")
        MSUF_AddTooltip(btnAuto, "全域 UI 縮放：自動", "停止強制套用 MSUF 全域縮放，並恢復您之前的暴雪 UI 縮放設定。")
    end
    ...
    msufLabel:SetText("UI 縮放 (10–100%)")
    ...
    if MSUF_AddTooltip then
        MSUF_AddTooltip(msufMinus, "UI 縮放 -", "降低全域 UI 縮放。Shift=5%，Ctrl=10%。")
        MSUF_AddTooltip(msufPlus, "UI 縮放 +", "增加全域 UI 縮放。Shift=5%，Ctrl=10%。")
    end
    ...
    local function MSUF_SetScalingToggleVisual(disabled)
        ...
        if not fs then return end
        if disabled then
            if msufOff.SetText then msufOff:SetText("縮放關閉") end
            if fs.SetTextColor then fs:SetTextColor(1.0, 0.2, 0.2, 1.0) end -- red
        else
            if msufOff.SetText then msufOff:SetText("縮放開啟") end
            if fs.SetTextColor then fs:SetTextColor(0.2, 1.0, 0.2, 1.0) end -- green
        end
    end

    local msufReset = UI_Button(parent, "重置", resetW, 18, "TOPLEFT", msufSlider, "BOTTOMLEFT", 0, -6, function()
    ...
    msufOff = UI_Button(parent, "縮放關閉", offW, 18, "LEFT", msufReset, "RIGHT", 8, 0, function()
        local g = MSUF_EnsureGeneral and MSUF_EnsureGeneral() or nil
    ...
    if MSUF_AddTooltip then
        MSUF_AddTooltip(msufReset, "重置 UI 縮放", "將全域 UI 縮放重置回 100% (自訂)。")
        MSUF_AddTooltip(msufOff, "停用所有 MSUF 縮放", "關閉所有 MSUF 套用的縮放 (全域 UI 縮放 + MSUF 縮放)，然後重載介面。由暴雪負責縮放。")
        MSUF_AddTooltip(msufSlider, "UI 縮放 (全域)", "縮放整個 WoW UI (UIParent)。100% = 1.0，10% = 0.10。這會將預設值設為自訂。")
    end
    ...
    function api.Refresh()
        ...
        if cur then
            globalCur:SetText(string.format("目前：%.3f", cur))
        else
            globalCur:SetText("目前：?")
        end
...
local function MSUF_EnsureModulesPanelBuilt()
    ...
    note:SetText("停用可能需要重載介面 (/reload) 才能完全移除現有的風格設定。")
...
if rb.Text then
    rb.Text:SetText("圓角單位框架")
    if type(_G.MSUF_SkinText) == "function" then pcall(_G.MSUF_SkinText, rb.Text) end
end
rb.tooltipText = "將 MSUF 單位框架變為圓角，使用超橢圓遮罩來處理生命/能量/吸收條與背景。"
...
main     = { title = "MSUF 選項",  nav = "Options",  build = MSUF_EnsureMainOptionsPanelBuilt,
    ...
    uf_player       = { title = "MSUF 玩家",           nav = nil, build = MSUF_EnsureMainOptionsPanelBuilt, select = function() MSUF_SelectMainOptionsKey("player") end },
    uf_target       = { title = "MSUF 目標",           nav = nil, build = MSUF_EnsureMainOptionsPanelBuilt, select = function() MSUF_SelectMainOptionsKey("target") end },
    uf_targettarget = { title = "MSUF 目標的目標", nav = nil, build = MSUF_EnsureMainOptionsPanelBuilt, select = function() MSUF_SelectMainOptionsKey("targettarget") end },
    uf_focus        = { title = "MSUF 專注目標",            nav = nil, build = MSUF_EnsureMainOptionsPanelBuilt, select = function() MSUF_SelectMainOptionsKey("focus") end },
    uf_boss         = { title = "MSUF 首領框架",      nav = nil, build = MSUF_EnsureMainOptionsPanelBuilt, select = function() MSUF_SelectMainOptionsKey("boss") end },
    uf_pet          = { title = "MSUF 寵物",              nav = nil, build = MSUF_EnsureMainOptionsPanelBuilt, select = function() MSUF_SelectMainOptionsKey("pet") end },

    -- Options tree (Phase 4b)
    opt_bars        = { title = "MSUF 計量條",             nav = nil, build = MSUF_EnsureMainOptionsPanelBuilt, select = function() MSUF_SelectMainOptionsKey("bars") end },
    opt_fonts       = { title = "MSUF 字型",            nav = nil, build = MSUF_EnsureMainOptionsPanelBuilt, select = function() MSUF_SelectMainOptionsKey("fonts") end },
    opt_auras       = { title = "MSUF 光環",            nav = nil, build = MSUF_EnsureMainOptionsPanelBuilt, select = function() MSUF_SelectMainOptionsKey("auras") end },
    auras2         = { title = "MSUF 光環 2.0",        nav = nil, build = MSUF_EnsureAuras2PanelBuilt },
    opt_castbar     = { title = "MSUF 施法條",          nav = nil, build = MSUF_EnsureMainOptionsPanelBuilt, select = function() MSUF_SelectMainOptionsKey("castbar") end },
    opt_misc        = { title = "MSUF 其他設定",    nav = nil, build = MSUF_EnsureMainOptionsPanelBuilt, select = function() MSUF_SelectMainOptionsKey("misc") end },
    opt_colors      = { title = "MSUF 顏色",           nav = nil, build = MSUF_EnsureColorsPanelBuilt },
    -- Phase 2: low-risk shortcuts into the main panel (same underlying panel, just pre-select a tab)
    castbar  = { title = "MSUF 施法條",  nav = "nil",  build = MSUF_EnsureMainOptionsPanelBuilt,
        select = function(subkey)
    ...
    profiles = { title = "MSUF 設定檔", nav = "nil", build = MSUF_EnsureMainOptionsPanelBuilt,
        select = function()
            MSUF_SelectMainOptionsKey("profiles")
        end
    },

    colors   = { title = "MSUF 顏色",   nav = "nil",   build = MSUF_EnsureColorsPanelBuilt },
    gameplay = { title = "MSUF 遊戲性", nav = "nil", build = MSUF_EnsureGameplayPanelBuilt },
    modules  = { title = "MSUF 模組", nav = "nil", build = MSUF_EnsureModulesPanelBuilt },
}
...
local _MSUF_LEGACY_TAB_TEXTS = {
    ...
    -- Traditional Chinese Translation (Added for TW/Taiwan compatibility)
    ["玩家"] = true,
    ["目標"] = true,
    ["目標的目標"] = true,
    ["專注目標"] = true,
    ...
    ["其他設定"] = true,
    ["設定檔"] = true,
    ["專注目標斷法"] = true, -- Focus Kick translation
}
...
local function MSUF_SwitchMirrorPage(key, subkey)
    ...
    local function UpdateTitle(activeKey)
        if S.win and S.win._msufTitleFS and S.win._msufTitleFS.SetText then
            ...
            S.win._msufTitleFS:SetText((info and info.title) or "MSUF 選單")
        end
    end
...
local function MSUF_BuildMirrorNavButtons(navParent, btnW, btnH)
    ...
    -- Build
    AddLeaf("home", "儀表板", "Menu")

    local hdrUF = AddHeader("unitframes", "- 單位框架", true)
    out["hdr_unitframes"] = hdrUF
    local ufButtons = {
        AddIndentedLeaf("uf_player", "玩家", "Unit Frames"),
        AddIndentedLeaf("uf_target", "目標", "Unit Frames"),
        AddIndentedLeaf("uf_targettarget", "目標的目標", "Unit Frames"),
        AddIndentedLeaf("uf_focus", "專注目標", "Unit Frames"),
        AddIndentedLeaf("uf_boss", "首領框架", "Unit Frames"),
        AddIndentedLeaf("uf_pet", "寵物", "Unit Frames"),
    }

    local hdrOpt = AddHeader("options", "- 選項", true)
    out["hdr_options"] = hdrOpt
    local optButtons = {
        AddIndentedLeaf("opt_bars", "計量條", "Options"),
        AddIndentedLeaf("opt_fonts", "字型", "Options"),
        -- Auras (legacy) intentionally hidden (Auras 2.0 replaces it)
        AddIndentedLeaf("auras2", "光環 2.0", "Options"),
        AddIndentedLeaf("opt_castbar", "施法條", "Options"),
        AddIndentedLeaf("opt_misc", "其他設定", "Options"),
        AddIndentedLeaf("opt_colors", "顏色", "Options"),
    }


-- Gameplay (standalone, no tree)
AddLeaf("gameplay", "遊戲性", "Menu")

-- Modules tree (starts with Style)
local hdrModules = AddHeader("modules", "+ 模組", false)
out["hdr_modules"] = hdrModules
local modButtons = {
    AddIndentedLeaf("modules", "風格", "Modules"),
}
-- Standalone leaf pages
AddLeaf("profiles", "設定檔", "Menu")
    ...
    local function Reflow()
        ...
        -- Unit Frames header
        if hdrUF then
            hdrUF:SetText((headers.unitframes and "- 單位框架") or "+ 單位框架")
            Place(hdrUF, 0)
        end
        ...
        -- Options header
        if hdrOpt then
            hdrOpt:SetText((headers.options and "- 選項") or "+ 選項")
            Place(hdrOpt, 0)
        end
        ...
        if hdrModules then
            hdrModules:SetText((headers.modules and "- 模組") or "+ 模組")
            Place(hdrModules, 0)
        end
...
local close = UI_CloseButton(f, "TOPRIGHT", f, "TOPRIGHT", -4, -4)
    ...
    homeHint:SetText("快速工具與 UI 縮放 (內容同 /msuf options)。")
...
local function MSUF_ShowResetPositionsConfirm()
    ...
    if not StaticPopupDialogs["MSUF_RESET_POS_CONFIRM"] then
        StaticPopupDialogs["MSUF_RESET_POS_CONFIRM"] = {
            text = "現在重置 MSUF 框架位置？\n\n這會將使用中設定檔的 MSUF 框架位置與可見性重置為預設值。",
            button1 = YES,
            button2 = NO,
            timeout = 0,
            whileDead = 1,
            ...
            OnAccept = function()
                if _G.SlashCmdList and _G.SlashCmdList["MIDNIGHTSUF"] then
                    ...
                    MSUF_Print("找不到 /msuf reset 處理程序。")
                end
            end,
        }
    end
...
local quick = CreateCard(colL, "快速動作")
quick:SetHeight(206)

local bigW = 410
local bigH = 26

local bEdit = UI_Button(quick, "切換編輯模式", bigW, bigH, "TOPLEFT", quick, "TOPLEFT", 12, -34, DashToggleEditMode)
MSUF_SkinDashboardButton(bEdit)
if MSUF_AddTooltip then
    MSUF_AddTooltip(bEdit, "切換編輯模式", "進入 MSUF 編輯模式以拖曳框架並調整位置。")
end
...
local bReset = UI_Button(quick, "重置框架位置", bigW, bigH, "TOPLEFT", bEdit, "BOTTOMLEFT", 0, -8, MSUF_ShowResetPositionsConfirm)
MSUF_SkinDashboardButton(bReset)
if MSUF_AddTooltip then
    MSUF_AddTooltip(bReset, "重置框架位置", "將 MSUF 框架位置與可見性重置為預設值 (啟用設定檔)。")
end
...
local bColors = UI_Button(quick, "顏色", 160, smallH, "TOPLEFT", bReset, "BOTTOMLEFT", 0, -10, function() MSUF_SwitchMirrorPage("colors") end)
MSUF_SkinDashboardButton(bColors)

local bGame = UI_Button(quick, "遊戲性", 118, smallH, "LEFT", bColors, "RIGHT", 10, 0, function() MSUF_SwitchMirrorPage("gameplay") end)
...
local bProfiles = UI_Button(profCard, "開啟設定檔", 160, 22, "TOPRIGHT", profCard, "TOPRIGHT", -12, -30, function() MSUF_SwitchMirrorPage("profiles") end)
...
do
    ...
    local bDiscordSelect = UI_Button(discordRow, "選取", 72, 18, "RIGHT", discordRow, "RIGHT", 0, 0, function()
    ...
    if MSUF_AddTooltip then
        MSUF_AddTooltip(bDiscordSelect, "選取", "點擊以選取此文字。")
    end
...
local advToggle = UI_Button(adv, "顯示", 88, 18, "TOPRIGHT", adv, "TOPRIGHT", -12, -8, function() end)
...
cmds:SetWidth(410)
cmds:SetText("/msuf  - 開啟 MSUF 選單\n" .. "/msuf options  - 開啟選項\n" .. "/msuf colors  - 開啟顏色\n" .. "/msuf gameplay  - 開啟遊戲性\n" .. "/msuf reset  - 重置框架位置\n" .. "/msuf fullreset  - 原廠重置\n" .. "/msuf absorb  - 切換血量文字中的吸收值\n" .. "/msufprofile on/off/reset/show\n" .. "/msuf help  - 在聊天視窗列出說明")
MSUF_SkinText(cmds)
...
local bHelp = UI_Button(btnRow, "列出說明", 120, 20, "BOTTOMLEFT", btnRow, "BOTTOMLEFT", 0, 0, function()
    if _G.SlashCmdList and _G.SlashCmdList["MIDNIGHTSUF"] then
...
local bFullReset = UI_Button(btnRow, "原廠重置", 120, 20, "LEFT", bHelp, "RIGHT", 8, 0, function()
...
local function AdvApplyState(open)
    if open then
        advToggle:SetText("隱藏")
        advHint:SetText("")
        advBody:Show()
    else
        advToggle:SetText("顯示")
        advHint:SetText("已隱藏。點擊顯示以查看指令與進階工具。")
        advBody:Hide()
    end
end
...
local presetsCard = CreateCard(colR, "預設配置", scaleCard, -12)
...
UIDropDownMenu_SetText(presetDrop, presetsCard._msufSelectedPreset or "選擇預設配置...")
...
local bLoadPreset = UI_Button(presetsCard, "載入預設配置", 240, 24, "TOPLEFT", presetDrop, "BOTTOMLEFT", 16, -6, function()
    local sel = presetsCard._msufSelectedPreset
    if not sel then
        MSUF_Print("請先選擇一個預設配置。")
        return
    end
...
if MSUF_AddTooltip then
    MSUF_AddTooltip(bLoadPreset, "載入預設配置", "將選定的預設配置套用到你目前啟用的設定檔。這會覆蓋現有設定 (如不確定請先匯出備份)。")
end
...
presetHint:SetText("將會覆蓋你目前啟用的設定檔設定。")
...
do
    ...
    supportLabel:SetText("支援 MSUF 開發：")
...
local function MSUF_CreateMiniMenu()
    ...
    if MSUF_AddTooltip then
        MSUF_AddTooltip(fullBtn, "開啟完整選項", "開啟完整的 MSUF 選項視窗。")
    end
