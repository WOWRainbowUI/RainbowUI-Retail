...
ns.UF.Unitframe_OnEnter = ns.UF.Unitframe_OnEnter or function(self)
    ...
    if hb then if g.enableHighlightOnHover then if self.UpdateHighlightColor then self:UpdateHighlightColor() end; hb:Show() else hb:Hide() end end
...
function ns.Text.RenderHpMode(self, show, hpStr, hpPct, hasPct, conf, g, absorbText, absorbStyle)
    ...
    local function SetWithAbsorb(fmtNo, fmtSpace, fmtParen, ...)
        if absorbText then
            if absorbStyle == "PAREN" then
                ns.Text.SetFormatted(self.hpText, true, fmtParen, ..., absorbText)
            else
                ns.Text.SetFormatted(self.hpText, true, fmtSpace, ..., absorbText)
            end
        else
...
function _G.MSUF_ApplyUnitAlpha(frame, key)
    ...
    if unit and F.UnitExists and F.UnitExists(unit) then
        if F.UnitIsConnected and (F.UnitIsConnected(unit) == false) then
...
function _G.MSUF_RefreshAllUnitAlphas()
    ...
    local IsDisabled = ns and ns.UF and ns.UF.IsDisabled
    local UnitExists = F.UnitExists
    local GetKey = GetConfigKeyForUnit
    local bossTest = (MSUF_BossTestMode == true)
    local function ApplyOne(f, unitKey)
        if not f then return end
        local unit = f.unit or unitKey
        if not unit then return end
        local k = f.msufConfigKey or (GetKey and GetKey(unit))
        local conf = (k and MSUF_DB and MSUF_DB[k]) or nil
        if IsDisabled and IsDisabled(conf) then return end
        local exists = (UnitExists and UnitExists(unit)) and true or false
        if exists or unit == "player" or (f.isBoss and bossTest) then
            ApplyUnitAlpha(f, k)
        else
    ...
    MSUF_ForEachUnitFrame(ApplyOne)
end
if not _G.MSUF_AlphaEventFrame then
    _G.MSUF_AlphaEventFrame = F.CreateFrame("Frame")
    ...
    local function MSUF_DoAlphaRefresh()
        if _G.MSUF_RefreshAllUnitAlphas then
            ...
            C_Timer.After(0, function() if _G.MSUF_RefreshAllUnitAlphas then _G.MSUF_RefreshAllUnitAlphas() end end)
    end
    end
...
if PetCastingBarFrame then
    ...
    print("|cff7aa2f7MSUF|r: |cffc0caf5/msuf|r |cff565f89to open options|r  |cff565f89•|r  |cff9ece6a Beta Build 1.9b4|r  |cff565f89•|r  |cffc0caf5 Huge background changes -|r  |cfff7768eReport bugs in the Discord.|r")
end, nil, true)
do
