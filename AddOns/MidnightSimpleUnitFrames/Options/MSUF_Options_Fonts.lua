...
function ns.MSUF_Options_Fonts_Build(panel, fontGroup)
    ...
    fontColorHeader:SetText("字型顏色與樣式")
    ...
    local colorList = {
        ...
        { key = "magenta",   r=1,   g=0,   b=1,   label="洋紅" },
        { key = "orange",    r=1,   g=0.5, b=0,   label="橘色" },
        { key = "purple",    r=0.6, g=0,   b=0.8, label="紫色" },
        { key = "pink",      r=1,   g=0.6, b=0.8, label="粉紅" },
        { key = "turquoise", r=0,   g=0.9, b=0.8, label="藍綠色" },
    ...
    _G.MSUF_COLOR_LIST = colorList
        ...
        boldCheck.text:SetText("使用粗體文字 (THICKOUTLINE)")
        ...
        noOutlineCheck.text:SetText("停用文字黑色輪廓 (描邊)")
        ...
        nameClassColorCheck.text:SetText("玩家名字依職業著色")
        ...
        npcNameRedCheck.text:SetText("NPC/首領名字使用 NPC 顏色")
        ...
        powerTextColorByTypeCheck.text:SetText("能量數值依據能量類型著色")
        ...
        shortenNamesCheck.text:SetText("縮短單位名字 (玩家除外)")
        ...
        shortenNameClipSideLabel:SetText("縮短樣式")
    ...
    _G["MSUF_ShortenNameMaxCharsSliderText"]:SetText("名字最大長度")
    ...
    _G["MSUF_ShortenNameFrontMaskSliderText"]:SetText("保留空間")
    ...
    msufShortenInfoBtn:SetScript("OnClick", function(self)
        ...
        GameTooltip:SetText("名字縮短")
        EnsureDB()
        local side = (MSUF_DB and MSUF_DB.general and MSUF_DB.general.shortenNameClipSide) or "LEFT"
        if side == "RIGHT" then
            GameTooltip:AddLine("保留開頭：顯示前面的字母 (裁切尾端)。", 1, 1, 1, true)
            GameTooltip:AddLine("傳統模式使用單純的字串寬度裁切。", 0.95, 0.95, 0.95, true)
        else
            GameTooltip:AddLine("保留結尾：顯示後面的字母 (裁切前端)。", 1, 1, 1, true)
            GameTooltip:AddLine("保留空間可保護裁切邊緣 (避免重疊)。", 0.95, 0.95, 0.95, true)
        end
        GameTooltip:Show()
    end)
        ...
        textBackdropCheck.text:SetText("新增文字陰影 (背景)")
    ...
    if shortenNameFrontMaskSlider then
        ...
        textSizeHelp:SetText("全域預設值。除非在「單位框架 > 文字」中個別設定，否則框架將繼承此處設定。")
        ...
        local function MSUF_CompactTextSizeSlider(slider, shortLabel)
            ...
            if slider.minusButton then
                ...
                if slider.minusButton.text then
                    slider.minusButton.text:SetFont("Fonts\\bHEI01B.ttf", 10, "OUTLINE")
                end
            end
            ...
            if slider.plusButton then
                ...
                if slider.plusButton.text then
                    slider.plusButton.text:SetFont("Fonts\\bHEI01B.ttf", 10, "OUTLINE")
                end
            end
        end
        ...
        local function MSUF_FormatOverrideSummary(list)
            ...
            if #list == 1 then
                return "覆寫: " .. list[1], list[1]
            end
            return "覆寫: " .. list[1] .. " +" .. tostring(#list - 1), table.concat(list, ", ")
        end
        ...
        local function MSUF_ApplyOverrideTooltip(fs)
            ...
            fs:SetScript("OnEnter", function(self)
                if self.MSUF_FullOverrideList and self.MSUF_FullOverrideList ~= "" then
                    GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
                    GameTooltip:SetText("個別覆寫", 1, 0.9, 0.4)
        ...
        nameFontSizeSlider = CreateLabeledSlider(
            "MSUF_NameFontSizeSlider", "名字", fontGroup,
            8, 32, 1,
        ...
        MSUF_CompactTextSizeSlider(nameFontSizeSlider, "名字")
        ...
        hpFontSizeSlider = CreateLabeledSlider(
            "MSUF_HealthFontSizeSlider", "血量", fontGroup,
            8, 32, 1,
        ...
        MSUF_CompactTextSizeSlider(hpFontSizeSlider, "血量")
        ...
        powerFontSizeSlider = CreateLabeledSlider(
            "MSUF_PowerFontSizeSlider", "能量", fontGroup,
            8, 32, 1,
        ...
        MSUF_CompactTextSizeSlider(powerFontSizeSlider, "能量")
        ...
        MSUF_CompactTextSizeSlider(castbarSpellNameFontSizeSlider, "施法條")
        ...
        resetFontOverridesBtn:SetText("重置個別設定")
        resetFontOverridesBtn.tooltipText = "清除各單位的名字/血量/能量以及施法條的名字/時間文字大小個別設定，讓所有項目再次繼承全域預設值。"

        if not StaticPopupDialogs["MSUF_RESET_FONT_OVERRIDES"] then
            StaticPopupDialogs["MSUF_RESET_FONT_OVERRIDES"] = {
                text = "重置所有文字大小個別設定？\n\n這將清除名字/血量/能量以及施法條的所有個別設定，恢復為全域預設值。",
                button1 = YES,
                button2 = NO,
                whileDead = true,
                hideOnEscape = true,
        ...
        do
            if not _G["MSUF_FontsMenuPanelLeft"] then
                ...
                SetupPanel(right, "字型顏色與樣式")

                CreateSectionHeader(left, "MSUF_FontsMenuSection_Global", "全域字型")
                CreateSectionHeader(left, "MSUF_FontsMenuSection_Sizes", "文字大小")

                CreateSectionHeader(right, "MSUF_FontsMenuSection_Style", "文字樣式")
                CreateSectionHeader(right, "MSUF_FontsMenuSection_Colors", "名字顏色")
                CreateSectionHeader(right, "MSUF_FontsMenuSection_Names", "名字顯示")
            end
