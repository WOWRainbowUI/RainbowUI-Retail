...
local function CreateBuffDebuffAnchorDropdown(parent, x, y, getter, setter, layoutGetter)
	...
	GameTooltip:SetPoint("BOTTOM", anchor, "TOP", 0, 8)
        GameTooltip:SetText("增益/減益位置", 1, 1, 1)
        GameTooltip:AddLine("控制增益和減益在單位框架周圍的擺放位置。", 0.8, 0.8, 0.8, true)
        GameTooltip:AddLine(" ")
        GameTooltip:AddLine("需要版面配置：分行顯示。", 1, 0.82, 0, true)
        GameTooltip:AddLine("在「單行 (混合)」模式下，增益與減益共用一行，因此無法分開錨定。", 0.8, 0.8, 0.8, true)
        GameTooltip:Show()
    end
    ...
    UIDropDownMenu_Initialize(dd, function()
        ...
        AddItem("堆疊 (舊式)", "STACKED")
        AddItem("增益在頂部 / 減益在底部", "TOP_BOTTOM_BUFFS")
        AddItem("減益在頂部 / 增益在底部", "TOP_BOTTOM_DEBUFFS")

        AddItem("增益在頂部 / 減益在右側", "TOP_RIGHT_BUFFS")
        AddItem("減益在頂部 / 增益在右側", "TOP_RIGHT_DEBUFFS")

        AddItem("增益在底部 / 減益在右側", "BOTTOM_RIGHT_BUFFS")
        AddItem("減益在底部 / 增益在右側", "BOTTOM_RIGHT_DEBUFFS")

        AddItem("增益在底部 / 減益在左側", "BOTTOM_LEFT_BUFFS")
        AddItem("減益在底部 / 增益在左側", "BOTTOM_LEFT_DEBUFFS")

        AddItem("增益在頂部 / 減益在左側", "TOP_LEFT_BUFFS")
        AddItem("減益在頂部 / 增益在左側", "TOP_LEFT_DEBUFFS")
    end)
...
function ns.MSUF_RegisterAurasOptions_Full(parentCategory)
    ...
    local title = CreateTitle(panel, "Midnight Simple Unit Frames - 光環 2.0")
	...
	editBtn:SetScript("OnEnter", function(self)
		...
		GameTooltip:SetText("MSUF 編輯模式", 1, 1, 1)
...
local function A2_AutoOverrideFiltersIfNeeded()
    ...
    A2_ShowOverrideWarn("已啟用此單位的過濾器覆寫 (你編輯了過濾器)。")
    return true
end
...
local function ApplyOverrideUISafety()
    ...
    local fs = panel.__msufA2_overrideWarn
    if fs then
        local msg = "單位覆寫啟用中：灰色選項為「共用」。"
        if overrideFilters and not overrideCaps then
            msg = "過濾器覆寫啟用中：灰色選項為「共用」。"
        elseif overrideCaps and not overrideFilters then
            msg = "上限覆寫啟用中：灰色選項為「共用」。"
        end
        fs:SetText(msg)
        fs:SetAlpha(1)
        fs:Show()
...
-- ------------------------------------------------------------
    ...
    CreateBoolCheckboxPath(leftTop, "啟用光環 2.0", 12, -34, A2_DB, "enabled", nil,
        "總開關。關閉時，目標/專注/首領將不顯示光環。")

    -- Filters (master): gates all filter logic (Only-mine/Hide-permanent + Advanced)
    local cbEnableFilters = CreateBoolCheckboxPath(leftTop, "啟用過濾器", 200, -34, GetEditingFilters, "enabled", nil,
        "所選設定檔 (共用或單位覆寫) 的所有過濾邏輯總開關。關閉時，不套用任何過濾/高亮。")
    ...
    local cbMasque = CreateCheckbox(leftTop, "啟用 Masque 佈景主題", 200, -58,
        function() local _, s = GetAuras2DB(); return s and s.masqueEnabled end,
        function(v)
            local _, s = GetAuras2DB()
            if s then s.masqueEnabled = (v == true) end
        end,
        "使用 Masque 替換光環 2.0 圖示的佈景主題 (如果有安裝)。\n\n警告：在使用某些 Masque 佈景主題時，高亮邊框 (可竊取/可驅散/自身) 可能會看起來很怪。")
    ...
    RefreshMasqueToggleState = function()
        ...
        if not ready then
            cbMasque.tooltipText = "Masque 未載入或未就緒。請啟用/載入 Masque 插件，然後輸入 /reload。"
        else
...
do
    ...
    cbOverrideFilters = CreateCheckbox(leftTop, "覆寫共用過濾器", 380, -70,
        function() return GetOverrideForEditing() end,
        function(v) SetOverrideForEditing(v) end,
        "關閉時，此單位使用「共用」過濾器設定。開啟時，它會使用自己獨立的過濾器設定。")
    ...
    cbOverrideCaps = CreateCheckbox(leftTop, "覆寫共用上限", 380, -92,
        ...
        "關閉時，此單位使用「共用」上限 (最大增益/減益、每行圖示)。開啟時，它會使用自己獨立的上限。")
...
panel.__msufA2_overrideWarn = overrideWarn
    ...
    btnResetOverrides:SetScript("OnEnter", function(self)
        ...
        GameTooltip:ClearAllPoints()
        GameTooltip:SetPoint("TOPLEFT", self, "TOPRIGHT", 12, 0)
        GameTooltip:SetText("重置覆寫", 1, 1, 1)
        GameTooltip:AddLine("關閉所有單位的「覆寫共用過濾器」和「覆寫共用上限」，並回復為共用設定。", 0.8, 0.8, 0.8, true)
        GameTooltip:Show()
    end)
    ...
    local TIP_SHOW_STACK = '在光環圖示上顯示堆疊/應用層數 (例如 "2")。停用以隱藏堆疊數字。'
    local TIP_HIDE_PERMANENT = '隱藏沒有持續時間的增益。此選項絕不會隱藏減益。\n\n注意：由於 API 限制，目標/專注目標在戰鬥中可能仍會顯示常駐增益。'
    local TIP_ADV_INFO = '請使用光環 2.0 區域中的「啟用過濾器」作為總開關。\n\n「包含」選項是附加的 (它們絕不會隱藏你的一般光環)。\n「顯著標示」選項只會改變邊框顏色。\n\n減益類型：如果你選擇了任何類型，減益將僅限於所選的類型。'


    do
        local displayCB = {}
        local TIP_SWIPE_STYLE = "啟用時，冷卻轉圈代表經過的時間 (隨著時間流逝而變暗)。\n\n關閉此選項以保留預設的冷卻轉圈樣式。"
        BuildBoolPathCheckboxes(leftTop, {
            ...
            { "顯著標示自身增益", 12, -228, A2_Settings, "highlightOwnBuffs", nil,
                "以邊框顏色顯著標示你自己的增益 (僅視覺效果；不過濾)。", "cbHLOwnBuffs" },
            { "顯著標示自身減益", 200, -228, A2_Settings, "highlightOwnDebuffs", nil,
                "以邊框顏色顯著標示你自己的減益 (僅視覺效果；不過濾)。", "cbHLOwnDebuffs" },

            { "顯示冷卻轉圈", 12, -252, A2_Settings, "showCooldownSwipe", nil, nil, "cbShowSwipe" },
            { "冷卻轉圈隨時間變暗", 12, -300, A2_Settings, "cooldownSwipeDarkenOnLoss", nil, TIP_SWIPE_STYLE, "cbSwipeStyle" },

            { "顯示堆疊層數", 200, -276, A2_Settings, "showStackCount", nil, TIP_SHOW_STACK, "cbShowStackCount" },
            { "顯示冷卻文字", 200, -300, A2_Settings, "showCooldownText", nil,
                "在光環圖示上顯示倒數數字。停用以隱藏冷卻數字 (冷卻轉圈可保留啟用)。",
                "cbShowCooldownText" },

            { "顯示提示資訊", 12, -276, A2_Settings, "showTooltip", nil, nil, "cbShowTooltip" },
        }, displayCB)
    ...
    BuildBoolPathCheckboxes(leftTop, {
        { "僅顯示我的增益", 12, -204, A2_FilterBuffs, "onlyMine", nil, nil, nil, SyncLegacySharedFromSharedFilters },
        { "僅顯示我的減益", 200, -204, A2_FilterDebuffs, "onlyMine", nil, nil, nil, SyncLegacySharedFromSharedFilters },
        { "隱藏常駐增益", 200, -252, GetEditingFilters, "hidePermanent", nil, TIP_HIDE_PERMANENT, nil, SyncLegacySharedFromSharedFilters },
    })
    ...
    local maxBuffsSlider = CreateAuras2CompactSlider(leftTop, "最大增益數", 0, 40, 1, 12, -336, nil, GetMaxBuffs, function(v) A2_AutoOverrideCapsIfNeeded(); SetMaxBuffs(v) end)
    ...
    local maxDebuffsSlider = CreateAuras2CompactSlider(leftTop, "最大減益數", 0, 40, 1, 200, -336, nil, GetMaxDebuffs, function(v) A2_AutoOverrideCapsIfNeeded(); SetMaxDebuffs(v) end)
    ...
    local splitSpacingSlider = CreateAuras2CompactSlider(leftTop, "區塊間距", 0, 40, 1, 200, -414, nil, GetSplitSpacing, function(v) A2_AutoOverrideCapsIfNeeded(); SetSplitSpacing(v) end)
    ...
    local function ShowSplitSpacingTooltip()
        ...
        GameTooltip:SetText("區塊間距", 1, 1, 1)
        GameTooltip:AddLine("使用分開錨定時，控制增益和減益區塊距離單位框架邊緣的距離。", 0.8, 0.8, 0.8, true)
        GameTooltip:AddLine("需要版面配置：分行顯示。", 1, 0.82, 0, true)
        GameTooltip:Show()
    end
    ...
    local perRowSlider = CreateAuras2CompactSlider(leftTop, "每行圖示數", 4, 20, 1, 12, -414, nil, GetPerRow, function(v) A2_AutoOverrideCapsIfNeeded(); SetPerRow(v) end)
    ...
    local growthDD = CreateDropdown(leftTop, "增長方向", A2_DD_X, A2_DD_Y0 - (A2_DD_STEP * 9) - 8,
    ...
    do
        ...
        local cbTimerBuckets = CreateBoolCheckboxPath(timerBox, '依剩餘時間為光環計時器著色', 12, -34, GetGeneral, 'aurasCooldownTextUseBuckets', nil,
            '啟用時，光環冷卻文字會根據剩餘時間使用 安全 / 警告 / 緊急 顏色。\n停用時，光環冷卻文字將始終使用安全顏色。',
            function()
        ...
        local safeSlider = CreateAuras2CompactSlider(timerBox, '安全 (秒)', 0, 600, 1, 12, -72, 220, GetSafe, SetSafe)
        ...
        local warnSlider = CreateAuras2CompactSlider(timerBox, '警告 (<=)', 0, 30, 1, 260, -72, 200, GetWarn, SetWarn)
        ...
        local urgSlider = CreateAuras2CompactSlider(timerBox, '緊急 (<=)', 0, 15, 1, 486, -72, 200, GetUrg, SetUrg)
    ...
    do
        ...
        BuildBoolPathCheckboxes(advBox, {
            ...
            { "總是包含可竊取增益", 12, -114, A2_FilterBuffs, "includeStealable", nil,
                "附加：這不會隱藏你的一般增益。", "cbStealable" },

            { "總是包含可驅散減益", 12, -142, A2_FilterDebuffs, "includeDispellable", nil,
                "附加：這不會隱藏你的一般減益。", "cbDispellable" },

            { "僅顯示首領光環", 380, -58, GetEditingFilters, "onlyBossAuras", nil,
                "強制過濾：啟用時 (且過濾器啟用)，只會顯示被標記為首領光環的光環。", "cbOnlyBoss" },
        }, refs)
...
do
    ...
    for i = 1, #globalKeys do
        ...
        BuildBoolPathCheckboxes(advBox, {
            { "顯著標示可竊取增益", 12, -194, A2_Settings, "highlightStealableBuffs", nil,
                "顯著標示可竊取的增益 (僅視覺效果；不過濾)。", "cbHLSteal" },
            { "顯著標示可驅散減益", 12, -222, A2_Settings, "highlightDispellableDebuffs", nil,
                "顯著標示可驅散的減益 (僅視覺效果；不過濾)。", "cbHLDispel" },
        }, refs)
        -- ------------------------------------------------------------
        ...
        BuildBoolPathCheckboxes(privateBox, {
            { "顯示（玩家）", 12, -64, A2_Settings, "showPrivateAurasPlayer", nil,
                "將暴雪的私人光環重新錨定到 MSUF（不含法術清單）。", "cbPrivateShowP" },
            { "顯示（專注目標）", 12, -92, A2_Settings, "showPrivateAurasFocus", nil,
                "將暴雪的私人光環重新錨定到 MSUF 專注目標框架。", "cbPrivateShowF" },
            { "顯示（首領）", 12, -120, A2_Settings, "showPrivateAurasBoss", nil,
                "將暴雪的私人光環重新錨定到 MSUF 首領框架。", "cbPrivateShowB" },

            { "預覽", 12, -148, A2_Settings, "highlightPrivateAuras", nil,
                "僅視覺效果：在私人光環欄位加上紫色邊框與角落標記。", "cbPrivateHL" },
        }, refs)
