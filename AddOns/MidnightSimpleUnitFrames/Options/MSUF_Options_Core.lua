...
function MSUF_RegisterOptionsCategoryLazy()
    ...
    local function MSUF_RunAfterCombat(fn)
        if InCombatLockdown and InCombatLockdown() then
            ...
            if DEFAULT_CHAT_FRAME and DEFAULT_CHAT_FRAME.AddMessage then
                DEFAULT_CHAT_FRAME:AddMessage("|cffffaa00MSUF:|r 無法在戰鬥中開啟選單。將在戰鬥結束後開啟。")
            elseif print then
                print("MSUF: 無法在戰鬥中開啟選單。將在戰鬥結束後開啟。")
            end
            return
        end
    ...
    local function MSUF_BuildLauncherUI()
        ...
        desc:SetText("MSUF 透過遊戲內的 MSUF 選單進行設定。\n\n使用下方按鈕 (或輸入 /msuf) 開啟。")
...
editModeButton:SetScript("OnClick", function()
    ...
    if not movableKeys[currentKey] then
        print("|cffffd700MSUF:|r 編輯模式僅適用於單位分頁 (玩家/目標/ToT/專注/寵物/首領)。請選擇這些分頁之一。")
        return
    end
        ...
        elseif AudioOptionsFrame and AudioOptionsFrame:IsShown() then
            if HideUIPanel then
                ...
                AudioOptionsFrame:Hide()
                        ...
                        print("|cffffd700MSUF:|r " .. label .. " 編輯模式 |cff00ff00開啟|r – 使用滑鼠左鍵拖曳 " .. label .. " 框架或使用方向按鈕。")
        else
...
border:SetBackdrop({
    ...
    fs:SetPoint("CENTER")
    fs:SetFont("Fonts\\bHEI01B.ttf", 11, "OUTLINE")
...
local function MSUF_CreateGradientDirectionPad(parent)
    ...
    local function MakeDirButton(dirKey, glyph, dbKey)
        ...
        if b.text then
            ...
            b.text:SetFont("Fonts\\bHEI01B.ttf", 12, "OUTLINE")
        ...
        for k, btn in pairs(self.buttons) do
            ...
            if btn.text then
                if isOn then
                    ...
                    btn.text:SetFont("Fonts\\bHEI01B.ttf", 14, "OUTLINE")
                else
                    btn.text:SetTextColor(0.35, 0.35, 0.35, 1)
                    btn.text:SetFont("Fonts\\bHEI01B.ttf", 12, "OUTLINE")
                end
            end
        end
...
local function MSUF_StyleToggleText(cb)
    ...
    StaticPopupDialogs["MSUF_CONFIRM_RESET_PROFILE"] = {
        text = "重置所有字型大小覆蓋設定？\n\n這將清除各單位的 名稱/生命/能量 以及施法條的 名稱/時間 覆蓋設定，使一切繼承全域預設值。",
            button1 = YES,
        button2 = NO,
...
resetBtn:SetText("重置設定檔")
resetBtn:SetScript("OnClick", function()
    if not MSUF_ActiveProfile then
        print("|cffff0000MSUF:|r 未選擇要重置的設定檔。")
        return
    end
...
if MSUF_SkinMidnightActionButton then
    ...
    helpText:SetText("設定檔是全域的。每個角色選擇一個啟用的設定檔。在左側建立新設定檔或在右側選擇現有設定檔。")
...
deleteBtn:SetScript("OnClick", function()
    ...
    if name == "Default" then
        print("|cffff0000MSUF:|r 預設設定檔 'Default' 無法刪除。")
        return
    end
    ...
    local function MSUF_ShowCopyPopup(str)
        if not copyPopup then
            copyPopup, copyTitle = MSUF_CreateSimpleDialog("MSUF_ProfileCopyPopup", "Ctrl+C 複製", 560, 96)
    ...
    local function MSUF_ShowImportPopup(mode)
        ...
        if not importPopup then
            importPopup, importTitleFS = MSUF_CreateSimpleDialog("MSUF_ProfileImportPopup", "Ctrl+V 貼上", 560, 110)
            ...
            local function runImport()
                ...
                if type(Importer) ~= "function" then
                    print("|cffff0000MSUF:|r 匯入失敗: 找不到匯入器。")
                    return
                end
        ...
        if importTitleFS then
            if mode == "legacy" then
                importTitleFS:SetText("Ctrl+V 貼上 (舊版匯入)")
            else
                importTitleFS:SetText("Ctrl+V 貼上")
            end
        end
        ...
        if not exportPopup then
            ...
            local function doExport(kind)
                ...
                if type(Exporter) ~= "function" then
                    print("|cffff0000MSUF:|r 匯出失敗: 找不到匯出器 (MSUF_ExportSelectionToString)。")
                    exportPopup:Hide()
                    return
                end
...
castbarFocusButton:SetText("專注斷法")
if MSUF_SkinMidnightActionButton then
...
local function MSUF_SyncCastbarsTabToggles()
    ...
    if castbarFillDirDrop then
        ...
        if UIDropDownMenu_SetText then
            if dir == "LTR" then
                UIDropDownMenu_SetText(castbarFillDirDrop, "由左至右")
            else
                UIDropDownMenu_SetText(castbarFillDirDrop, "由右至左 (預設)")
            end
        end
...
if LSM then
    ...
    castbarTextureLabel:SetText("施法條材質 (SharedMedia)")
    ...
    castbarTextureInfo:SetText("安裝插件 'SharedMedia' (LibSharedMedia-3.0) 以選擇施法條材質。若無安裝，將使用預設材質。")
end
    ...
    castbarTexColorTitle:SetText("材質與蓄力施法")
    ...
    castbarFillDirLabel:SetText("施法條填充方向")
...
empowerStageBlinkTimeSlider:SetScript("OnShow", function(self)
    ...
    do
        ...
        if castbarTextureLabel and rightCol then
            ...
            castbarTextureLabel:SetText("施法條材質")
        end
...
do
    ...
    if not p then
        ...
        note:SetJustifyH("LEFT")
        note:SetText("光環設定現在由專用的 |cffffd200Auras 2.0|r 選單處理。\n\n此分頁現在僅為捷徑。")
...
absorbDisplayLabel:SetText("吸收盾顯示")
...
UIDropDownMenu_Initialize(absorbDisplayDrop, function(self, level)
    ...
    AddOption('關閉吸收盾',        1, false, false)
    AddOption('吸收盾條',        2, true,  false)
    AddOption('吸收盾條 + 文字', 3, true,  true)
    AddOption('僅吸收盾文字',  4, false, true)
end)
...
absorbAnchorLabel:SetText("吸收盾條對齊點")
...
UIDropDownMenu_Initialize(absorbAnchorDrop, function(self, level)
    ...
    AddOption("左: 吸收 | 右: 治療吸收", 1)
    AddOption("右: 吸收 | 左: 治療吸收", 2)
end)
...
absorbAnchorDrop:SetScript('OnShow', function(self)
    ...
    local text = (mode == 1) and "左: 吸收 | 右: 治療吸收" or "右: 吸收 | 左: 治療吸收"
    UIDropDownMenu_SetSelectedValue(self, mode)
    UIDropDownMenu_SetText(self, text)
end)
...
absorbTextureLabel:SetText("吸收盾條材質 (SharedMedia)")
...
if absorbTexTestCB then
    ...
    absorbTexTestCB.tooltip = "暫時顯示假的吸收與治療吸收覆蓋層，以便您預覽這些材質。\n\n當您離開此選單時將自動關閉。"
...
gradientCheck = CreateLabeledCheckButton(
    ...
    powerBarHeightLabel:SetText("能量條高度")
    ...
    powerBarBorderSizeLabel:SetText("邊框粗細")
    ...
    powerModeOptions = {
        { key = "FULL_SLASH_MAX",     label = "目前 / 最大值" },
        { key = "FULL_ONLY",          label = "僅顯示數值" },
        { key = "FULL_PLUS_PERCENT",  label = "數值 + 百分比" },
        { key = "PERCENT_PLUS_FULL",  label = "百分比 + 數值" },
        { key = "PERCENT_ONLY",       label = "僅顯示百分比" },
    ...
    local function HPSeparatorDropdown_Initialize(self, level)
        ...
        for _, opt in ipairs(hpSepOptions) do
            ...
            info.text  = (thisKey == "" and "空白 / 無" or thisLabel)
            info.value = thisKey
            info.func  = function(btn)
                EnsureDB()
                local g2 = MSUF_DB.general or {}
...
hpSpacerSelectedLabel:SetText("目前選擇: 玩家")
...
if hpSpacerCheck.text then
    hpSpacerCheck.text:SetText("間距 開/關")
end
...
hpSpacerSlider = CreateLabeledSlider("MSUF_HPTextSpacerSlider", "生命力文字間距 (X)", barGroup, 0, 1000, 1, 16, -200)
...
if powerSpacerSlider.SetWidth then
    ...
    local function RefreshHPSpacerControls()
        ...
        if hpSpacerSelectedLabel and hpSpacerSelectedLabel.SetText then
            local nice = unitKey
            if nice == "player" then nice = "玩家"
            elseif nice == "target" then nice = "目標"
            elseif nice == "focus" then nice = "專注"
            elseif nice == "targettarget" then nice = "ToT"
            elseif nice == "pet" then nice = "寵物"
            elseif nice == "boss" then nice = "首領"
            end
            hpSpacerSelectedLabel:SetText("目前選擇: " .. tostring(nice))
        end
...
local barTextureDrop
        ...
        do
            ...
            barTextureLabel:SetText("條列材質 (SharedMedia)")
            ...
            barTextureInfo:SetText('安裝 "SharedMedia" (LibSharedMedia-3.0) 以選擇更多材質。若無安裝，您仍可選擇暴雪內建材質。')
        ...
        do -- Bar background texture dropdown
            ...
            barBgTextureLabel:SetText("條列背景材質")
            ...
            local function BarBgTextureDropdown_Initialize(self, level)
                ...
                info.func = function(btn)
                    ...
                    UIDropDownMenu_SetText(barBgTextureDrop, "使用前景材質")
                end
...
do
    ...
    if not _G["MSUF_BarsMenuPanelLeft"] then
        ...
        borderHeader:SetText("邊框與文字選項") -- 原文: Border & Text Options
    ...
    if absorbDisplayLabel and _G.MSUF_BarsMenuAbsorbHeader then
        ...
        absorbDisplayLabel:SetText("吸收量顯示") -- 原文: Absorb Display
    end
...
if absorbTextureLabel and absorbAnchorDrop then
    ...
    absorbTextureLabel:SetText("吸收條材質 (SharedMedia)") -- 原文: Absorb bar texture (SharedMedia)
end
...
if absorbBarTextureDrop and absorbTextureLabel then
    ...
    if _G.MSUF_BarsMenu_MakeInlineDropdown then
        _G.MSUF_BarsMenu_MakeInlineDropdown(absorbBarTextureDrop, "吸收", nil, "CENTER") -- 原文: Absorb
    end
end
...
if healAbsorbTextureDrop and absorbBarTextureDrop then
    ...
    if _G.MSUF_BarsMenu_MakeInlineDropdown then
        _G.MSUF_BarsMenu_MakeInlineDropdown(healAbsorbTextureDrop, "治療吸收", nil, "CENTER") -- 原文: Heal-Absorb
    end
end
...
-- Textures section (foreground + background)
    ...
    if barTextureLabel and texHeader then
        ...
        barTextureLabel:SetText("條列材質 (SharedMedia)") -- 原文: Bar texture (SharedMedia)
    end
    ...
    if barTextureDrop and barTextureLabel then
        ...
        if _G.MSUF_BarsMenu_MakeInlineDropdown then
            ...
            _G.MSUF_BarsMenu_MakeInlineDropdown(barTextureDrop, "前景", nil, "CENTER") -- 原文: Foreground
        end
    end
    ...
    if barBgTextureDrop and barTextureDrop then
        ...
        if _G.MSUF_BarsMenu_MakeInlineDropdown then
            -- Keep label on the left, show the selected texture name centered.
            _G.MSUF_BarsMenu_MakeInlineDropdown(barBgTextureDrop, "背景", nil, "CENTER") -- 原文: Background
        end
    end
...
if leftPanel and not outlineHeader then
    ...
    outlineHeader:SetText("輪廓粗細") -- 原文: Outline thickness
end
...
if panel.bossPortraitDrop and panel.bossPortraitLabel then
    if currentKey == "boss" then
        ...
        local textLabel = "頭像關閉" -- 原文: Portrait Off
        if mode == "LEFT" then
            textLabel = "頭像靠左" -- 原文: Portrait Left
        elseif mode == "RIGHT" then
            textLabel = "頭像靠右" -- 原文: Portrait Right
        end
...
MSUF_CallUpdateAllFonts()
    ...
    if not rootCat and Settings and Settings.RegisterCanvasLayoutCategory then
        ...
        local launcher = (_G and _G.MSUF_LauncherPanel) or CreateFrame("Frame")
        if _G then _G.MSUF_LauncherPanel = launcher end
        launcher.name = "Midnight 簡易單位框架" -- 原文: Midnight Simple Unit Frames
    ...
    if Settings and Settings.RegisterCanvasLayoutSubcategory and rootCat then
        if not (_G and _G.MSUF_LegacyCategory) then
            local legacyCat = Settings.RegisterCanvasLayoutSubcategory(rootCat, panel, (panel.name or "舊版設定")) -- 原文: Legacy
