...
function ns.MSUF_Options_Misc_Build(panel, miscGroup)
    ...
    miscLeftHeader:SetText("滑鼠指向與更新")
    ...
    miscRightHeader:SetText("單位資訊面板")
    ...
    updateThrottleLabel:SetText("單位更新頻率 (秒)")
    ...
    MSUF_CastbarUpdateLabel:SetText("施法條更新")
...
infoTooltipDisableCheck.text:SetText("停用 MSUF 單位資訊面板提示")
    ...
    infoTooltipPosLabel:SetText("MSUF 單位資訊面板位置")
    ...
    infoTooltipPosDrop:SetScript("OnShow", function(self)
        ...
        if current == "modern" then
            UIDropDownMenu_SetText(self, "現代 (游標下方)")
        else
            UIDropDownMenu_SetText(self, "暴雪經典")
        end
    end)
    ...
    if not StaticPopupDialogs["MSUF_RELOAD_PLAYERFRAME_HIDE_MODE"] then
        StaticPopupDialogs["MSUF_RELOAD_PLAYERFRAME_HIDE_MODE"] = {
            text = "這會變更 MSUF 隱藏暴雪玩家框架的方式。\n\n關閉：相容模式 (保持玩家框架存活，作為資源條插件的隱藏父級)。\n開啟：強制隱藏模式 (完全隱藏玩家框架；可能會導致某些資源條插件失效)。\n\n需要重新載入介面。",
            button1 = RELOADUI,
            button2 = CANCEL,
    ...
    hardKillPFCheck.text:SetText("完全隱藏暴雪玩家框架 - 若需相容資源條插件請關閉")
    ...
    hardKillPFCheck:SetScript("OnEnter", function(self)
        ...
        GameTooltip:SetText("隱藏暴雪玩家框架 (若需相容其他插件請關閉)", 1, 0.9, 0.4)
        GameTooltip:AddLine("關閉：保持玩家框架存活作為隱藏父級。", 0.95, 0.95, 0.95, true)
        GameTooltip:AddLine("開啟：完全隱藏玩家框架 (可能導致某些資源條插件失效)。", 1, 0.82, 0.2, true)
    ...
    if minimapIconCheck.Text then
        minimapIconCheck.Text:SetText("顯示 MSUF 小地圖圖示")
...
local bottomPanel = CreateFrame("Frame", nil, miscGroup, "BackdropTemplate")
  ...
  centerDivider:SetPoint("BOTTOM", bottomPanel, "BOTTOMLEFT", leftW, 12)
            ...
            if not ufcoreBudgetSlider then
                ...
                ufcoreBudgetSlider.tooltipText = "限制每幀 UFCore 的運算時間 (毫秒)。較低 = 較平滑 (較少卡頓)，較高 = 更新更即時。"
            ...
            if not ufcoreUrgentSlider then
                ...
                ufcoreUrgentSlider.tooltipText = "限制每次刷新時的緊急單位更新數量。較低 = 卡頓較小，較高 = 追趕更新較快。"
            ...
            if updateSlider then
                ...
                local lbl = MakeLabel(leftPanel, "單位更新頻率 (秒)", "TOPLEFT", leftPanel, 14, -50)
            ...
            if castbarUpdateSlider then
                ...
                local lbl = MakeLabel(leftPanel, "施法條更新", "TOPLEFT", rel, (rel == leftPanel and 14) or 0, (rel == leftPanel and -130) or -36)
            ...
            if ufcoreBudgetSlider then
                ...
                local lbl = MakeLabel(leftPanel, "UFCore 刷新預算", "TOPLEFT", rel, (rel == leftPanel and 14) or 0, (rel == leftPanel and -130) or -36)
                if rel ~= leftPanel then
                    lbl:ClearAllPoints()
                    lbl:SetPoint("TOPLEFT", rel, "BOTTOMLEFT", 0, -16)
                end
            ...
            if ufcoreUrgentSlider then
                ...
                local lbl = MakeLabel(leftPanel, "UFCore 緊急上限", "TOPLEFT", rel, (rel == leftPanel and 14) or 0, (rel == leftPanel and -130) or -36)
                if rel ~= leftPanel then
                    lbl:ClearAllPoints()
                    lbl:SetPoint("TOPLEFT", rel, "BOTTOMLEFT", 0, -16)
                end
            ...
            if infoTooltipPosDrop then
                ...
                local lbl = MakeLabel(rightPanel, "MSUF 單位資訊面板位置", "TOPLEFT", rel, 0, (rel == rightPanel and -50) or -28)
            ...
            if blizzUFDisable then
                ...
                if not rightPanel._msufBlizzHeader then
                    rightPanel._msufBlizzHeader = MakeLabel(rightPanel, "暴雪框架", "TOPLEFT", rel, 0, -35)
...
if minimapIconCheck then
    ...
    if targetSoundsCheck then
        ...
        StyleCheckbox(targetSoundsCheck)
            ...
            local rightHeader = MakeLabel(bottomPanel, "狀態指示器", "TOPLEFT", bottomPanel, leftX, -34)
            ...
            local y0 = -10
            local cbAFK   = MakeStatusCB("showAFK",   "顯示暫離 (AFK)",   y0)
            local cbDND   = MakeStatusCB("showDND",   "顯示勿擾 (DND)",   y0 - step)
            local cbDead  = MakeStatusCB("showDead",  "顯示死亡",  y0 - (step * 2))
            local cbGhost = MakeStatusCB("showGhost", "顯示靈魂", y0 - (step * 3))
