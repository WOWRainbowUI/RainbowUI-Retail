...
local MSUF_A2_GetStealableBorderRGB = _G and _G.MSUF_A2_GetStealableBorderRGB or nil
...
if type(MSUF_A2_GetStealableBorderRGB) ~= "function" then
    MSUF_A2_GetStealableBorderRGB = function() return 0.0, 0.75, 1.0 end
end
...
local A2_SHARED_DEFAULTS = {
    ...
    highlightStealableBuffs=true, highlightDispellableDebuffs=true,
...
local function MSUF_A2_NormalizeFilters(f, sharedSettings, migrateFlagKey)
    ...
    A2_Default(b, "onlyMine", false); A2_Default(b, "includeBoss", false); A2_Default(b, "includeStealable", false)
...
-- (Phase 5) Dispel/Stealable border colors moved to Auras2/MSUF_A2_Colors.lua
...
local function A2_AnchorStacked(entry, by)
...
local function UpdateAnchor(entry, shared, offX, offY, boxW, boxH, layoutModeOverride, buffDebuffAnchorOverride, splitSpacingOverride)
    ...
    local function Place(container, pos)
        container:ClearAllPoints()
        if pos == "ABOVE" then
            container:SetPoint("BOTTOMLEFT", entry.frame, "TOPLEFT", x, y + splitSpacing)
        elseif pos == "BELOW" then
            container:SetPoint("BOTTOMLEFT", entry.frame, "BOTTOMLEFT", x, y - iconSize - splitSpacing)
        elseif pos == "RIGHT" then
            container:SetPoint("BOTTOMLEFT", entry.frame, "RIGHT", x + splitSpacing, y - (iconSize * 0.5))
        elseif pos == "LEFT" then
            container:SetPoint("BOTTOMLEFT", entry.frame, "LEFT", x - iconSize - splitSpacing, y - (iconSize * 0.5))
        else
            container:SetPoint("BOTTOMLEFT", entry.anchor, "BOTTOMLEFT", 0, 0)
        end
    end
    ...
    if anchorMode == "STACKED" or anchorMode == nil then
        A2_AnchorStacked(entry, buffOffsetY)
        return
    end
    ...
    Place(entry.buffs, map.buffs)
    Place(entry.debuffs, map.debuffs)
...
local function SetDispelBorder(icon, unit, aura, isHelpful, shared, allowHighlights, isOwn)
    ...
    -- Stealable buff border
    if isHelpful then
        if shared and shared.highlightStealableBuffs == true then
            local stealable = (aura and aura._msufA2_previewStealable == true) and true or false
            if (not stealable) and unit and auraInstanceID and C_UnitAuras and type(C_UnitAuras.GetAuraDataByAuraInstanceID) == "function" then
                local ok, auraData = pcall(C_UnitAuras.GetAuraDataByAuraInstanceID, unit, auraInstanceID)
                if ok and type(auraData) == "table" and auraData.isStealable == true then
                    stealable = true
                end
            end
            if stealable then
...
local function MSUF_A2_BuildMergedAuraList(entry, unit, filter, baseShow, onlyMine, includeBoss, wantExtra, extraKind)
    ...
    if type(all) == "table" then
        ...
        elseif extraKind == "stealable" then
            for i = 1, #all do
                local aura = all[i]
...
local function MSUF_A2_ApplyPreviewIcon(icon, tex, spellId, opts, unit, shared, stackCountAnchor)
    ...
    pa.spellId = spellId
    pa._msufA2_previewStealable = (opts and opts.stealable) and true or false
...
if MSUF_A2_PrivateAuras_RebuildIfNeeded then
    ...
    local showExtraStealable = (masterOn and tf and tf.buffs and tf.buffs.includeStealable == true) or false
    ...
    if unitExists and not showTest then
        ...
        if type(buckets) ~= "table" then
            buckets = {
                ...
                { filter="HELPFUL", kind="stealable",  helpful=true,  listKey="buffs",   iKey="iBuff",   lenKey="buffsLen"   },
            }
        ...
        buckets[2].want, buckets[2].base, buckets[2].only, buckets[2].boss, buckets[2].extra, buckets[2].cap, buckets[2].hidePerm =
            wantBuffs, baseShowBuffs, buffsOnlyMine, buffsIncludeBoss, showExtraStealable, buffCap, (hidePermanentBuffs == true)
